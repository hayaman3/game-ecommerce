export const gameData = {
  results: [
    {
      id: 19709,
      slug: "half-life-2-episode-two",
      name: "Half-Life 2: Episode Two",
      released: "2007-10-10",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/198/1988a337305e008b41d7f536ce9b73f6.jpg",
      rating: 4.45,
      rating_top: 5,
      ratings: [
        { id: 5, title: "exceptional", count: 1144, percent: 59.4 },
        { id: 4, title: "recommended", count: 637, percent: 33.07 },
        { id: 3, title: "meh", count: 75, percent: 3.89 },
        { id: 1, title: "skip", count: 70, percent: 3.63 },
      ],
      ratings_count: 1918,
      reviews_text_count: 4,
      added: 10587,
      added_by_status: {
        yet: 592,
        owned: 7524,
        beaten: 2208,
        toplay: 105,
        dropped: 138,
        playing: 20,
      },
      metacritic: 90,
      playtime: 5,
      suggestions_count: 298,
      updated: "2024-03-17T16:15:31",
      user_game: null,
      reviews_count: 1926,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 21,
            name: "Android",
            slug: "android",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 52330,
            image_background:
              "https://media.rawg.io/media/games/530/5302dd22a190e664531236ca724e8726.jpg",
          },
          released_at: "2007-10-10",
          requirements_en: { minimum: "4.4 and up" },
          requirements_ru: null,
        },
        {
          platform: {
            id: 6,
            name: "Linux",
            slug: "linux",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 76454,
            image_background:
              "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
          },
          released_at: "2007-10-10",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 5,
            name: "macOS",
            slug: "macos",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 103400,
            image_background:
              "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
          },
          released_at: "2007-10-10",
          requirements_en: {
            minimum:
              "<strong>Minimum: </strong>OS X version Leopard 10.5.8, Snow Leopard 10.6.3, 1GB RAM, NVIDIA GeForce 8 or higher, ATI X1600 or higher, or Intel HD 3000 or higher Mouse, Keyboard, Internet Connection",
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2007-10-10",
          requirements_en: {
            minimum:
              '<p><strong>Minimum: </strong>1.7 GHz Processor, 512MB RAM, DirectX&reg; 8.1 level Graphics Card (Requires support for SSE), Windows&reg; 7 (32/64-bit)/Vista/XP, Mouse, Keyboard, Internet Connection</p><br>\t\t\t<p><strong>Recommended: </strong>Pentium 4 processor (3.0GHz, or better), 1GB RAM, DirectX&reg; 9 level Graphics Card, Windows&reg; 7 (32/64-bit)/Vista/XP, Mouse, Keyboard, Internet Connection</p><br>\t\t\t<br><img src="gfx/apps_extras/OB/nvidia.jpg">',
          },
          requirements_ru: {
            minimum:
              "Pentium 4/Athlon XP 1.7 ГГц,512 Мб памяти,3D-ускоритель со 128 Мб памяти,7.5 Гб на винчестере,Интернет-соединение 128 Кб/c",
            recommended:
              "Core 2 Duo/Athlon 64 3 ГГц,1 Гб памяти,3D-ускоритель с 512 Мб памяти,12 Гб на винчестере,Интернет-соединение 512 Кб/c",
          },
        },
        {
          platform: {
            id: 14,
            name: "Xbox 360",
            slug: "xbox360",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 2796,
            image_background:
              "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
          },
          released_at: "2007-10-10",
          requirements_en: null,
          requirements_ru: null,
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 3, name: "Xbox", slug: "xbox" } },
        { platform: { id: 8, name: "Android", slug: "android" } },
        { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
        { platform: { id: 6, name: "Linux", slug: "linux" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
        {
          id: 2,
          name: "Shooter",
          slug: "shooter",
          games_count: 59459,
          image_background:
            "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
        },
        {
          id: 7,
          name: "Puzzle",
          slug: "puzzle",
          games_count: 97242,
          image_background:
            "https://media.rawg.io/media/games/9f4/9f418898f5415668ca47b5f4ab1ecfeb.jpg",
        },
      ],
      stores: [
        {
          id: 22130,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
        {
          id: 218334,
          store: {
            id: 8,
            name: "Google Play",
            slug: "google-play",
            domain: "play.google.com",
            games_count: 17079,
            image_background:
              "https://media.rawg.io/media/games/13a/13a528ac9cf48bbb6be5d35fe029336d.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 40847,
          name: "Steam Achievements",
          slug: "steam-achievements",
          language: "eng",
          games_count: 35702,
          image_background:
            "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
        },
        {
          id: 7,
          name: "Multiplayer",
          slug: "multiplayer",
          language: "eng",
          games_count: 37130,
          image_background:
            "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
        },
        {
          id: 40849,
          name: "Steam Cloud",
          slug: "steam-cloud",
          language: "eng",
          games_count: 16769,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 42,
          name: "Great Soundtrack",
          slug: "great-soundtrack",
          language: "eng",
          games_count: 3386,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 118,
          name: "Story Rich",
          slug: "story-rich",
          language: "eng",
          games_count: 20561,
          image_background:
            "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
        },
        {
          id: 8,
          name: "First-Person",
          slug: "first-person",
          language: "eng",
          games_count: 30626,
          image_background:
            "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
        },
        {
          id: 32,
          name: "Sci-fi",
          slug: "sci-fi",
          language: "eng",
          games_count: 18535,
          image_background:
            "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
        },
        {
          id: 40845,
          name: "Partial Controller Support",
          slug: "partial-controller-support",
          language: "eng",
          games_count: 11229,
          image_background:
            "https://media.rawg.io/media/games/62c/62c7c8b28a27b83680b22fb9d33fc619.jpg",
        },
        {
          id: 16,
          name: "Horror",
          slug: "horror",
          language: "eng",
          games_count: 43534,
          image_background:
            "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
        },
        {
          id: 30,
          name: "FPS",
          slug: "fps",
          language: "eng",
          games_count: 12893,
          image_background:
            "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
        },
        {
          id: 193,
          name: "Classic",
          slug: "classic",
          language: "eng",
          games_count: 1783,
          image_background:
            "https://media.rawg.io/media/games/960/960b601d9541cec776c5fa42a00bf6c4.jpg",
        },
        {
          id: 11669,
          name: "stats",
          slug: "stats",
          language: "eng",
          games_count: 4821,
          image_background:
            "https://media.rawg.io/media/games/f52/f5206d55f918edf8ee07803101106fa6.jpg",
        },
        {
          id: 63,
          name: "Zombies",
          slug: "zombies",
          language: "eng",
          games_count: 10103,
          image_background:
            "https://media.rawg.io/media/games/cee/cee577e2097a59b77193fe2bce94667d.jpg",
        },
        {
          id: 43,
          name: "Post-apocalyptic",
          slug: "post-apocalyptic",
          language: "eng",
          games_count: 3777,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        {
          id: 40833,
          name: "Captions available",
          slug: "captions-available",
          language: "eng",
          games_count: 1311,
          image_background:
            "https://media.rawg.io/media/games/7a2/7a2500ee8b2c0e1ff268bb4479463dea.jpg",
        },
        {
          id: 172,
          name: "Aliens",
          slug: "aliens",
          language: "eng",
          games_count: 6591,
          image_background:
            "https://media.rawg.io/media/games/ae1/ae1518c3dc1e847344661905fd2a8d16.jpg",
        },
        {
          id: 119,
          name: "Dystopian",
          slug: "dystopian",
          language: "eng",
          games_count: 2001,
          image_background:
            "https://media.rawg.io/media/games/a0e/a0ef08621301a1eab5e04fa5c96978fa.jpeg",
        },
        {
          id: 40834,
          name: "Commentary available",
          slug: "commentary-available",
          language: "eng",
          games_count: 266,
          image_background:
            "https://media.rawg.io/media/games/594/5949baae74fe9e399adbce0c44e28783.jpg",
        },
        {
          id: 232,
          name: "Episodic",
          slug: "episodic",
          language: "eng",
          games_count: 459,
          image_background:
            "https://media.rawg.io/media/games/a28/a289e23b4d4d84f26ab59125e3be4483.jpg",
        },
        {
          id: 40839,
          name: "Includes Source SDK",
          slug: "includes-source-sdk",
          language: "eng",
          games_count: 62,
          image_background:
            "https://media.rawg.io/media/screenshots/ae8/ae873e0da5973a9896bdf5ebce1e9bf4.jpg",
        },
        {
          id: 319,
          name: "Silent Protagonist",
          slug: "silent-protagonist",
          language: "eng",
          games_count: 91,
          image_background:
            "https://media.rawg.io/media/games/198/1988a337305e008b41d7f536ce9b73f6.jpg",
        },
      ],
      esrb_rating: { id: 4, name: "Mature", slug: "mature" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/198/1988a337305e008b41d7f536ce9b73f6.jpg",
        },
        {
          id: 185826,
          image:
            "https://media.rawg.io/media/screenshots/04b/04b62b9115ccd64ebac1e8d813c69d08.jpg",
        },
        {
          id: 185827,
          image:
            "https://media.rawg.io/media/screenshots/278/278f1654ade9d116ca1bb42bb37d94ba.jpg",
        },
        {
          id: 185828,
          image:
            "https://media.rawg.io/media/screenshots/0f2/0f2b40d5aa1eb776fdedcdb4d11aca8e.jpg",
        },
        {
          id: 185829,
          image:
            "https://media.rawg.io/media/screenshots/895/895f1c364465b338e2d1b41b0b4eff67.jpg",
        },
        {
          id: 185830,
          image:
            "https://media.rawg.io/media/screenshots/016/01638fb0f73a6acde1714a107e078807.jpg",
        },
      ],
    },
    {
      id: 4427,
      slug: "bioshock-2",
      name: "BioShock 2",
      released: "2010-02-09",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/157/15742f2f67eacff546738e1ab5c19d20.jpg",
      rating: 4.07,
      rating_top: 4,
      ratings: [
        { id: 4, title: "recommended", count: 992, percent: 52.57 },
        { id: 5, title: "exceptional", count: 602, percent: 31.9 },
        { id: 3, title: "meh", count: 205, percent: 10.86 },
        { id: 1, title: "skip", count: 88, percent: 4.66 },
      ],
      ratings_count: 1873,
      reviews_text_count: 8,
      added: 10577,
      added_by_status: {
        yet: 767,
        owned: 7292,
        beaten: 1830,
        toplay: 257,
        dropped: 388,
        playing: 43,
      },
      metacritic: 88,
      playtime: 3,
      suggestions_count: 645,
      updated: "2024-03-20T02:21:07",
      user_game: null,
      reviews_count: 1887,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 18,
            name: "PlayStation 4",
            slug: "playstation4",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 6767,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
          released_at: "2010-02-09",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 1,
            name: "Xbox One",
            slug: "xbox-one",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5590,
            image_background:
              "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
          },
          released_at: "2010-02-09",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 7,
            name: "Nintendo Switch",
            slug: "nintendo-switch",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5406,
            image_background:
              "https://media.rawg.io/media/games/5eb/5eb49eb2fa0738fdb5bacea557b1bc57.jpg",
          },
          released_at: "2010-02-09",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2010-02-09",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows XP, Vista, Windows 7<br></li><li><strong>Processor:</strong> AMD Athlon 64 Processor 3800+ 2.4Ghz or better, Intel Pentium 4 530 3.0Ghz Processor or better<br></li><li><strong>Memory:</strong> 2GB<br></li><li><strong>Graphics:</strong> NVIDIA 7800GT 256MB graphics card or better, ATI Radeon X1900 256MB graphics card or better<br></li><li><strong>DirectX®:</strong> DirectX 9.0c<br></li><li><strong>Hard Drive:</strong> 11GB<br></li><li><strong>Sound:</strong> 100% DirectX 9.0C compliant sound card or onboard sound</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>Processor:</strong> AMD Athlon 64 X2 5200+ Dual Core 2.60Ghz, Intel Core 2 Duo E6420 Dual Core 2.13Ghz<br></li><li><strong>Memory:</strong> 3GB<br></li><li><strong>Graphics:</strong> NVIDIA 8800GT 512MB graphics card or better, ATI Radeon HD4830 512MB graphics card or better<br></li><li><strong>Other Requirements:</strong> Initial installation requires one-time internet connection; software installations required including Microsoft Visual C++2008 Runtime Libraries, Microsoft DirectX.</li></ul>',
          },
          requirements_ru: {
            minimum:
              "Pentium 4 3 ГГц/Athlon 64 3800+,2 Гб памяти,GeForce 7800 GT/Radeon X1900,11 Гб на винчестере,интернет-соединение",
            recommended:
              "Core 2 Duo/Athlon X2 2.5 ГГц,3 Гб памяти,GeForce 8800/Radeon HD 2600,11 Гб на винчестере,интернет-соединение",
          },
        },
        {
          platform: {
            id: 5,
            name: "macOS",
            slug: "macos",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 103400,
            image_background:
              "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
          },
          released_at: "2010-02-09",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 14,
            name: "Xbox 360",
            slug: "xbox360",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 2796,
            image_background:
              "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
          },
          released_at: "2010-02-09",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 16,
            name: "PlayStation 3",
            slug: "playstation3",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 3168,
            image_background:
              "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
          },
          released_at: "2010-02-09",
          requirements_en: null,
          requirements_ru: null,
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
        { platform: { id: 3, name: "Xbox", slug: "xbox" } },
        { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
        { platform: { id: 7, name: "Nintendo", slug: "nintendo" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
        {
          id: 2,
          name: "Shooter",
          slug: "shooter",
          games_count: 59459,
          image_background:
            "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
        },
      ],
      stores: [
        {
          id: 79405,
          store: {
            id: 4,
            name: "App Store",
            slug: "apple-appstore",
            domain: "apps.apple.com",
            games_count: 75436,
            image_background:
              "https://media.rawg.io/media/games/f46/f466571d536f2e3ea9e815ad17177501.jpg",
          },
        },
        {
          id: 440408,
          store: {
            id: 2,
            name: "Xbox Store",
            slug: "xbox-store",
            domain: "microsoft.com",
            games_count: 4813,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
        },
        {
          id: 19732,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
        {
          id: 4755,
          store: {
            id: 3,
            name: "PlayStation Store",
            slug: "playstation-store",
            domain: "store.playstation.com",
            games_count: 7900,
            image_background:
              "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
          },
        },
        {
          id: 33604,
          store: {
            id: 7,
            name: "Xbox 360 Store",
            slug: "xbox360",
            domain: "marketplace.xbox.com",
            games_count: 1912,
            image_background:
              "https://media.rawg.io/media/games/d1a/d1a2e99ade53494c6330a0ed945fe823.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 40847,
          name: "Steam Achievements",
          slug: "steam-achievements",
          language: "eng",
          games_count: 35702,
          image_background:
            "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
        },
        {
          id: 7,
          name: "Multiplayer",
          slug: "multiplayer",
          language: "eng",
          games_count: 37130,
          image_background:
            "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
        },
        {
          id: 40836,
          name: "Full controller support",
          slug: "full-controller-support",
          language: "eng",
          games_count: 16746,
          image_background:
            "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 42,
          name: "Great Soundtrack",
          slug: "great-soundtrack",
          language: "eng",
          games_count: 3386,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 24,
          name: "RPG",
          slug: "rpg",
          language: "eng",
          games_count: 19865,
          image_background:
            "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
        },
        {
          id: 118,
          name: "Story Rich",
          slug: "story-rich",
          language: "eng",
          games_count: 20561,
          image_background:
            "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
        },
        {
          id: 8,
          name: "First-Person",
          slug: "first-person",
          language: "eng",
          games_count: 30626,
          image_background:
            "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
        },
        {
          id: 32,
          name: "Sci-fi",
          slug: "sci-fi",
          language: "eng",
          games_count: 18535,
          image_background:
            "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
        },
        {
          id: 16,
          name: "Horror",
          slug: "horror",
          language: "eng",
          games_count: 43534,
          image_background:
            "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
        },
        {
          id: 30,
          name: "FPS",
          slug: "fps",
          language: "eng",
          games_count: 12893,
          image_background:
            "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
        },
        {
          id: 97,
          name: "Action RPG",
          slug: "action-rpg",
          language: "eng",
          games_count: 6501,
          image_background:
            "https://media.rawg.io/media/games/157/15742f2f67eacff546738e1ab5c19d20.jpg",
        },
        {
          id: 41,
          name: "Dark",
          slug: "dark",
          language: "eng",
          games_count: 15192,
          image_background:
            "https://media.rawg.io/media/games/490/49016e06ae2103881ff6373248843069.jpg",
        },
        {
          id: 119,
          name: "Dystopian",
          slug: "dystopian",
          language: "eng",
          games_count: 2001,
          image_background:
            "https://media.rawg.io/media/games/a0e/a0ef08621301a1eab5e04fa5c96978fa.jpeg",
        },
        {
          id: 154,
          name: "Steampunk",
          slug: "steampunk",
          language: "eng",
          games_count: 1169,
          image_background:
            "https://media.rawg.io/media/games/4e6/4e6e8e7f50c237d76f38f3c885dae3d2.jpg",
        },
        {
          id: 305,
          name: "Linear",
          slug: "linear",
          language: "eng",
          games_count: 5442,
          image_background:
            "https://media.rawg.io/media/games/569/569ea25d2b56bd05c7fa309ddabe81ff.jpg",
        },
        {
          id: 250,
          name: "Underwater",
          slug: "underwater",
          language: "eng",
          games_count: 2048,
          image_background:
            "https://media.rawg.io/media/games/386/3869a2540b5fd39776fd640686610d05.jpg",
        },
        {
          id: 281,
          name: "Philisophical",
          slug: "philisophical",
          language: "eng",
          games_count: 39,
          image_background:
            "https://media.rawg.io/media/screenshots/573/573f6403cf6de862ca5b886db8cf449a.jpg",
        },
      ],
      esrb_rating: null,
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/157/15742f2f67eacff546738e1ab5c19d20.jpg",
        },
        {
          id: 162566,
          image:
            "https://media.rawg.io/media/screenshots/a13/a130b342c9830f9c56d65c204638fe17.jpg",
        },
        {
          id: 162567,
          image:
            "https://media.rawg.io/media/screenshots/f38/f38a519f1545ef5cda66676c155cc5b8.jpg",
        },
        {
          id: 162568,
          image:
            "https://media.rawg.io/media/screenshots/996/996d1459d3f2ec1f03daba488d96c521.jpg",
        },
        {
          id: 162569,
          image:
            "https://media.rawg.io/media/screenshots/8da/8daab3b2c3d9e355f636e7d408a08315.jpg",
        },
        {
          id: 162570,
          image:
            "https://media.rawg.io/media/screenshots/f5c/f5c713b706c6b153b6a22ca1b08a1f5a.jpg",
        },
        {
          id: 162571,
          image:
            "https://media.rawg.io/media/screenshots/b7c/b7c32275a6be8134744322f1204ce176.jpg",
        },
      ],
    },
    {
      id: 9767,
      slug: "hollow-knight",
      name: "Hollow Knight",
      released: "2017-02-23",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
      rating: 4.4,
      rating_top: 5,
      ratings: [
        { id: 5, title: "exceptional", count: 1561, percent: 61.97 },
        { id: 4, title: "recommended", count: 649, percent: 25.76 },
        { id: 3, title: "meh", count: 192, percent: 7.62 },
        { id: 1, title: "skip", count: 117, percent: 4.64 },
      ],
      ratings_count: 2466,
      reviews_text_count: 33,
      added: 10557,
      added_by_status: {
        yet: 816,
        owned: 6234,
        beaten: 1521,
        toplay: 613,
        dropped: 859,
        playing: 514,
      },
      metacritic: 88,
      playtime: 6,
      suggestions_count: 503,
      updated: "2024-03-19T12:43:47",
      user_game: null,
      reviews_count: 2519,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 18,
            name: "PlayStation 4",
            slug: "playstation4",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 6767,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
          released_at: "2017-02-23",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 6,
            name: "Linux",
            slug: "linux",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 76454,
            image_background:
              "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
          },
          released_at: "2017-02-23",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 16.04 LTS (64bit)<br></li><li><strong>Processor:</strong> Intel Core 2 Duo E5200<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> GeForce 9800GTX+ (1GB)<br></li><li><strong>Storage:</strong> 9 GB available space<br></li><li><strong>Additional Notes:</strong> 1080p, 16:9 recommended</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 16.04 LTS (64bit)<br></li><li><strong>Processor:</strong> Intel Core i5<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> GeForce GTX 560<br></li><li><strong>Storage:</strong> 9 GB available space<br></li><li><strong>Additional Notes:</strong> 1080p, 16:9 recommended</li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 5,
            name: "macOS",
            slug: "macos",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 103400,
            image_background:
              "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
          },
          released_at: "2017-02-23",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Mac OS 10.7 Lion<br></li><li><strong>Processor:</strong> Intel Core i3<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> GeForce GTX 470<br></li><li><strong>Storage:</strong> 9 GB available space<br></li><li><strong>Additional Notes:</strong> 1080p, 16:9 recommended</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Mac OS 10.12 Sierra<br></li><li><strong>Processor:</strong> Intel Core i5<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> GeForce GTX 570<br></li><li><strong>Storage:</strong> 9 GB available space<br></li><li><strong>Additional Notes:</strong> 1080p, 16:9 recommended</li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2017-02-23",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows 7<br></li><li><strong>Processor:</strong> Intel Core 2 Duo E5200<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> GeForce 9800GTX+ (1GB)<br></li><li><strong>DirectX:</strong> Version 10<br></li><li><strong>Storage:</strong> 9 GB available space<br></li><li><strong>Additional Notes:</strong> 1080p, 16:9 recommended</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows 10<br></li><li><strong>Processor:</strong> Intel Core i5<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> GeForce GTX 560<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Storage:</strong> 9 GB available space<br></li><li><strong>Additional Notes:</strong> 1080p, 16:9 recommended</li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 7,
            name: "Nintendo Switch",
            slug: "nintendo-switch",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5406,
            image_background:
              "https://media.rawg.io/media/games/5eb/5eb49eb2fa0738fdb5bacea557b1bc57.jpg",
          },
          released_at: "2017-02-23",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 1,
            name: "Xbox One",
            slug: "xbox-one",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5590,
            image_background:
              "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
          },
          released_at: "2017-02-23",
          requirements_en: null,
          requirements_ru: null,
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
        { platform: { id: 3, name: "Xbox", slug: "xbox" } },
        { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
        { platform: { id: 6, name: "Linux", slug: "linux" } },
        { platform: { id: 7, name: "Nintendo", slug: "nintendo" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
        {
          id: 51,
          name: "Indie",
          slug: "indie",
          games_count: 62257,
          image_background:
            "https://media.rawg.io/media/games/9dd/9ddabb34840ea9227556670606cf8ea3.jpg",
        },
        {
          id: 83,
          name: "Platformer",
          slug: "platformer",
          games_count: 100773,
          image_background:
            "https://media.rawg.io/media/games/fc8/fc838d98c9b944e6a15176eabf40bee8.jpg",
        },
      ],
      stores: [
        {
          id: 10646,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
        {
          id: 26919,
          store: {
            id: 5,
            name: "GOG",
            slug: "gog",
            domain: "gog.com",
            games_count: 5817,
            image_background:
              "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
          },
        },
        {
          id: 48908,
          store: {
            id: 6,
            name: "Nintendo Store",
            slug: "nintendo",
            domain: "nintendo.com",
            games_count: 8966,
            image_background:
              "https://media.rawg.io/media/games/737/737ea5662211d2e0bbd6f5989189e4f1.jpg",
          },
        },
        {
          id: 296863,
          store: {
            id: 2,
            name: "Xbox Store",
            slug: "xbox-store",
            domain: "microsoft.com",
            games_count: 4813,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
        },
        {
          id: 296864,
          store: {
            id: 3,
            name: "PlayStation Store",
            slug: "playstation-store",
            domain: "store.playstation.com",
            games_count: 7900,
            image_background:
              "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 40847,
          name: "Steam Achievements",
          slug: "steam-achievements",
          language: "eng",
          games_count: 35702,
          image_background:
            "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
        },
        {
          id: 40836,
          name: "Full controller support",
          slug: "full-controller-support",
          language: "eng",
          games_count: 16746,
          image_background:
            "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
        },
        {
          id: 40849,
          name: "Steam Cloud",
          slug: "steam-cloud",
          language: "eng",
          games_count: 16769,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 7808,
          name: "steam-trading-cards",
          slug: "steam-trading-cards",
          language: "eng",
          games_count: 7569,
          image_background:
            "https://media.rawg.io/media/games/6fc/6fcf4cd3b17c288821388e6085bb0fc9.jpg",
        },
        {
          id: 42,
          name: "Great Soundtrack",
          slug: "great-soundtrack",
          language: "eng",
          games_count: 3386,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 118,
          name: "Story Rich",
          slug: "story-rich",
          language: "eng",
          games_count: 20561,
          image_background:
            "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
        },
        {
          id: 36,
          name: "Open World",
          slug: "open-world",
          language: "eng",
          games_count: 7102,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        {
          id: 45,
          name: "2D",
          slug: "2d",
          language: "eng",
          games_count: 193065,
          image_background:
            "https://media.rawg.io/media/games/4cb/4cb855e8ef1578415a928e53c9f51867.png",
        },
        {
          id: 49,
          name: "Difficult",
          slug: "difficult",
          language: "eng",
          games_count: 13062,
          image_background:
            "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
        },
        {
          id: 6,
          name: "Exploration",
          slug: "exploration",
          language: "eng",
          games_count: 21834,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
        {
          id: 115,
          name: "Controller",
          slug: "controller",
          language: "eng",
          games_count: 10862,
          image_background:
            "https://media.rawg.io/media/games/295/295eb868c241e6ad32ac033b8e6a2ede.jpg",
        },
        {
          id: 336,
          name: "controller support",
          slug: "controller-support",
          language: "eng",
          games_count: 293,
          image_background:
            "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
        },
        {
          id: 88,
          name: "Cute",
          slug: "cute",
          language: "eng",
          games_count: 32125,
          image_background:
            "https://media.rawg.io/media/games/e44/e445335e611b4ccf03af71fffcbd30a4.jpg",
        },
        {
          id: 113,
          name: "Side Scroller",
          slug: "side-scroller",
          language: "eng",
          games_count: 10197,
          image_background:
            "https://media.rawg.io/media/screenshots/3f1/3f1c417b405a86ed7d92b903e0fcfd0c.jpg",
        },
        {
          id: 40,
          name: "Dark Fantasy",
          slug: "dark-fantasy",
          language: "eng",
          games_count: 3876,
          image_background:
            "https://media.rawg.io/media/games/29c/29c6c21cc0c78cff6f45d23631cc82f4.jpg",
        },
        {
          id: 218,
          name: "Multiple Endings",
          slug: "multiple-endings",
          language: "eng",
          games_count: 8219,
          image_background:
            "https://media.rawg.io/media/games/951/951572a3dd1e42544bd39a5d5b42d234.jpg",
        },
        {
          id: 259,
          name: "Metroidvania",
          slug: "metroidvania",
          language: "eng",
          games_count: 4229,
          image_background:
            "https://media.rawg.io/media/games/fc8/fc838d98c9b944e6a15176eabf40bee8.jpg",
        },
        {
          id: 258,
          name: "Hand-drawn",
          slug: "hand-drawn",
          language: "eng",
          games_count: 6601,
          image_background:
            "https://media.rawg.io/media/games/3ef/3eff92562640e452d3487c04ba6d7fae.jpg",
        },
        {
          id: 204,
          name: "Gothic",
          slug: "gothic",
          language: "eng",
          games_count: 856,
          image_background:
            "https://media.rawg.io/media/games/fd9/fd92f105dcd6491bc5d61135033d1f19.jpg",
        },
      ],
      esrb_rating: { id: 2, name: "Everyone 10+", slug: "everyone-10-plus" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
        },
        {
          id: 72619,
          image:
            "https://media.rawg.io/media/screenshots/6b3/6b309936c1fe07e9b7fa5e62a372790d.jpg",
        },
        {
          id: 72620,
          image:
            "https://media.rawg.io/media/screenshots/1a7/1a7a69db58c19d323f1dfbcc340d3f1e.jpg",
        },
        {
          id: 72621,
          image:
            "https://media.rawg.io/media/screenshots/723/7237d0c546b0d17a6a226f38823081d4.jpg",
        },
        {
          id: 72622,
          image:
            "https://media.rawg.io/media/screenshots/331/331095489397e7387681d921e8e472d4.jpg",
        },
        {
          id: 72623,
          image:
            "https://media.rawg.io/media/screenshots/5db/5db89e896496352c8f0a0a0bd545bd6d.jpg",
        },
        {
          id: 72624,
          image:
            "https://media.rawg.io/media/screenshots/843/843e06c9c5b26c309b47bd7075320dae.jpg",
        },
      ],
    },
    {
      id: 41,
      slug: "little-nightmares",
      name: "Little Nightmares",
      released: "2017-04-27",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      rating: 4.04,
      rating_top: 4,
      ratings: [
        { id: 4, title: "recommended", count: 835, percent: 54.5 },
        { id: 5, title: "exceptional", count: 465, percent: 30.35 },
        { id: 3, title: "meh", count: 149, percent: 9.73 },
        { id: 1, title: "skip", count: 83, percent: 5.42 },
      ],
      ratings_count: 1502,
      reviews_text_count: 24,
      added: 10460,
      added_by_status: {
        yet: 750,
        owned: 7604,
        beaten: 1330,
        toplay: 367,
        dropped: 285,
        playing: 124,
      },
      metacritic: 81,
      playtime: 3,
      suggestions_count: 329,
      updated: "2024-03-14T15:13:47",
      user_game: null,
      reviews_count: 1532,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 18,
            name: "PlayStation 4",
            slug: "playstation4",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 6767,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
          released_at: "2017-04-27",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2017-04-27",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong>  Windows 7, 64-bit<br></li><li><strong>Processor:</strong>  Intel CPU Core i3<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> Nvidia GTX 460<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Network:</strong> Broadband Internet connection<br></li><li><strong>Storage:</strong> 10 GB available space<br></li><li><strong>Additional Notes:</strong> SSE4.2 required</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong>  Windows 7, 64-bit<br></li><li><strong>Processor:</strong>  Intel CPU Core i7<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong>  Nvidia GPU GeForce GTX 660<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Network:</strong> Broadband Internet connection<br></li><li><strong>Storage:</strong> 10 GB available space<br></li><li><strong>Additional Notes:</strong> SSE4.2 required</li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 1,
            name: "Xbox One",
            slug: "xbox-one",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5590,
            image_background:
              "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
          },
          released_at: "2017-04-27",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 7,
            name: "Nintendo Switch",
            slug: "nintendo-switch",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5406,
            image_background:
              "https://media.rawg.io/media/games/5eb/5eb49eb2fa0738fdb5bacea557b1bc57.jpg",
          },
          released_at: "2017-04-27",
          requirements_en: null,
          requirements_ru: null,
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
        { platform: { id: 3, name: "Xbox", slug: "xbox" } },
        { platform: { id: 7, name: "Nintendo", slug: "nintendo" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
        {
          id: 83,
          name: "Platformer",
          slug: "platformer",
          games_count: 100773,
          image_background:
            "https://media.rawg.io/media/games/fc8/fc838d98c9b944e6a15176eabf40bee8.jpg",
        },
      ],
      stores: [
        {
          id: 41,
          store: {
            id: 3,
            name: "PlayStation Store",
            slug: "playstation-store",
            domain: "store.playstation.com",
            games_count: 7900,
            image_background:
              "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
          },
        },
        {
          id: 11257,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
        {
          id: 26963,
          store: {
            id: 5,
            name: "GOG",
            slug: "gog",
            domain: "gog.com",
            games_count: 5817,
            image_background:
              "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
          },
        },
        {
          id: 25310,
          store: {
            id: 2,
            name: "Xbox Store",
            slug: "xbox-store",
            domain: "microsoft.com",
            games_count: 4813,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 40847,
          name: "Steam Achievements",
          slug: "steam-achievements",
          language: "eng",
          games_count: 35702,
          image_background:
            "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
        },
        {
          id: 40836,
          name: "Full controller support",
          slug: "full-controller-support",
          language: "eng",
          games_count: 16746,
          image_background:
            "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
        },
        {
          id: 40849,
          name: "Steam Cloud",
          slug: "steam-cloud",
          language: "eng",
          games_count: 16769,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 7808,
          name: "steam-trading-cards",
          slug: "steam-trading-cards",
          language: "eng",
          games_count: 7569,
          image_background:
            "https://media.rawg.io/media/games/6fc/6fcf4cd3b17c288821388e6085bb0fc9.jpg",
        },
        {
          id: 42,
          name: "Great Soundtrack",
          slug: "great-soundtrack",
          language: "eng",
          games_count: 3386,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 118,
          name: "Story Rich",
          slug: "story-rich",
          language: "eng",
          games_count: 20561,
          image_background:
            "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
        },
        {
          id: 149,
          name: "Third Person",
          slug: "third-person",
          language: "eng",
          games_count: 10861,
          image_background:
            "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
        },
        {
          id: 16,
          name: "Horror",
          slug: "horror",
          language: "eng",
          games_count: 43534,
          image_background:
            "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
        },
        {
          id: 189,
          name: "Female Protagonist",
          slug: "female-protagonist",
          language: "eng",
          games_count: 11837,
          image_background:
            "https://media.rawg.io/media/games/530/5302dd22a190e664531236ca724e8726.jpg",
        },
        {
          id: 15,
          name: "Stealth",
          slug: "stealth",
          language: "eng",
          games_count: 6100,
          image_background:
            "https://media.rawg.io/media/games/b7d/b7d3f1715fa8381a4e780173a197a615.jpg",
        },
        {
          id: 41,
          name: "Dark",
          slug: "dark",
          language: "eng",
          games_count: 15192,
          image_background:
            "https://media.rawg.io/media/games/490/49016e06ae2103881ff6373248843069.jpg",
        },
        {
          id: 336,
          name: "controller support",
          slug: "controller-support",
          language: "eng",
          games_count: 293,
          image_background:
            "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
        },
        {
          id: 17,
          name: "Survival Horror",
          slug: "survival-horror",
          language: "eng",
          games_count: 8168,
          image_background:
            "https://media.rawg.io/media/games/ebd/ebdbb7eb52bd58b0e7fa4538d9757b60.jpg",
        },
        {
          id: 111,
          name: "Short",
          slug: "short",
          language: "eng",
          games_count: 57680,
          image_background:
            "https://media.rawg.io/media/games/54a/54a3e4c617217848dc43c4de9989fe37.jpg",
        },
        {
          id: 334,
          name: "achievements",
          slug: "achievements",
          language: "eng",
          games_count: 4910,
          image_background:
            "https://media.rawg.io/media/games/8df/8df64136042eb6e2ed4fa910a6ad96ac.jpg",
        },
        {
          id: 83,
          name: "Puzzle-Platformer",
          slug: "puzzle-platformer",
          language: "eng",
          games_count: 9498,
          image_background:
            "https://media.rawg.io/media/games/a5a/a5abaa1b5cc1567b026fa3aa9fbd828e.jpg",
        },
        {
          id: 116,
          name: "2.5D",
          slug: "25d",
          language: "eng",
          games_count: 1748,
          image_background:
            "https://media.rawg.io/media/screenshots/fe0/fe0ccce931cdb5aaff9500a331e5c2eb.jpg",
        },
      ],
      esrb_rating: { id: 3, name: "Teen", slug: "teen" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 258,
          image:
            "https://media.rawg.io/media/screenshots/00a/00a53be1dcfba6c63ac6807637c4a45a.jpg",
        },
        {
          id: 259,
          image:
            "https://media.rawg.io/media/screenshots/78a/78aa666996cf9d2708f529c18784262f.jpg",
        },
        {
          id: 260,
          image:
            "https://media.rawg.io/media/screenshots/96d/96d356977066c0c2159ed7ef3f470cab.jpg",
        },
        {
          id: 261,
          image:
            "https://media.rawg.io/media/screenshots/3dc/3dc7c607ff7873eaa84a8d69eac26335.jpg",
        },
        {
          id: 262,
          image:
            "https://media.rawg.io/media/screenshots/a83/a83cce4aae86d8ed0b7c8d933231b9bf.jpg",
        },
        {
          id: 263,
          image:
            "https://media.rawg.io/media/screenshots/0c6/0c6642fb4a74893a4c950528791fe7dd.jpg",
        },
      ],
    },
    {
      id: 10035,
      slug: "hitman",
      name: "Hitman",
      released: "2016-03-11",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/16b/16b1b7b36e2042d1128d5a3e852b3b2f.jpg",
      rating: 3.92,
      rating_top: 4,
      ratings: [
        { id: 4, title: "recommended", count: 1107, percent: 56.02 },
        { id: 5, title: "exceptional", count: 462, percent: 23.38 },
        { id: 3, title: "meh", count: 298, percent: 15.08 },
        { id: 1, title: "skip", count: 109, percent: 5.52 },
      ],
      ratings_count: 1965,
      reviews_text_count: 3,
      added: 10276,
      added_by_status: {
        yet: 770,
        owned: 7177,
        beaten: 1166,
        toplay: 351,
        dropped: 681,
        playing: 131,
      },
      metacritic: 83,
      playtime: 7,
      suggestions_count: 572,
      updated: "2024-03-13T20:20:40",
      user_game: null,
      reviews_count: 1976,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 18,
            name: "PlayStation 4",
            slug: "playstation4",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 6767,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
          released_at: "2016-03-11",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 6,
            name: "Linux",
            slug: "linux",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 76454,
            image_background:
              "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
          },
          released_at: "2016-03-11",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 16.04 or Steam OS 2.0<br></li><li><strong>Processor:</strong> Intel Core i5-2500K/AMD FX-8350<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> Nvidia GeForce GTX 680/AMD R9 270X graphics card or better*<br></li><li><strong>Storage:</strong> 67 GB available space<br></li><li><strong>Additional Notes:</strong> *NVIDIA graphics cards require driver version 375.26. AMD graphics cards require MESA 13.0.3 or better. Intel graphics cards will not be supported on release.</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 16.10 or Steam OS 2.0<br></li><li><strong>Processor:</strong> Intel Core i7 3770<br></li><li><strong>Memory:</strong> 16 GB RAM<br></li><li><strong>Graphics:</strong> Nvidia GeForce GTX 970/AMD R9 290 graphics card or better*<br></li><li><strong>Storage:</strong> 67 GB available space<br></li><li><strong>Additional Notes:</strong> *NVIDIA graphics cards require driver version 375.26. AMD graphics cards require MESA 13.0.3 or better. Intel graphics cards will not be supported on release.</li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 5,
            name: "macOS",
            slug: "macos",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 103400,
            image_background:
              "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
          },
          released_at: "2016-03-11",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> macOS 10.12.5<br></li><li><strong>Processor:</strong> 2.0Ghz Intel Core i5<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> 2GB AMD M290 or better (see notes for more details)<br></li><li><strong>Storage:</strong> 68 GB available space<br></li><li><strong>Additional Notes:</strong> <strong>The game is supported on the following Macs. To check your Mac model and when it was released, select About This Mac from the Apple menu on your menu bar.</strong>   <ul class="bb_ul">  <li>*    All 27&quot; iMacs released since Late 2014 </li><li>*    All 15&quot; MacBook Pros released since 2016  </li><li>*    All Mac Pros released since Late 2013 </li></ul>    ——— <strong>The following Macs are capable of running the game but do not consistently meet the standards required for official support.</strong> <ul class="bb_ul">  <li>*    All 15&quot; MacBook Pros with an AMD graphics card released in 2015 </li></ul> ——— <ul class="bb_ul"><strong>Please note: HITMAN for macOS will launch with support for AMD graphics cards only. We hope to announce more details regarding support for Nvidia and Intel graphics cards shortly.</strong></ul></li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2016-03-11",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> OS 64-bit Windows 7<br></li><li><strong>Processor:</strong> Intel CPU Core i5-2500K 3.3GHz / AMD CPU Phenom II X4 940<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> NVIDIA GeForce GTX 660 / Radeon HD 7870<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Storage:</strong> 50 GB available space</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> OS 64-bit Windows 7 / 64-bit Windows 8 (8.1) or Windows 10<br></li><li><strong>Processor:</strong> Intel CPU Core i7 3770 3,4 GHz / AMD CPU AMD FX-8350 4 GHz<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> Nvidia GPU GeForce GTX 770 / AMD GPU Radeon R9 290<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Storage:</strong> 50 GB available space</li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 1,
            name: "Xbox One",
            slug: "xbox-one",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5590,
            image_background:
              "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
          },
          released_at: "2016-03-11",
          requirements_en: null,
          requirements_ru: null,
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
        { platform: { id: 3, name: "Xbox", slug: "xbox" } },
        { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
        { platform: { id: 6, name: "Linux", slug: "linux" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
        {
          id: 2,
          name: "Shooter",
          slug: "shooter",
          games_count: 59459,
          image_background:
            "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
        },
        {
          id: 14,
          name: "Simulation",
          slug: "simulation",
          games_count: 68547,
          image_background:
            "https://media.rawg.io/media/games/e40/e40cc9d1957b0a0ed7e389834457b524.jpg",
        },
      ],
      stores: [
        {
          id: 38172,
          store: {
            id: 3,
            name: "PlayStation Store",
            slug: "playstation-store",
            domain: "store.playstation.com",
            games_count: 7900,
            image_background:
              "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
          },
        },
        {
          id: 10947,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
        {
          id: 35171,
          store: {
            id: 2,
            name: "Xbox Store",
            slug: "xbox-store",
            domain: "microsoft.com",
            games_count: 4813,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
        },
        {
          id: 593095,
          store: {
            id: 11,
            name: "Epic Games",
            slug: "epic-games",
            domain: "epicgames.com",
            games_count: 1319,
            image_background:
              "https://media.rawg.io/media/games/4be/4be6a6ad0364751a96229c56bf69be59.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 40836,
          name: "Full controller support",
          slug: "full-controller-support",
          language: "eng",
          games_count: 16746,
          image_background:
            "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 42,
          name: "Great Soundtrack",
          slug: "great-soundtrack",
          language: "eng",
          games_count: 3386,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 118,
          name: "Story Rich",
          slug: "story-rich",
          language: "eng",
          games_count: 20561,
          image_background:
            "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
        },
        {
          id: 36,
          name: "Open World",
          slug: "open-world",
          language: "eng",
          games_count: 7102,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        {
          id: 149,
          name: "Third Person",
          slug: "third-person",
          language: "eng",
          games_count: 10861,
          image_background:
            "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
        },
        {
          id: 37,
          name: "Sandbox",
          slug: "sandbox",
          language: "eng",
          games_count: 6607,
          image_background:
            "https://media.rawg.io/media/games/7cf/7cfc9220b401b7a300e409e539c9afd5.jpg",
        },
        {
          id: 15,
          name: "Stealth",
          slug: "stealth",
          language: "eng",
          games_count: 6100,
          image_background:
            "https://media.rawg.io/media/games/b7d/b7d3f1715fa8381a4e780173a197a615.jpg",
        },
        {
          id: 80,
          name: "Tactical",
          slug: "tactical",
          language: "eng",
          games_count: 4789,
          image_background:
            "https://media.rawg.io/media/games/997/997ab4d67e96fb20a4092383477d4463.jpg",
        },
        {
          id: 150,
          name: "Third-Person Shooter",
          slug: "third-person-shooter",
          language: "eng",
          games_count: 3266,
          image_background:
            "https://media.rawg.io/media/games/ebd/ebdbb7eb52bd58b0e7fa4538d9757b60.jpg",
        },
        {
          id: 44,
          name: "Nudity",
          slug: "nudity",
          language: "eng",
          games_count: 6072,
          image_background:
            "https://media.rawg.io/media/games/260/26023c855f1769a93411d6a7ea084632.jpeg",
        },
        {
          id: 144,
          name: "Crime",
          slug: "crime",
          language: "eng",
          games_count: 2718,
          image_background:
            "https://media.rawg.io/media/games/5fa/5fae5fec3c943179e09da67a4427d68f.jpg",
        },
        {
          id: 278,
          name: "Assassin",
          slug: "assassin",
          language: "eng",
          games_count: 807,
          image_background:
            "https://media.rawg.io/media/games/858/858c016de0cf7bc21a57dcc698a04a0c.jpg",
        },
        {
          id: 232,
          name: "Episodic",
          slug: "episodic",
          language: "eng",
          games_count: 459,
          image_background:
            "https://media.rawg.io/media/games/a28/a289e23b4d4d84f26ab59125e3be4483.jpg",
        },
        {
          id: 178,
          name: "Illuminati",
          slug: "illuminati",
          language: "eng",
          games_count: 315,
          image_background:
            "https://media.rawg.io/media/games/46b/46b62c37c8faf3268a0768b548e6cd9d.jpg",
        },
        {
          id: 216,
          name: "Heist",
          slug: "heist",
          language: "eng",
          games_count: 502,
          image_background:
            "https://media.rawg.io/media/games/7e0/7e09315d72e12b6cd8995ba18b5f70d0.jpg",
        },
      ],
      esrb_rating: { id: 5, name: "Adults Only", slug: "adults-only" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/16b/16b1b7b36e2042d1128d5a3e852b3b2f.jpg",
        },
        {
          id: 76082,
          image:
            "https://media.rawg.io/media/screenshots/ef6/ef6c0a92d08a99d2e405cac53c597d10.jpg",
        },
        {
          id: 76083,
          image:
            "https://media.rawg.io/media/screenshots/fb3/fb3c23014fcb24a28fb94af0a009906c.jpg",
        },
        {
          id: 76084,
          image:
            "https://media.rawg.io/media/screenshots/202/202293192d1b6245c8a1e252d9df604c.jpg",
        },
        {
          id: 76085,
          image:
            "https://media.rawg.io/media/screenshots/278/278bd63ee564982e816ec7fe802df420.jpg",
        },
        {
          id: 76086,
          image:
            "https://media.rawg.io/media/screenshots/002/0021b0e5db6959989d90979abba68c8d.jpg",
        },
        {
          id: 76087,
          image:
            "https://media.rawg.io/media/screenshots/c8a/c8a537532ab8ce8750293a1fc41ee7f8.jpg",
        },
      ],
    },
    {
      id: 2551,
      slug: "dark-souls-iii",
      name: "Dark Souls III",
      released: "2016-04-11",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/da1/da1b267764d77221f07a4386b6548e5a.jpg",
      rating: 4.4,
      rating_top: 5,
      ratings: [
        { id: 5, title: "exceptional", count: 1600, percent: 61.4 },
        { id: 4, title: "recommended", count: 698, percent: 26.78 },
        { id: 3, title: "meh", count: 185, percent: 7.1 },
        { id: 1, title: "skip", count: 123, percent: 4.72 },
      ],
      ratings_count: 2581,
      reviews_text_count: 17,
      added: 10229,
      added_by_status: {
        yet: 630,
        owned: 6201,
        beaten: 1960,
        toplay: 536,
        dropped: 633,
        playing: 269,
      },
      metacritic: 89,
      playtime: 29,
      suggestions_count: 708,
      updated: "2024-03-20T13:47:45",
      user_game: null,
      reviews_count: 2606,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 1,
            name: "Xbox One",
            slug: "xbox-one",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5590,
            image_background:
              "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
          },
          released_at: "2016-04-11",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 18,
            name: "PlayStation 4",
            slug: "playstation4",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 6767,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
          released_at: "2016-04-11",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2016-04-11",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows 7 SP1 64bit, Windows 8.1 64bit Windows 10 64bit<br></li><li><strong>Processor:</strong> Intel Core i3-2100 / AMD® FX-6300<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> NVIDIA® GeForce GTX 750 Ti / ATI Radeon HD 7950<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Network:</strong> Broadband Internet connection<br></li><li><strong>Storage:</strong> 25 GB available space<br></li><li><strong>Sound Card:</strong> DirectX 11 sound device<br></li><li><strong>Additional Notes:</strong> Internet connection required for online play and product activation</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows 7 SP1 64bit, Windows 8.1 64bit Windows 10 64bit<br></li><li><strong>Processor:</strong> Intel Core i7-3770 / AMD® FX-8350<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> NVIDIA® GeForce GTX 970 / ATI Radeon R9 series<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Network:</strong> Broadband Internet connection<br></li><li><strong>Storage:</strong> 25 GB available space<br></li><li><strong>Sound Card:</strong> DirectX 11 sound device<br></li><li><strong>Additional Notes:</strong> Internet connection required for online play and product activation</li></ul>',
          },
          requirements_ru: null,
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
        { platform: { id: 3, name: "Xbox", slug: "xbox" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
        {
          id: 5,
          name: "RPG",
          slug: "role-playing-games-rpg",
          games_count: 55107,
          image_background:
            "https://media.rawg.io/media/games/618/618c2031a07bbff6b4f611f10b6bcdbc.jpg",
        },
      ],
      stores: [
        {
          id: 2679,
          store: {
            id: 3,
            name: "PlayStation Store",
            slug: "playstation-store",
            domain: "store.playstation.com",
            games_count: 7900,
            image_background:
              "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
          },
        },
        {
          id: 9067,
          store: {
            id: 2,
            name: "Xbox Store",
            slug: "xbox-store",
            domain: "microsoft.com",
            games_count: 4813,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
        },
        {
          id: 11140,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 40847,
          name: "Steam Achievements",
          slug: "steam-achievements",
          language: "eng",
          games_count: 35702,
          image_background:
            "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
        },
        {
          id: 7,
          name: "Multiplayer",
          slug: "multiplayer",
          language: "eng",
          games_count: 37130,
          image_background:
            "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
        },
        {
          id: 40836,
          name: "Full controller support",
          slug: "full-controller-support",
          language: "eng",
          games_count: 16746,
          image_background:
            "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 7808,
          name: "steam-trading-cards",
          slug: "steam-trading-cards",
          language: "eng",
          games_count: 7569,
          image_background:
            "https://media.rawg.io/media/games/6fc/6fcf4cd3b17c288821388e6085bb0fc9.jpg",
        },
        {
          id: 42,
          name: "Great Soundtrack",
          slug: "great-soundtrack",
          language: "eng",
          games_count: 3386,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 24,
          name: "RPG",
          slug: "rpg",
          language: "eng",
          games_count: 19865,
          image_background:
            "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
        },
        {
          id: 18,
          name: "Co-op",
          slug: "co-op",
          language: "eng",
          games_count: 11072,
          image_background:
            "https://media.rawg.io/media/games/511/5118aff5091cb3efec399c808f8c598f.jpg",
        },
        {
          id: 118,
          name: "Story Rich",
          slug: "story-rich",
          language: "eng",
          games_count: 20561,
          image_background:
            "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
        },
        {
          id: 36,
          name: "Open World",
          slug: "open-world",
          language: "eng",
          games_count: 7102,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        {
          id: 411,
          name: "cooperative",
          slug: "cooperative",
          language: "eng",
          games_count: 4703,
          image_background:
            "https://media.rawg.io/media/games/73e/73eecb8909e0c39fb246f457b5d6cbbe.jpg",
        },
        {
          id: 149,
          name: "Third Person",
          slug: "third-person",
          language: "eng",
          games_count: 10861,
          image_background:
            "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
        },
        {
          id: 16,
          name: "Horror",
          slug: "horror",
          language: "eng",
          games_count: 43534,
          image_background:
            "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
        },
        {
          id: 49,
          name: "Difficult",
          slug: "difficult",
          language: "eng",
          games_count: 13062,
          image_background:
            "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
        },
        {
          id: 6,
          name: "Exploration",
          slug: "exploration",
          language: "eng",
          games_count: 21834,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
        {
          id: 97,
          name: "Action RPG",
          slug: "action-rpg",
          language: "eng",
          games_count: 6501,
          image_background:
            "https://media.rawg.io/media/games/157/15742f2f67eacff546738e1ab5c19d20.jpg",
        },
        {
          id: 157,
          name: "PvP",
          slug: "pvp",
          language: "eng",
          games_count: 8687,
          image_background:
            "https://media.rawg.io/media/games/737/737ea5662211d2e0bbd6f5989189e4f1.jpg",
        },
        {
          id: 121,
          name: "Character Customization",
          slug: "character-customization",
          language: "eng",
          games_count: 4284,
          image_background:
            "https://media.rawg.io/media/games/33b/33b825c76382931df0fd8ecddf5caebe.jpg",
        },
        {
          id: 5,
          name: "Replay Value",
          slug: "replay-value",
          language: "eng",
          games_count: 1548,
          image_background:
            "https://media.rawg.io/media/games/1f4/1f47a270b8f241e4676b14d39ec620f7.jpg",
        },
        {
          id: 40,
          name: "Dark Fantasy",
          slug: "dark-fantasy",
          language: "eng",
          games_count: 3876,
          image_background:
            "https://media.rawg.io/media/games/29c/29c6c21cc0c78cff6f45d23631cc82f4.jpg",
        },
        {
          id: 205,
          name: "Lore-Rich",
          slug: "lore-rich",
          language: "eng",
          games_count: 1145,
          image_background:
            "https://media.rawg.io/media/games/c24/c24f4434882ae9c2c8d9d38de82cb7a5.jpg",
        },
      ],
      esrb_rating: { id: 4, name: "Mature", slug: "mature" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/da1/da1b267764d77221f07a4386b6548e5a.jpg",
        },
        {
          id: 805127,
          image:
            "https://media.rawg.io/media/screenshots/d7c/d7c05cdfb30ec07147bcd0d3985ec54c.jpg",
        },
        {
          id: 805128,
          image:
            "https://media.rawg.io/media/screenshots/479/479cb74c874748ca70a3a14e79a0c232.jpg",
        },
        {
          id: 805129,
          image:
            "https://media.rawg.io/media/screenshots/070/0703edc6d3db345a3acf19b4e6e43ebd.jpg",
        },
        {
          id: 805130,
          image:
            "https://media.rawg.io/media/screenshots/e4f/e4feaf5b078949102e72780091eb12af.jpg",
        },
        {
          id: 805131,
          image:
            "https://media.rawg.io/media/screenshots/028/02863a140eea53c1f51fd790aca753aa.jpg",
        },
        {
          id: 805132,
          image:
            "https://media.rawg.io/media/screenshots/8d2/8d2c0bb90867031f9265ef62c01445c5.jpg",
        },
      ],
    },
    {
      id: 4252,
      slug: "mirrors-edge",
      name: "Mirror's Edge",
      released: "2008-11-11",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/8e4/8e4de3f54ac659e08a7ba6a2b731682a.jpg",
      rating: 4.07,
      rating_top: 4,
      ratings: [
        { id: 4, title: "recommended", count: 1221, percent: 53.23 },
        { id: 5, title: "exceptional", count: 689, percent: 30.03 },
        { id: 3, title: "meh", count: 312, percent: 13.6 },
        { id: 1, title: "skip", count: 72, percent: 3.14 },
      ],
      ratings_count: 2273,
      reviews_text_count: 12,
      added: 10202,
      added_by_status: {
        yet: 333,
        owned: 6549,
        beaten: 2336,
        toplay: 233,
        dropped: 710,
        playing: 41,
      },
      metacritic: 81,
      playtime: 3,
      suggestions_count: 343,
      updated: "2024-03-14T09:50:08",
      user_game: null,
      reviews_count: 2294,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 14,
            name: "Xbox 360",
            slug: "xbox360",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 2796,
            image_background:
              "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
          },
          released_at: "2008-11-11",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 16,
            name: "PlayStation 3",
            slug: "playstation3",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 3168,
            image_background:
              "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
          },
          released_at: "2008-11-11",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2008-11-11",
          requirements_en: {
            minimum:
              '<ul class="bb_ul"><li><strong>Supported OS:</strong> Microsoft Windows® XP SP2 or Vista                    <br>\t\t\t\t\t</li><li><strong>Processor:</strong> 3.0 GHz or faster                    <br>\t\t\t\t\t</li><li><strong>Memory:</strong> 1 GB RAM or more                    <br>\t\t\t\t\t</li><li><strong>Graphics:</strong> DirectX® 9.0c  compatible video card, Shader Model 3.0 required. Video card must have 256 MB, NVIDIA GeForce 6800 or better                    <br>\t\t\t\t\t</li><li><strong>Hard Drive:</strong> 8 GB free space <br>\t\t\t\t\t</li><li><strong>Sound:</strong> DirectX® 9.0c compatible sound card </li></ul><ul class="bb_ul"><li>INTERNET CONNECTION, ONLINE AUTHENTICATION AND ACCEPTANCE OF END USER LICENSE AGREEMENT REQUIRED TO PLAY. TO ACCESS ONLINE FEATURES, YOU MUST REGISTER ONLINE. ONLY ONE REGISTRATION IS AVAILABLE PER GAME. EA ONLINE TERMS &amp; CONDITIONS AND FEATURE UPDATES CAN BE FOUND AT <a href="https://steamcommunity.com/linkfilter/?url=http://www.ea.com" target="_blank" rel="noopener"  >www.ea.com</a>. YOU MUST BE 13+ TO REGISTER WITH EA ONLINE.</li></ul>',
          },
          requirements_ru: {
            minimum:
              "Pentium 4/Athlon XP 3 ГГц,1 Гб памяти,3D-ускоритель с 256 Мб памяти,8 Гб на винчестере",
            recommended:
              "Core 2 Duo/Athlon X2 2.5 ГГц,2 Гб памяти,3D-ускоритель с 512 Мб памяти,8 Гб на винчестере,интернет-соединение",
          },
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
        { platform: { id: 3, name: "Xbox", slug: "xbox" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
      ],
      stores: [
        {
          id: 4580,
          store: {
            id: 3,
            name: "PlayStation Store",
            slug: "playstation-store",
            domain: "store.playstation.com",
            games_count: 7900,
            image_background:
              "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
          },
        },
        {
          id: 21595,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
        {
          id: 26791,
          store: {
            id: 5,
            name: "GOG",
            slug: "gog",
            domain: "gog.com",
            games_count: 5817,
            image_background:
              "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
          },
        },
        {
          id: 34058,
          store: {
            id: 7,
            name: "Xbox 360 Store",
            slug: "xbox360",
            domain: "marketplace.xbox.com",
            games_count: 1912,
            image_background:
              "https://media.rawg.io/media/games/d1a/d1a2e99ade53494c6330a0ed945fe823.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 40836,
          name: "Full controller support",
          slug: "full-controller-support",
          language: "eng",
          games_count: 16746,
          image_background:
            "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 42,
          name: "Great Soundtrack",
          slug: "great-soundtrack",
          language: "eng",
          games_count: 3386,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 8,
          name: "First-Person",
          slug: "first-person",
          language: "eng",
          games_count: 30626,
          image_background:
            "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
        },
        {
          id: 32,
          name: "Sci-fi",
          slug: "sci-fi",
          language: "eng",
          games_count: 18535,
          image_background:
            "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
        },
        {
          id: 40845,
          name: "Partial Controller Support",
          slug: "partial-controller-support",
          language: "eng",
          games_count: 11229,
          image_background:
            "https://media.rawg.io/media/games/62c/62c7c8b28a27b83680b22fb9d33fc619.jpg",
        },
        {
          id: 193,
          name: "Classic",
          slug: "classic",
          language: "eng",
          games_count: 1783,
          image_background:
            "https://media.rawg.io/media/games/960/960b601d9541cec776c5fa42a00bf6c4.jpg",
        },
        {
          id: 189,
          name: "Female Protagonist",
          slug: "female-protagonist",
          language: "eng",
          games_count: 11837,
          image_background:
            "https://media.rawg.io/media/games/530/5302dd22a190e664531236ca724e8726.jpg",
        },
        {
          id: 111,
          name: "Short",
          slug: "short",
          language: "eng",
          games_count: 57680,
          image_background:
            "https://media.rawg.io/media/games/54a/54a3e4c617217848dc43c4de9989fe37.jpg",
        },
        {
          id: 165,
          name: "Colorful",
          slug: "colorful",
          language: "eng",
          games_count: 20827,
          image_background:
            "https://media.rawg.io/media/games/3e3/3e355e1b8a5ee47f4c76e28e3055236d.jpg",
        },
        {
          id: 167,
          name: "Futuristic",
          slug: "futuristic",
          language: "eng",
          games_count: 5050,
          image_background:
            "https://media.rawg.io/media/games/90f/90fd5e569bc4c4a666c588a732124908.jpg",
        },
        {
          id: 188,
          name: "Parkour",
          slug: "parkour",
          language: "eng",
          games_count: 3425,
          image_background:
            "https://media.rawg.io/media/games/c2e/c2eb6021a2596644b437e943612af25c.jpg",
        },
        {
          id: 119,
          name: "Dystopian",
          slug: "dystopian",
          language: "eng",
          games_count: 2001,
          image_background:
            "https://media.rawg.io/media/games/a0e/a0ef08621301a1eab5e04fa5c96978fa.jpeg",
        },
        {
          id: 166,
          name: "Stylized",
          slug: "stylized",
          language: "eng",
          games_count: 5738,
          image_background:
            "https://media.rawg.io/media/screenshots/f7f/f7fb42b2115c3730c00248dafe57e2d0.jpg",
        },
        {
          id: 280,
          name: "Time Attack",
          slug: "time-attack",
          language: "eng",
          games_count: 1670,
          image_background:
            "https://media.rawg.io/media/screenshots/3ac/3ace8d21a5d653c775844d591f3f14ef.jpg",
        },
        {
          id: 19728,
          name: "european",
          slug: "european",
          language: "eng",
          games_count: 12,
          image_background:
            "https://media.rawg.io/media/screenshots/d14/d148f8ffc332b1d0f70be9bad91c2171.jpg",
        },
      ],
      esrb_rating: { id: 3, name: "Teen", slug: "teen" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/8e4/8e4de3f54ac659e08a7ba6a2b731682a.jpg",
        },
        {
          id: 179965,
          image:
            "https://media.rawg.io/media/screenshots/38b/38bb5b035c811402248bbc19297d5183.jpg",
        },
        {
          id: 179966,
          image:
            "https://media.rawg.io/media/screenshots/4b6/4b6daf2d868abed65984f6ba308eb5f6.jpg",
        },
        {
          id: 179967,
          image:
            "https://media.rawg.io/media/screenshots/b07/b07d46f11697eb8ee5002d37d9ddc696.jpg",
        },
        {
          id: 179968,
          image:
            "https://media.rawg.io/media/screenshots/ddd/ddde2f1b028ded72097066b504db6f8b.jpg",
        },
        {
          id: 179969,
          image:
            "https://media.rawg.io/media/screenshots/a8b/a8b9fe7b7e04ff20405e40fb31cfe56e.jpg",
        },
        {
          id: 179970,
          image:
            "https://media.rawg.io/media/screenshots/741/741957fcdece6895a653bf51786838e9.jpg",
        },
      ],
    },
    {
      id: 3612,
      slug: "hotline-miami",
      name: "Hotline Miami",
      released: "2012-10-22",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/9fa/9fa63622543e5d4f6d99aa9d73b043de.jpg",
      rating: 4.38,
      rating_top: 5,
      ratings: [
        { id: 5, title: "exceptional", count: 1382, percent: 54.22 },
        { id: 4, title: "recommended", count: 911, percent: 35.74 },
        { id: 3, title: "meh", count: 174, percent: 6.83 },
        { id: 1, title: "skip", count: 82, percent: 3.22 },
      ],
      ratings_count: 2524,
      reviews_text_count: 12,
      added: 10128,
      added_by_status: {
        yet: 367,
        owned: 6672,
        beaten: 2196,
        toplay: 205,
        dropped: 607,
        playing: 81,
      },
      metacritic: 85,
      playtime: 5,
      suggestions_count: 294,
      updated: "2024-03-17T18:52:09",
      user_game: null,
      reviews_count: 2549,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 6,
            name: "Linux",
            slug: "linux",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 76454,
            image_background:
              "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
          },
          released_at: "2012-10-22",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 7,
            name: "Nintendo Switch",
            slug: "nintendo-switch",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5406,
            image_background:
              "https://media.rawg.io/media/games/5eb/5eb49eb2fa0738fdb5bacea557b1bc57.jpg",
          },
          released_at: "2012-10-22",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 1,
            name: "Xbox One",
            slug: "xbox-one",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5590,
            image_background:
              "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
          },
          released_at: "2012-10-22",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 18,
            name: "PlayStation 4",
            slug: "playstation4",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 6767,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
          released_at: "2012-10-22",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 19,
            name: "PS Vita",
            slug: "ps-vita",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 1444,
            image_background:
              "https://media.rawg.io/media/games/8d6/8d69eb6c32ed6acfd75f82d532144993.jpg",
          },
          released_at: "2012-10-22",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2012-10-22",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong>Microsoft® Windows® XP / Vista / 7<br>\t</li><li><strong>Processor:</strong>1.2GHz processor<br>\t</li><li><strong>Memory:</strong>512 MB RAM<br>\t</li><li><strong>Graphics:</strong>DirectX 8-compatible graphics card with at least 32MB of video memory<br>\t</li><li><strong>DirectX®:</strong>9.0c<br>\t</li><li><strong>Hard Drive:</strong>250 MB HD space<br>\t</li><li><strong>Additional:</strong>Microsoft Xbox 360 Controller or Direct Input compatible controller, OpenGL 2.x or better highly recommended.</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong>Microsoft® Windows® XP / Vista / 7<br>\t</li><li><strong>Processor:</strong>1.4GHz processor or faster<br>\t</li><li><strong>Memory:</strong>1 GB RAM<br>\t</li><li><strong>Graphics:</strong>DirectX 8-compatible graphics card with at least 32MB of video memory<br>\t</li><li><strong>DirectX®:</strong>9.0c<br>\t</li><li><strong>Hard Drive:</strong>250 MB HD space<br>\t</li><li><strong>Additional:</strong>Microsoft Xbox 360 Controller or Direct Input compatible controller</li></ul>',
          },
          requirements_ru: {
            minimum:
              "Pentium 3/Athlon 1.2 ГГц,512 Мб памяти,видеокарта с 32 Мб памяти,250 Мб на винчестере",
            recommended:
              "Pentium 4/Athlon XP 1.5 ГГц,1 Гб памяти,видеокарта с 32 Мб памяти,250 Мб на винчестере",
          },
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
        { platform: { id: 3, name: "Xbox", slug: "xbox" } },
        { platform: { id: 6, name: "Linux", slug: "linux" } },
        { platform: { id: 7, name: "Nintendo", slug: "nintendo" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
        {
          id: 2,
          name: "Shooter",
          slug: "shooter",
          games_count: 59459,
          image_background:
            "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
        },
        {
          id: 11,
          name: "Arcade",
          slug: "arcade",
          games_count: 22649,
          image_background:
            "https://media.rawg.io/media/games/7cf/7cf38d2d80267c121c6d0d361e9429ce.jpg",
        },
        {
          id: 51,
          name: "Indie",
          slug: "indie",
          games_count: 62257,
          image_background:
            "https://media.rawg.io/media/games/9dd/9ddabb34840ea9227556670606cf8ea3.jpg",
        },
      ],
      stores: [
        {
          id: 25947,
          store: {
            id: 5,
            name: "GOG",
            slug: "gog",
            domain: "gog.com",
            games_count: 5817,
            image_background:
              "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
          },
        },
        {
          id: 3905,
          store: {
            id: 3,
            name: "PlayStation Store",
            slug: "playstation-store",
            domain: "store.playstation.com",
            games_count: 7900,
            image_background:
              "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
          },
        },
        {
          id: 19491,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
        {
          id: 214847,
          store: {
            id: 8,
            name: "Google Play",
            slug: "google-play",
            domain: "play.google.com",
            games_count: 17079,
            image_background:
              "https://media.rawg.io/media/games/13a/13a528ac9cf48bbb6be5d35fe029336d.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 40847,
          name: "Steam Achievements",
          slug: "steam-achievements",
          language: "eng",
          games_count: 35702,
          image_background:
            "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 7808,
          name: "steam-trading-cards",
          slug: "steam-trading-cards",
          language: "eng",
          games_count: 7569,
          image_background:
            "https://media.rawg.io/media/games/6fc/6fcf4cd3b17c288821388e6085bb0fc9.jpg",
        },
        {
          id: 42,
          name: "Great Soundtrack",
          slug: "great-soundtrack",
          language: "eng",
          games_count: 3386,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 45,
          name: "2D",
          slug: "2d",
          language: "eng",
          games_count: 193065,
          image_background:
            "https://media.rawg.io/media/games/4cb/4cb855e8ef1578415a928e53c9f51867.png",
        },
        {
          id: 40845,
          name: "Partial Controller Support",
          slug: "partial-controller-support",
          language: "eng",
          games_count: 11229,
          image_background:
            "https://media.rawg.io/media/games/62c/62c7c8b28a27b83680b22fb9d33fc619.jpg",
        },
        {
          id: 26,
          name: "Gore",
          slug: "gore",
          language: "eng",
          games_count: 5515,
          image_background:
            "https://media.rawg.io/media/games/7f6/7f6cd70ba2ad57053b4847c13569f2d8.jpg",
        },
        {
          id: 49,
          name: "Difficult",
          slug: "difficult",
          language: "eng",
          games_count: 13062,
          image_background:
            "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
        },
        {
          id: 34,
          name: "Violent",
          slug: "violent",
          language: "eng",
          games_count: 6476,
          image_background:
            "https://media.rawg.io/media/games/511/5118aff5091cb3efec399c808f8c598f.jpg",
        },
        {
          id: 122,
          name: "Pixel Graphics",
          slug: "pixel-graphics",
          language: "eng",
          games_count: 92565,
          image_background:
            "https://media.rawg.io/media/games/f46/f466571d536f2e3ea9e815ad17177501.jpg",
        },
        {
          id: 74,
          name: "Retro",
          slug: "retro",
          language: "eng",
          games_count: 41033,
          image_background:
            "https://media.rawg.io/media/games/bbf/bbf8d74ab64440ad76294cff2f4d9cfa.jpg",
        },
        {
          id: 336,
          name: "controller support",
          slug: "controller-support",
          language: "eng",
          games_count: 293,
          image_background:
            "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
        },
        {
          id: 131,
          name: "Fast-Paced",
          slug: "fast-paced",
          language: "eng",
          games_count: 10466,
          image_background:
            "https://media.rawg.io/media/games/9fa/9fa63622543e5d4f6d99aa9d73b043de.jpg",
        },
        {
          id: 61,
          name: "Top-Down",
          slug: "top-down",
          language: "eng",
          games_count: 24503,
          image_background:
            "https://media.rawg.io/media/screenshots/2fc/2fc6994425146f9dba3133400b414e29.jpg",
        },
        {
          id: 337,
          name: "overlay",
          slug: "overlay",
          language: "eng",
          games_count: 217,
          image_background:
            "https://media.rawg.io/media/games/fad/fadc4be043ed07904012d47cd02671e4.jpg",
        },
        {
          id: 136,
          name: "Music",
          slug: "music",
          language: "eng",
          games_count: 25902,
          image_background:
            "https://media.rawg.io/media/games/f99/f9979698c43fd84c3ab69280576dd3af.jpg",
        },
        {
          id: 46,
          name: "Surreal",
          slug: "surreal",
          language: "eng",
          games_count: 5057,
          image_background:
            "https://media.rawg.io/media/screenshots/5a3/5a36930b16d9b1949d8c8c92b5a28d89.jpg",
        },
        {
          id: 236,
          name: "Top-Down Shooter",
          slug: "top-down-shooter",
          language: "eng",
          games_count: 2026,
          image_background:
            "https://media.rawg.io/media/screenshots/80a/80a0d07be6109fde6b92ac5283e7056c.jpg",
        },
        {
          id: 224,
          name: "1980s",
          slug: "1980s",
          language: "eng",
          games_count: 2265,
          image_background:
            "https://media.rawg.io/media/games/5e4/5e4bff02098b2b6f824c68286d5da1a6.jpg",
        },
        {
          id: 286,
          name: "Psychedelic",
          slug: "psychedelic",
          language: "eng",
          games_count: 2020,
          image_background:
            "https://media.rawg.io/media/games/9fa/9fa63622543e5d4f6d99aa9d73b043de.jpg",
        },
      ],
      esrb_rating: null,
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/9fa/9fa63622543e5d4f6d99aa9d73b043de.jpg",
        },
        {
          id: 32084,
          image:
            "https://media.rawg.io/media/screenshots/38b/38b9bb0de0a380434b78587e132b2e21.jpg",
        },
        {
          id: 32085,
          image:
            "https://media.rawg.io/media/screenshots/e7a/e7a11a669aa609b9b3b0c7aeab2dc804.jpg",
        },
        {
          id: 32086,
          image:
            "https://media.rawg.io/media/screenshots/88b/88b907144995e57c45ee043c59dd6810.jpg",
        },
        {
          id: 32087,
          image:
            "https://media.rawg.io/media/screenshots/53f/53fd44fd759bc571b1445898c50b418e.jpg",
        },
        {
          id: 32088,
          image:
            "https://media.rawg.io/media/screenshots/ddd/dddc7151559716c7c0dddb05874496da.jpg",
        },
        {
          id: 32089,
          image:
            "https://media.rawg.io/media/screenshots/b2a/b2a25c20510e3111a13d56a9751cadd7.jpg",
        },
      ],
    },
    {
      id: 18080,
      slug: "half-life",
      name: "Half-Life",
      released: "1998-11-08",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/6c5/6c55e22185876626881b76c11922b073.jpg",
      rating: 4.38,
      rating_top: 5,
      ratings: [
        { id: 5, title: "exceptional", count: 1268, percent: 55.66 },
        { id: 4, title: "recommended", count: 766, percent: 33.63 },
        { id: 3, title: "meh", count: 164, percent: 7.2 },
        { id: 1, title: "skip", count: 80, percent: 3.51 },
      ],
      ratings_count: 2259,
      reviews_text_count: 13,
      added: 10060,
      added_by_status: {
        yet: 396,
        owned: 6514,
        beaten: 2382,
        toplay: 168,
        dropped: 541,
        playing: 59,
      },
      metacritic: 96,
      playtime: 3,
      suggestions_count: 469,
      updated: "2024-03-17T16:17:16",
      user_game: null,
      reviews_count: 2278,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 106,
            name: "Dreamcast",
            slug: "dreamcast",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 362,
            image_background:
              "https://media.rawg.io/media/games/5b4/5b4af50ce68cf94dcab24f3dba33eaab.jpg",
          },
          released_at: "1998-11-08",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 6,
            name: "Linux",
            slug: "linux",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 76454,
            image_background:
              "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
          },
          released_at: "1998-11-08",
          requirements_en: {
            minimum:
              "Minimum: Linux Ubuntu 12.04, Dual-core from Intel or AMD at 2.8 GHz, 1GB Memory, nVidia GeForce 8600/9600GT, ATI/AMD Radeaon HD2600/3600 (Graphic Drivers: nVidia 310, AMD 12.11), OpenGL 2.1, 4GB Hard Drive Space, OpenAL Compatible Sound Card",
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 5,
            name: "macOS",
            slug: "macos",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 103400,
            image_background:
              "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
          },
          released_at: "1998-11-08",
          requirements_en: {
            minimum:
              "Minimum: OS X  Snow Leopard 10.6.3, 1GB RAM, 4GB Hard Drive Space,NVIDIA GeForce 8 or higher, ATI X1600 or higher, or Intel HD 3000 or higher Mouse, Keyboard, Internet Connection",
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "1998-11-08",
          requirements_en: {
            minimum:
              "<p><strong>Minimum:</strong> 500 mhz processor, 96mb ram, 16mb video card, Windows XP, Mouse, Keyboard, Internet Connection<br /></p>\r\n\t\t\t<p><strong>Recommended:</strong> 800 mhz processor, 128mb ram, 32mb+ video card, Windows XP, Mouse, Keyboard, Internet Connection<br /></p>",
          },
          requirements_ru: {
            minimum: "Pentium 133, 32 Мб памяти",
            recommended: "Pentium II 333 МГц, 64 Мб памяти, 3D-ускоритель",
          },
        },
        {
          platform: {
            id: 15,
            name: "PlayStation 2",
            slug: "playstation2",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 2032,
            image_background:
              "https://media.rawg.io/media/games/615/615e9fc0a325e0d87b84dad029b8b7b9.jpg",
          },
          released_at: "1998-11-08",
          requirements_en: null,
          requirements_ru: null,
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
        { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
        { platform: { id: 6, name: "Linux", slug: "linux" } },
        { platform: { id: 11, name: "SEGA", slug: "sega" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
        {
          id: 2,
          name: "Shooter",
          slug: "shooter",
          games_count: 59459,
          image_background:
            "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
        },
      ],
      stores: [
        {
          id: 20250,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 7,
          name: "Multiplayer",
          slug: "multiplayer",
          language: "eng",
          games_count: 37130,
          image_background:
            "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 42,
          name: "Great Soundtrack",
          slug: "great-soundtrack",
          language: "eng",
          games_count: 3386,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 118,
          name: "Story Rich",
          slug: "story-rich",
          language: "eng",
          games_count: 20561,
          image_background:
            "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
        },
        {
          id: 8,
          name: "First-Person",
          slug: "first-person",
          language: "eng",
          games_count: 30626,
          image_background:
            "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
        },
        {
          id: 32,
          name: "Sci-fi",
          slug: "sci-fi",
          language: "eng",
          games_count: 18535,
          image_background:
            "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
        },
        {
          id: 30,
          name: "FPS",
          slug: "fps",
          language: "eng",
          games_count: 12893,
          image_background:
            "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
        },
        {
          id: 4,
          name: "Funny",
          slug: "funny",
          language: "eng",
          games_count: 24154,
          image_background:
            "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
        },
        {
          id: 49,
          name: "Difficult",
          slug: "difficult",
          language: "eng",
          games_count: 13062,
          image_background:
            "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
        },
        {
          id: 193,
          name: "Classic",
          slug: "classic",
          language: "eng",
          games_count: 1783,
          image_background:
            "https://media.rawg.io/media/games/960/960b601d9541cec776c5fa42a00bf6c4.jpg",
        },
        {
          id: 74,
          name: "Retro",
          slug: "retro",
          language: "eng",
          games_count: 41033,
          image_background:
            "https://media.rawg.io/media/games/bbf/bbf8d74ab64440ad76294cff2f4d9cfa.jpg",
        },
        {
          id: 62,
          name: "Moddable",
          slug: "moddable",
          language: "eng",
          games_count: 880,
          image_background:
            "https://media.rawg.io/media/games/c22/c22d804ac753c72f2617b3708a625dec.jpg",
        },
        {
          id: 172,
          name: "Aliens",
          slug: "aliens",
          language: "eng",
          games_count: 6591,
          image_background:
            "https://media.rawg.io/media/games/ae1/ae1518c3dc1e847344661905fd2a8d16.jpg",
        },
        {
          id: 40856,
          name: "Valve Anti-Cheat enabled",
          slug: "valve-anti-cheat-enabled",
          language: "eng",
          games_count: 105,
          image_background:
            "https://media.rawg.io/media/games/78d/78dfae12fb8c5b16cd78648553071e0a.jpg",
        },
        {
          id: 243,
          name: "1990's",
          slug: "1990s",
          language: "eng",
          games_count: 2299,
          image_background:
            "https://media.rawg.io/media/games/202/20253cf9a7bd4f3ddb26bf68cf9117ba.jpg",
        },
        {
          id: 305,
          name: "Linear",
          slug: "linear",
          language: "eng",
          games_count: 5442,
          image_background:
            "https://media.rawg.io/media/games/569/569ea25d2b56bd05c7fa309ddabe81ff.jpg",
        },
        {
          id: 319,
          name: "Silent Protagonist",
          slug: "silent-protagonist",
          language: "eng",
          games_count: 91,
          image_background:
            "https://media.rawg.io/media/games/198/1988a337305e008b41d7f536ce9b73f6.jpg",
        },
        {
          id: 62349,
          name: "vr mod",
          slug: "vr-mod",
          language: "eng",
          games_count: 17,
          image_background:
            "https://media.rawg.io/media/screenshots/1bb/1bb3f78f0fe43b5d5ca2f3da5b638840.jpg",
        },
      ],
      esrb_rating: { id: 4, name: "Mature", slug: "mature" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/6c5/6c55e22185876626881b76c11922b073.jpg",
        },
        {
          id: 167835,
          image:
            "https://media.rawg.io/media/screenshots/345/3458269ae8ea44a6b8c8268d39fe36a1.jpg",
        },
        {
          id: 167836,
          image:
            "https://media.rawg.io/media/screenshots/3c5/3c55c835054009de798c0a9fa886ef8b.jpg",
        },
        {
          id: 167837,
          image:
            "https://media.rawg.io/media/screenshots/700/70005f6c84708d988a287d406cbb038c.jpg",
        },
        {
          id: 167838,
          image:
            "https://media.rawg.io/media/screenshots/755/755a2b42257cf09c7f37e902dfa08400.jpg",
        },
        {
          id: 167839,
          image:
            "https://media.rawg.io/media/screenshots/921/9213ce16eba80f51b3562a3e5d322e02.jpg",
        },
        {
          id: 167840,
          image:
            "https://media.rawg.io/media/screenshots/948/948de39d02c1f08c8c98fa10e69e87af.jpg",
        },
      ],
    },
    {
      id: 3790,
      slug: "outlast",
      name: "Outlast",
      released: "2013-09-03",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/9dd/9ddabb34840ea9227556670606cf8ea3.jpg",
      rating: 3.73,
      rating_top: 4,
      ratings: [
        { id: 4, title: "recommended", count: 945, percent: 54.03 },
        { id: 3, title: "meh", count: 352, percent: 20.13 },
        { id: 5, title: "exceptional", count: 311, percent: 17.78 },
        { id: 1, title: "skip", count: 141, percent: 8.06 },
      ],
      ratings_count: 1738,
      reviews_text_count: 9,
      added: 10053,
      added_by_status: {
        yet: 595,
        owned: 7173,
        beaten: 1437,
        toplay: 181,
        dropped: 628,
        playing: 39,
      },
      metacritic: 80,
      playtime: 3,
      suggestions_count: 469,
      updated: "2024-03-17T10:46:00",
      user_game: null,
      reviews_count: 1749,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 6,
            name: "Linux",
            slug: "linux",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 76454,
            image_background:
              "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
          },
          released_at: "2013-09-03",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 12 / Fedora 20 / Steam OS<br></li><li><strong>Processor:</strong> 2.2 GHz Dual Core CPU<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> GeForce GT 750M / 9800GTX / Radeon HD 3xxx. OpenGL 3.2+<br></li><li><strong>Storage:</strong> 5 GB available space<br></li><li><strong>Sound Card:</strong> Standard</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 14 / Fedora 21 / Steam OS<br></li><li><strong>Processor:</strong> 2.8 GHz Quad Core CPU<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> 1GB NVIDIA GTX 460 / ATI Radeon HD 6850. OpenGL 4.x+<br></li><li><strong>Storage:</strong> 5 GB available space<br></li><li><strong>Sound Card:</strong> Standard</li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 5,
            name: "macOS",
            slug: "macos",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 103400,
            image_background:
              "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
          },
          released_at: "2013-09-03",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> OS X 10.9<br></li><li><strong>Processor:</strong> 2.2 GHz Dual Core CPU<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> GeForce GT 750M / Intel Iris Pro 5200 / Radeon HD 3xxx<br></li><li><strong>Storage:</strong> 5 GB available space<br></li><li><strong>Sound Card:</strong> Standard</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> OS X 10.9<br></li><li><strong>Processor:</strong> 2.8 GHz Quad Core CPU<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> 1GB NVIDIA GTX 460 / ATI Radeon HD 6850<br></li><li><strong>Storage:</strong> 5 GB available space<br></li><li><strong>Sound Card:</strong> Standard</li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 7,
            name: "Nintendo Switch",
            slug: "nintendo-switch",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5406,
            image_background:
              "https://media.rawg.io/media/games/5eb/5eb49eb2fa0738fdb5bacea557b1bc57.jpg",
          },
          released_at: "2013-09-03",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2013-09-03",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows XP / Vista / 7 / 8 - 64 bits *<br></li><li><strong>Processor:</strong> 2.2 GHz Dual Core CPU<br></li><li><strong>Memory:</strong> 2 GB RAM<br></li><li><strong>Graphics:</strong> 512 MB NVIDIA GeForce 9800GTX / ATI Radeon HD 3xxx series<br></li><li><strong>DirectX:</strong> Version 9.0c<br></li><li><strong>Network:</strong> Broadband Internet connection<br></li><li><strong>Storage:</strong> 5 GB available space<br></li><li><strong>Sound Card:</strong> DirectX Compatible<br></li><li><strong>Additional Notes:</strong> * 32 bits systems are not officially supported, but should work if configured to provide 3Gb of user-mode address space. See <a href="https://steamcommunity.com/linkfilter/?url=http://msdn.microsoft.com/en-us/windows/bb613473" target="_blank" rel="noopener"  >http://msdn.microsoft.com/en-us/windows/bb613473</a> or <a href="http://steamcommunity.com/sharedfiles/filedetails/?id=175801311" target="_blank" rel="noreferrer"  id="dynamiclink_0" >http://steamcommunity.com/sharedfiles/filedetails/?id=175801311</a></li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows Vista / 7 / 8 - 64 bits<br></li><li><strong>Processor:</strong> 2.8 GHz Quad Core CPU<br></li><li><strong>Memory:</strong> 3 GB RAM<br></li><li><strong>Graphics:</strong> 1GB NVIDIA GTX 460 / ATI Radeon HD 6850 or better<br></li><li><strong>DirectX:</strong> Version 9.0c<br></li><li><strong>Network:</strong> Broadband Internet connection<br></li><li><strong>Storage:</strong> 5 GB available space<br></li><li><strong>Sound Card:</strong> DirectX Compatible</li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 1,
            name: "Xbox One",
            slug: "xbox-one",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5590,
            image_background:
              "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
          },
          released_at: "2013-09-03",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 18,
            name: "PlayStation 4",
            slug: "playstation4",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 6767,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
          released_at: "2013-09-03",
          requirements_en: null,
          requirements_ru: null,
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
        { platform: { id: 3, name: "Xbox", slug: "xbox" } },
        { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
        { platform: { id: 6, name: "Linux", slug: "linux" } },
        { platform: { id: 7, name: "Nintendo", slug: "nintendo" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
        {
          id: 51,
          name: "Indie",
          slug: "indie",
          games_count: 62257,
          image_background:
            "https://media.rawg.io/media/games/9dd/9ddabb34840ea9227556670606cf8ea3.jpg",
        },
      ],
      stores: [
        {
          id: 863662,
          store: {
            id: 11,
            name: "Epic Games",
            slug: "epic-games",
            domain: "epicgames.com",
            games_count: 1319,
            image_background:
              "https://media.rawg.io/media/games/4be/4be6a6ad0364751a96229c56bf69be59.jpg",
          },
        },
        {
          id: 4100,
          store: {
            id: 3,
            name: "PlayStation Store",
            slug: "playstation-store",
            domain: "store.playstation.com",
            games_count: 7900,
            image_background:
              "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
          },
        },
        {
          id: 7939,
          store: {
            id: 2,
            name: "Xbox Store",
            slug: "xbox-store",
            domain: "microsoft.com",
            games_count: 4813,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
        },
        {
          id: 26098,
          store: {
            id: 5,
            name: "GOG",
            slug: "gog",
            domain: "gog.com",
            games_count: 5817,
            image_background:
              "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
          },
        },
        {
          id: 20159,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 40847,
          name: "Steam Achievements",
          slug: "steam-achievements",
          language: "eng",
          games_count: 35702,
          image_background:
            "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
        },
        {
          id: 40836,
          name: "Full controller support",
          slug: "full-controller-support",
          language: "eng",
          games_count: 16746,
          image_background:
            "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
        },
        {
          id: 40849,
          name: "Steam Cloud",
          slug: "steam-cloud",
          language: "eng",
          games_count: 16769,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 7808,
          name: "steam-trading-cards",
          slug: "steam-trading-cards",
          language: "eng",
          games_count: 7569,
          image_background:
            "https://media.rawg.io/media/games/6fc/6fcf4cd3b17c288821388e6085bb0fc9.jpg",
        },
        {
          id: 8,
          name: "First-Person",
          slug: "first-person",
          language: "eng",
          games_count: 30626,
          image_background:
            "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
        },
        {
          id: 32,
          name: "Sci-fi",
          slug: "sci-fi",
          language: "eng",
          games_count: 18535,
          image_background:
            "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
        },
        {
          id: 16,
          name: "Horror",
          slug: "horror",
          language: "eng",
          games_count: 43534,
          image_background:
            "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
        },
        {
          id: 30,
          name: "FPS",
          slug: "fps",
          language: "eng",
          games_count: 12893,
          image_background:
            "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
        },
        {
          id: 26,
          name: "Gore",
          slug: "gore",
          language: "eng",
          games_count: 5515,
          image_background:
            "https://media.rawg.io/media/games/7f6/7f6cd70ba2ad57053b4847c13569f2d8.jpg",
        },
        {
          id: 49,
          name: "Difficult",
          slug: "difficult",
          language: "eng",
          games_count: 13062,
          image_background:
            "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
        },
        {
          id: 1,
          name: "Survival",
          slug: "survival",
          language: "eng",
          games_count: 8231,
          image_background:
            "https://media.rawg.io/media/games/476/476178ef18ab0534771d099f51cdc694.jpg",
        },
        {
          id: 15,
          name: "Stealth",
          slug: "stealth",
          language: "eng",
          games_count: 6100,
          image_background:
            "https://media.rawg.io/media/games/b7d/b7d3f1715fa8381a4e780173a197a615.jpg",
        },
        {
          id: 41,
          name: "Dark",
          slug: "dark",
          language: "eng",
          games_count: 15192,
          image_background:
            "https://media.rawg.io/media/games/490/49016e06ae2103881ff6373248843069.jpg",
        },
        {
          id: 44,
          name: "Nudity",
          slug: "nudity",
          language: "eng",
          games_count: 6072,
          image_background:
            "https://media.rawg.io/media/games/260/26023c855f1769a93411d6a7ea084632.jpeg",
        },
        {
          id: 336,
          name: "controller support",
          slug: "controller-support",
          language: "eng",
          games_count: 293,
          image_background:
            "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
        },
        {
          id: 17,
          name: "Survival Horror",
          slug: "survival-horror",
          language: "eng",
          games_count: 8168,
          image_background:
            "https://media.rawg.io/media/games/ebd/ebdbb7eb52bd58b0e7fa4538d9757b60.jpg",
        },
        {
          id: 40833,
          name: "Captions available",
          slug: "captions-available",
          language: "eng",
          games_count: 1311,
          image_background:
            "https://media.rawg.io/media/games/7a2/7a2500ee8b2c0e1ff268bb4479463dea.jpg",
        },
        {
          id: 91,
          name: "Walking Simulator",
          slug: "walking-simulator",
          language: "eng",
          games_count: 6811,
          image_background:
            "https://media.rawg.io/media/games/639/639ce7d7fecbb9f0717e9fbc1180f8f8.jpg",
        },
        {
          id: 188,
          name: "Parkour",
          slug: "parkour",
          language: "eng",
          games_count: 3425,
          image_background:
            "https://media.rawg.io/media/games/c2e/c2eb6021a2596644b437e943612af25c.jpg",
        },
        {
          id: 133,
          name: "3D Vision",
          slug: "3d-vision",
          language: "eng",
          games_count: 737,
          image_background:
            "https://media.rawg.io/media/games/7cf/7cf38d2d80267c121c6d0d361e9429ce.jpg",
        },
      ],
      esrb_rating: { id: 4, name: "Mature", slug: "mature" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/9dd/9ddabb34840ea9227556670606cf8ea3.jpg",
        },
        {
          id: 32637,
          image:
            "https://media.rawg.io/media/screenshots/83f/83ff600f8e2dd8507e7961d3e9f32126.jpg",
        },
        {
          id: 32638,
          image:
            "https://media.rawg.io/media/screenshots/283/283c90039e31e07f99979ccb445cf7b7.jpg",
        },
        {
          id: 32639,
          image:
            "https://media.rawg.io/media/screenshots/03f/03f4171763bda5824da07fc087cec609.jpg",
        },
        {
          id: 32640,
          image:
            "https://media.rawg.io/media/screenshots/37a/37acd5ef186c8e018cbd64751b21f064.jpg",
        },
        {
          id: 32641,
          image:
            "https://media.rawg.io/media/screenshots/242/2426226b9eb1a7de43b8bf01ecb2c291.jpg",
        },
        {
          id: 32642,
          image:
            "https://media.rawg.io/media/screenshots/d06/d06d3baecb6da0ac9e53b40ade32d5f2.jpg",
        },
      ],
    },
    {
      id: 1447,
      slug: "deus-ex-mankind-divided",
      name: "Deus Ex: Mankind Divided",
      released: "2016-08-22",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
      rating: 3.97,
      rating_top: 4,
      ratings: [
        { id: 4, title: "recommended", count: 882, percent: 44.77 },
        { id: 5, title: "exceptional", count: 627, percent: 31.83 },
        { id: 3, title: "meh", count: 345, percent: 17.51 },
        { id: 1, title: "skip", count: 116, percent: 5.89 },
      ],
      ratings_count: 1950,
      reviews_text_count: 12,
      added: 9977,
      added_by_status: {
        yet: 903,
        owned: 6701,
        beaten: 1311,
        toplay: 395,
        dropped: 548,
        playing: 119,
      },
      metacritic: 83,
      playtime: 8,
      suggestions_count: 630,
      updated: "2024-03-13T20:19:07",
      user_game: null,
      reviews_count: 1970,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 6,
            name: "Linux",
            slug: "linux",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 76454,
            image_background:
              "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
          },
          released_at: "2016-08-22",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 16.10 64-bit, SteamOS 2.0<br></li><li><strong>Processor:</strong> Intel Core i3-4130, AMD FX8350<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> 2GB Nvidia 680 (driver version: 367.57) AMD GPUs are not supported<br></li><li><strong>Storage:</strong> 67 GB available space<br></li><li><strong>Additional Notes:</strong> AMD and Intel Graphics Cards are not supported at the time of release</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 16.10 64-bit, SteamOS 2.0<br></li><li><strong>Processor:</strong> Intel Core i7-3770K<br></li><li><strong>Memory:</strong> 16 GB RAM<br></li><li><strong>Graphics:</strong> 6GB Nvidia 1060 (driver version: 367.57)<br></li><li><strong>Storage:</strong> 70 GB available space<br></li><li><strong>Additional Notes:</strong> AMD and Intel Graphics Cards are not supported at the time of release</li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2016-08-22",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows 7.1SP1 or above (64-bit Operating System Required)<br></li><li><strong>Processor:</strong> Intel Core i3-2100 or AMD equivalent<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> AMD Radeon HD 7870 (2GB) or NVIDIA GeForce GTX 660 (2GB)<br></li><li><strong>Storage:</strong> 45 GB available space</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows 10 64-bit<br></li><li><strong>Processor:</strong> Intel Core i7-3770K or AMD FX 8350 Wraith<br></li><li><strong>Memory:</strong> 16 GB RAM<br></li><li><strong>Graphics:</strong> AMD Radeon RX 480 - 1920 x 1080 or NVIDIA GTX 970 - 1920 x 1080<br></li><li><strong>Storage:</strong> 55 GB available space<br></li><li><strong>Additional Notes:</strong> 55GB HD space includes DLC</li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 1,
            name: "Xbox One",
            slug: "xbox-one",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5590,
            image_background:
              "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
          },
          released_at: "2016-08-22",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 5,
            name: "macOS",
            slug: "macos",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 103400,
            image_background:
              "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
          },
          released_at: "2016-08-22",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 18,
            name: "PlayStation 4",
            slug: "playstation4",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 6767,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
          released_at: "2016-08-22",
          requirements_en: null,
          requirements_ru: null,
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
        { platform: { id: 3, name: "Xbox", slug: "xbox" } },
        { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
        { platform: { id: 6, name: "Linux", slug: "linux" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
        {
          id: 5,
          name: "RPG",
          slug: "role-playing-games-rpg",
          games_count: 55107,
          image_background:
            "https://media.rawg.io/media/games/618/618c2031a07bbff6b4f611f10b6bcdbc.jpg",
        },
      ],
      stores: [
        {
          id: 428219,
          store: {
            id: 5,
            name: "GOG",
            slug: "gog",
            domain: "gog.com",
            games_count: 5817,
            image_background:
              "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
          },
        },
        {
          id: 1486,
          store: {
            id: 3,
            name: "PlayStation Store",
            slug: "playstation-store",
            domain: "store.playstation.com",
            games_count: 7900,
            image_background:
              "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
          },
        },
        {
          id: 11939,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
        {
          id: 7125,
          store: {
            id: 2,
            name: "Xbox Store",
            slug: "xbox-store",
            domain: "microsoft.com",
            games_count: 4813,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 40836,
          name: "Full controller support",
          slug: "full-controller-support",
          language: "eng",
          games_count: 16746,
          image_background:
            "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 42,
          name: "Great Soundtrack",
          slug: "great-soundtrack",
          language: "eng",
          games_count: 3386,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 24,
          name: "RPG",
          slug: "rpg",
          language: "eng",
          games_count: 19865,
          image_background:
            "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
        },
        {
          id: 118,
          name: "Story Rich",
          slug: "story-rich",
          language: "eng",
          games_count: 20561,
          image_background:
            "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
        },
        {
          id: 36,
          name: "Open World",
          slug: "open-world",
          language: "eng",
          games_count: 7102,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        {
          id: 8,
          name: "First-Person",
          slug: "first-person",
          language: "eng",
          games_count: 30626,
          image_background:
            "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
        },
        {
          id: 149,
          name: "Third Person",
          slug: "third-person",
          language: "eng",
          games_count: 10861,
          image_background:
            "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
        },
        {
          id: 32,
          name: "Sci-fi",
          slug: "sci-fi",
          language: "eng",
          games_count: 18535,
          image_background:
            "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
        },
        {
          id: 30,
          name: "FPS",
          slug: "fps",
          language: "eng",
          games_count: 12893,
          image_background:
            "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
        },
        {
          id: 15,
          name: "Stealth",
          slug: "stealth",
          language: "eng",
          games_count: 6100,
          image_background:
            "https://media.rawg.io/media/games/b7d/b7d3f1715fa8381a4e780173a197a615.jpg",
        },
        {
          id: 167,
          name: "Futuristic",
          slug: "futuristic",
          language: "eng",
          games_count: 5050,
          image_background:
            "https://media.rawg.io/media/games/90f/90fd5e569bc4c4a666c588a732124908.jpg",
        },
        {
          id: 226,
          name: "Cyberpunk",
          slug: "cyberpunk",
          language: "eng",
          games_count: 4415,
          image_background:
            "https://media.rawg.io/media/games/a12/a12f806432cb385bc286f0935c49cd14.jpg",
        },
        {
          id: 119,
          name: "Dystopian",
          slug: "dystopian",
          language: "eng",
          games_count: 2001,
          image_background:
            "https://media.rawg.io/media/games/a0e/a0ef08621301a1eab5e04fa5c96978fa.jpeg",
        },
        {
          id: 178,
          name: "Illuminati",
          slug: "illuminati",
          language: "eng",
          games_count: 315,
          image_background:
            "https://media.rawg.io/media/games/46b/46b62c37c8faf3268a0768b548e6cd9d.jpg",
        },
        {
          id: 291,
          name: "Conspiracy",
          slug: "conspiracy",
          language: "eng",
          games_count: 720,
          image_background:
            "https://media.rawg.io/media/games/338/33848ebb15683ea022c08f39f01b810f.jpg",
        },
        {
          id: 301,
          name: "Transhumanism",
          slug: "transhumanism",
          language: "eng",
          games_count: 149,
          image_background:
            "https://media.rawg.io/media/games/ca7/ca7ca88681ad87eccd12e2acc65a2f6d.jpg",
        },
      ],
      esrb_rating: { id: 4, name: "Mature", slug: "mature" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        {
          id: 12861,
          image:
            "https://media.rawg.io/media/screenshots/d4c/d4ce2d053a78f5e05cea8c99be22b135.jpg",
        },
        {
          id: 12864,
          image:
            "https://media.rawg.io/media/screenshots/515/515438994fe978193980d9b259ad7c50.jpg",
        },
        {
          id: 12865,
          image:
            "https://media.rawg.io/media/screenshots/0a0/0a0fd428643d0491c96bf29840d18d02.jpg",
        },
        {
          id: 12866,
          image:
            "https://media.rawg.io/media/screenshots/a31/a31228f42e1e5025cb35c2ab1b21bc98.jpg",
        },
        {
          id: 12869,
          image:
            "https://media.rawg.io/media/screenshots/f06/f066b272534017b7f33b09cca803bd18.jpg",
        },
        {
          id: 12873,
          image:
            "https://media.rawg.io/media/screenshots/2ae/2ae9537fd36039072f8ce5f5da592b68.jpg",
        },
      ],
    },
    {
      id: 4161,
      slug: "far-cry-3",
      name: "Far Cry 3",
      released: "2012-11-28",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/15c/15c95a4915f88a3e89c821526afe05fc.jpg",
      rating: 4.24,
      rating_top: 4,
      ratings: [
        { id: 4, title: "recommended", count: 1383, percent: 49.23 },
        { id: 5, title: "exceptional", count: 1116, percent: 39.73 },
        { id: 3, title: "meh", count: 244, percent: 8.69 },
        { id: 1, title: "skip", count: 66, percent: 2.35 },
      ],
      ratings_count: 2777,
      reviews_text_count: 17,
      added: 9941,
      added_by_status: {
        yet: 309,
        owned: 5465,
        beaten: 3354,
        toplay: 205,
        dropped: 542,
        playing: 66,
      },
      metacritic: 88,
      playtime: 16,
      suggestions_count: 626,
      updated: "2024-03-19T23:35:25",
      user_game: null,
      reviews_count: 2809,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 1,
            name: "Xbox One",
            slug: "xbox-one",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5590,
            image_background:
              "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
          },
          released_at: "2012-11-28",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 14,
            name: "Xbox 360",
            slug: "xbox360",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 2796,
            image_background:
              "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
          },
          released_at: "2012-11-28",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 16,
            name: "PlayStation 3",
            slug: "playstation3",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 3168,
            image_background:
              "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
          },
          released_at: "2012-11-28",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2012-11-28",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong>Windows XP, Windows Vista and Windows 7<br>\t</li><li><strong>Processor:</strong>Intel Core®2 Duo E6700 @ 2.6 GHz or AMD Athlon64 X2 6000+ @ 3.0Ghz or better<br>\t</li><li><strong>Memory:</strong>4 GB RAM<br>\t</li><li><strong>Graphics:</strong>512MB Video RAM (1GB Video RAM), DirectX9c (DirectX11) Shader Model 3.0 (Shader Model 5.0)<br>\t</li><li><strong>DirectX®:</strong>9.0c<br>\t</li><li><strong>Hard Drive:</strong>15 GB HD space<br>\t</li><li><strong>Sound:</strong>DirectX Compatible  (Recommended Surround Sound 5.1 capable)<br>\t</li><li><strong>Other Requirements:</strong>Broadband Internet connection<br>\t</li><li><strong>Additional:</strong>*Supported Video Cards at Time of Release: AMD Radeon™ HD 2900 / 3000 / 4000 / 5000 / 6000 / 7000 series, NVIDIA® GeForce® 8800 GTX / 9 / 200 / 400 / 500 / 600 series. Laptop versions of these cards may work, but are not supported. These chipsets are the only ones that will run this game. For the most up-to-date minimum requirement listings, please visit the FAQ on our support website at <a href="https://steamcommunity.com/linkfilter/?url=http://support.ubi.com" target="_blank" rel="noopener"  >http://support.ubi.com</a>.<br>Requires a UPlay account</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>Memory:</strong>4 GB RAM<br>\t</li><li><strong>Graphics:</strong>512MB Video RAM (1GB Video RAM), DirectX9c (DirectX11) Shader Model 3.0 (Shader Model 5.0)<br>\t</li><li><strong>DirectX®:</strong>11<br>\t</li><li><strong>Hard Drive:</strong>15 GB HD space<br>\t</li><li><strong>Sound:</strong>DirectX Compatible  (Recommended Surround Sound 5.1 capable)<br>\t</li><li><strong>Other Requirements:</strong>Broadband Internet connection<br>\t</li><li><strong>Additional:</strong>*Supported Video Cards at Time of Release: AMD Radeon™ HD 2900 / 3000 / 4000 / 5000 / 6000 / 7000 series, NVIDIA® GeForce® 8800 GTX / 9 / 200 / 400 / 500 / 600 series. Laptop versions of these cards may work, but are not supported. These chipsets are the only ones that will run this game. For the most up-to-date minimum requirement listings, please visit the FAQ on our support website at <a href="https://steamcommunity.com/linkfilter/?url=http://support.ubi.com" target="_blank" rel="noopener"  >http://support.ubi.com</a>.<br>Requires a UPlay account</li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 18,
            name: "PlayStation 4",
            slug: "playstation4",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 6767,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
          released_at: "2012-11-28",
          requirements_en: null,
          requirements_ru: null,
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
        { platform: { id: 3, name: "Xbox", slug: "xbox" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
        {
          id: 2,
          name: "Shooter",
          slug: "shooter",
          games_count: 59459,
          image_background:
            "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
        },
      ],
      stores: [
        {
          id: 465959,
          store: {
            id: 2,
            name: "Xbox Store",
            slug: "xbox-store",
            domain: "microsoft.com",
            games_count: 4813,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
        },
        {
          id: 4486,
          store: {
            id: 3,
            name: "PlayStation Store",
            slug: "playstation-store",
            domain: "store.playstation.com",
            games_count: 7900,
            image_background:
              "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
          },
        },
        {
          id: 19655,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
        {
          id: 33907,
          store: {
            id: 7,
            name: "Xbox 360 Store",
            slug: "xbox360",
            domain: "marketplace.xbox.com",
            games_count: 1912,
            image_background:
              "https://media.rawg.io/media/games/d1a/d1a2e99ade53494c6330a0ed945fe823.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 7,
          name: "Multiplayer",
          slug: "multiplayer",
          language: "eng",
          games_count: 37130,
          image_background:
            "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 24,
          name: "RPG",
          slug: "rpg",
          language: "eng",
          games_count: 19865,
          image_background:
            "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
        },
        {
          id: 18,
          name: "Co-op",
          slug: "co-op",
          language: "eng",
          games_count: 11072,
          image_background:
            "https://media.rawg.io/media/games/511/5118aff5091cb3efec399c808f8c598f.jpg",
        },
        {
          id: 118,
          name: "Story Rich",
          slug: "story-rich",
          language: "eng",
          games_count: 20561,
          image_background:
            "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
        },
        {
          id: 36,
          name: "Open World",
          slug: "open-world",
          language: "eng",
          games_count: 7102,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        {
          id: 411,
          name: "cooperative",
          slug: "cooperative",
          language: "eng",
          games_count: 4703,
          image_background:
            "https://media.rawg.io/media/games/73e/73eecb8909e0c39fb246f457b5d6cbbe.jpg",
        },
        {
          id: 8,
          name: "First-Person",
          slug: "first-person",
          language: "eng",
          games_count: 30626,
          image_background:
            "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
        },
        {
          id: 40845,
          name: "Partial Controller Support",
          slug: "partial-controller-support",
          language: "eng",
          games_count: 11229,
          image_background:
            "https://media.rawg.io/media/games/62c/62c7c8b28a27b83680b22fb9d33fc619.jpg",
        },
        {
          id: 30,
          name: "FPS",
          slug: "fps",
          language: "eng",
          games_count: 12893,
          image_background:
            "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
        },
        {
          id: 9,
          name: "Online Co-Op",
          slug: "online-co-op",
          language: "eng",
          games_count: 5203,
          image_background:
            "https://media.rawg.io/media/games/48c/48cb04ca483be865e3a83119c94e6097.jpg",
        },
        {
          id: 37,
          name: "Sandbox",
          slug: "sandbox",
          language: "eng",
          games_count: 6607,
          image_background:
            "https://media.rawg.io/media/games/7cf/7cfc9220b401b7a300e409e539c9afd5.jpg",
        },
        {
          id: 1,
          name: "Survival",
          slug: "survival",
          language: "eng",
          games_count: 8231,
          image_background:
            "https://media.rawg.io/media/games/476/476178ef18ab0534771d099f51cdc694.jpg",
        },
        {
          id: 15,
          name: "Stealth",
          slug: "stealth",
          language: "eng",
          games_count: 6100,
          image_background:
            "https://media.rawg.io/media/games/b7d/b7d3f1715fa8381a4e780173a197a615.jpg",
        },
        {
          id: 44,
          name: "Nudity",
          slug: "nudity",
          language: "eng",
          games_count: 6072,
          image_background:
            "https://media.rawg.io/media/games/260/26023c855f1769a93411d6a7ea084632.jpeg",
        },
        {
          id: 165,
          name: "Colorful",
          slug: "colorful",
          language: "eng",
          games_count: 20827,
          image_background:
            "https://media.rawg.io/media/games/3e3/3e355e1b8a5ee47f4c76e28e3055236d.jpg",
        },
        {
          id: 125,
          name: "Crafting",
          slug: "crafting",
          language: "eng",
          games_count: 3896,
          image_background:
            "https://media.rawg.io/media/games/152/152e788b7504aa2753c86dae912fb34c.jpg",
        },
        {
          id: 181,
          name: "Hunting",
          slug: "hunting",
          language: "eng",
          games_count: 901,
          image_background:
            "https://media.rawg.io/media/screenshots/599/59905f4cc9614a53f36989a26098d13c.jpg",
        },
      ],
      esrb_rating: { id: 4, name: "Mature", slug: "mature" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/15c/15c95a4915f88a3e89c821526afe05fc.jpg",
        },
        {
          id: 161732,
          image:
            "https://media.rawg.io/media/screenshots/98e/98e4c2a0c3e84b3d2718f8801bba0fcc.jpg",
        },
        {
          id: 161733,
          image:
            "https://media.rawg.io/media/screenshots/87e/87e9ca5542b3e8da43d488c9252e20fe.jpg",
        },
        {
          id: 161734,
          image:
            "https://media.rawg.io/media/screenshots/0a9/0a9ff8bbfa49024159bcd884ce7128c3.jpg",
        },
        {
          id: 161735,
          image:
            "https://media.rawg.io/media/screenshots/971/971d3582a42ede482b9d90b47b50ac32.jpg",
        },
        {
          id: 161736,
          image:
            "https://media.rawg.io/media/screenshots/174/174333d080f475d5718219ef6e01d3f4.jpg",
        },
        {
          id: 161737,
          image:
            "https://media.rawg.io/media/screenshots/03d/03dcdb4292bcc6ccc2719c88202f4965.jpg",
        },
      ],
    },
    {
      id: 10754,
      slug: "bioshock-remastered",
      name: "BioShock Remastered",
      released: "2016-09-15",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/be0/be01c3d7d8795a45615da139322ca080.jpg",
      rating: 4.24,
      rating_top: 5,
      ratings: [
        { id: 5, title: "exceptional", count: 642, percent: 46.76 },
        { id: 4, title: "recommended", count: 554, percent: 40.35 },
        { id: 3, title: "meh", count: 109, percent: 7.94 },
        { id: 1, title: "skip", count: 68, percent: 4.95 },
      ],
      ratings_count: 1359,
      reviews_text_count: 8,
      added: 9866,
      added_by_status: {
        yet: 699,
        owned: 7420,
        beaten: 1185,
        toplay: 158,
        dropped: 299,
        playing: 105,
      },
      metacritic: null,
      playtime: 4,
      suggestions_count: 258,
      updated: "2024-03-11T16:06:18",
      user_game: null,
      reviews_count: 1373,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 18,
            name: "PlayStation 4",
            slug: "playstation4",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 6767,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
          released_at: "2016-09-15",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 7,
            name: "Nintendo Switch",
            slug: "nintendo-switch",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5406,
            image_background:
              "https://media.rawg.io/media/games/5eb/5eb49eb2fa0738fdb5bacea557b1bc57.jpg",
          },
          released_at: "2016-09-15",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 5,
            name: "macOS",
            slug: "macos",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 103400,
            image_background:
              "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
          },
          released_at: "2016-09-15",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2016-09-15",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows 7 Service Pack 1 64-bit. Platform Update for Windows 7 SP1 and Windows Server 2008 R2 SP1<br></li><li><strong>Processor:</strong> Intel E6750 Core 2 Duo 2.66 GHz / AMD Athlon X2 2.7 GHZ<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> 2GB AMD Radeon HD 7770 / 2GB NVIDIA GeForce GTX 670<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Storage:</strong> 25 GB available space<br></li><li><strong>Sound Card:</strong> DirectX Compatible Sound Device<br></li><li><strong>Additional Notes:</strong> Other Requirements: Software installations required including DirectX and Microsoft Visual C++ 2010 &amp; 2012 Redistributable Package. <strong>Some system components such as mobile chipsets, integrated, and AGP graphics cards may be incompatible. Unlisted specifications may not be supported by publisher. </strong></li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows 7 Service Pack 1 64-bit. Platform Update for Windows 7 SP1 and Windows Server 2008 R2 SP1<br></li><li><strong>Processor:</strong> 3GHz Quad-Core<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> 2GB ATI Radeon HD 7970, 2GB NVIDIA GeForce GTX 770 or better<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Storage:</strong> 25 GB available space<br></li><li><strong>Sound Card:</strong> DirectX Compatible Sound Device</li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 1,
            name: "Xbox One",
            slug: "xbox-one",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5590,
            image_background:
              "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
          },
          released_at: "2016-09-15",
          requirements_en: null,
          requirements_ru: null,
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
        { platform: { id: 3, name: "Xbox", slug: "xbox" } },
        { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
        { platform: { id: 7, name: "Nintendo", slug: "nintendo" } },
      ],
      genres: [
        {
          id: 2,
          name: "Shooter",
          slug: "shooter",
          games_count: 59459,
          image_background:
            "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
        },
      ],
      stores: [
        {
          id: 828867,
          store: {
            id: 11,
            name: "Epic Games",
            slug: "epic-games",
            domain: "epicgames.com",
            games_count: 1319,
            image_background:
              "https://media.rawg.io/media/games/4be/4be6a6ad0364751a96229c56bf69be59.jpg",
          },
        },
        {
          id: 440395,
          store: {
            id: 3,
            name: "PlayStation Store",
            slug: "playstation-store",
            domain: "store.playstation.com",
            games_count: 7900,
            image_background:
              "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
          },
        },
        {
          id: 525094,
          store: {
            id: 6,
            name: "Nintendo Store",
            slug: "nintendo",
            domain: "nintendo.com",
            games_count: 8966,
            image_background:
              "https://media.rawg.io/media/games/737/737ea5662211d2e0bbd6f5989189e4f1.jpg",
          },
        },
        {
          id: 11782,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
        {
          id: 315633,
          store: {
            id: 4,
            name: "App Store",
            slug: "apple-appstore",
            domain: "apps.apple.com",
            games_count: 75436,
            image_background:
              "https://media.rawg.io/media/games/f46/f466571d536f2e3ea9e815ad17177501.jpg",
          },
        },
        {
          id: 276209,
          store: {
            id: 5,
            name: "GOG",
            slug: "gog",
            domain: "gog.com",
            games_count: 5817,
            image_background:
              "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 40847,
          name: "Steam Achievements",
          slug: "steam-achievements",
          language: "eng",
          games_count: 35702,
          image_background:
            "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
        },
        {
          id: 40836,
          name: "Full controller support",
          slug: "full-controller-support",
          language: "eng",
          games_count: 16746,
          image_background:
            "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
        },
        {
          id: 40849,
          name: "Steam Cloud",
          slug: "steam-cloud",
          language: "eng",
          games_count: 16769,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 42,
          name: "Great Soundtrack",
          slug: "great-soundtrack",
          language: "eng",
          games_count: 3386,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 24,
          name: "RPG",
          slug: "rpg",
          language: "eng",
          games_count: 19865,
          image_background:
            "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
        },
        {
          id: 118,
          name: "Story Rich",
          slug: "story-rich",
          language: "eng",
          games_count: 20561,
          image_background:
            "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
        },
        {
          id: 8,
          name: "First-Person",
          slug: "first-person",
          language: "eng",
          games_count: 30626,
          image_background:
            "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
        },
        {
          id: 32,
          name: "Sci-fi",
          slug: "sci-fi",
          language: "eng",
          games_count: 18535,
          image_background:
            "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
        },
        {
          id: 16,
          name: "Horror",
          slug: "horror",
          language: "eng",
          games_count: 43534,
          image_background:
            "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
        },
        {
          id: 30,
          name: "FPS",
          slug: "fps",
          language: "eng",
          games_count: 12893,
          image_background:
            "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
        },
        {
          id: 193,
          name: "Classic",
          slug: "classic",
          language: "eng",
          games_count: 1783,
          image_background:
            "https://media.rawg.io/media/games/960/960b601d9541cec776c5fa42a00bf6c4.jpg",
        },
        {
          id: 41,
          name: "Dark",
          slug: "dark",
          language: "eng",
          games_count: 15192,
          image_background:
            "https://media.rawg.io/media/games/490/49016e06ae2103881ff6373248843069.jpg",
        },
        {
          id: 119,
          name: "Dystopian",
          slug: "dystopian",
          language: "eng",
          games_count: 2001,
          image_background:
            "https://media.rawg.io/media/games/a0e/a0ef08621301a1eab5e04fa5c96978fa.jpeg",
        },
        {
          id: 271,
          name: "Remake",
          slug: "remake",
          language: "eng",
          games_count: 1695,
          image_background:
            "https://media.rawg.io/media/games/297/297a51aa1f0999016d5a35e2e1d6d8ab.jpg",
        },
        {
          id: 154,
          name: "Steampunk",
          slug: "steampunk",
          language: "eng",
          games_count: 1169,
          image_background:
            "https://media.rawg.io/media/games/4e6/4e6e8e7f50c237d76f38f3c885dae3d2.jpg",
        },
        {
          id: 40834,
          name: "Commentary available",
          slug: "commentary-available",
          language: "eng",
          games_count: 266,
          image_background:
            "https://media.rawg.io/media/games/594/5949baae74fe9e399adbce0c44e28783.jpg",
        },
        {
          id: 208,
          name: "Alternate History",
          slug: "alternate-history",
          language: "eng",
          games_count: 1675,
          image_background:
            "https://media.rawg.io/media/games/f14/f1422eacab98c5f85a5123da4e9d9e89.jpg",
        },
        {
          id: 250,
          name: "Underwater",
          slug: "underwater",
          language: "eng",
          games_count: 2048,
          image_background:
            "https://media.rawg.io/media/games/386/3869a2540b5fd39776fd640686610d05.jpg",
        },
      ],
      esrb_rating: { id: 4, name: "Mature", slug: "mature" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/be0/be01c3d7d8795a45615da139322ca080.jpg",
        },
        {
          id: 84960,
          image:
            "https://media.rawg.io/media/screenshots/c70/c709280e11aabec614d0aafb5779114a.jpg",
        },
        {
          id: 84961,
          image:
            "https://media.rawg.io/media/screenshots/414/41463563e721aa62c605cd0bf8350af3.jpg",
        },
        {
          id: 84962,
          image:
            "https://media.rawg.io/media/screenshots/76d/76d11a76541bcfa63de3ef8fe5a5a668.jpg",
        },
        {
          id: 84963,
          image:
            "https://media.rawg.io/media/screenshots/25a/25ac07efdbf90a2d7e626353ebadc565.jpg",
        },
        {
          id: 84964,
          image:
            "https://media.rawg.io/media/screenshots/0f6/0f69943b1d81710ba2fdcac27d248ca9.jpg",
        },
      ],
    },
    {
      id: 11936,
      slug: "half-life-2-deathmatch",
      name: "Half-Life 2: Deathmatch",
      released: "2004-11-01",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
      rating: 3.29,
      rating_top: 4,
      ratings: [
        { id: 4, title: "recommended", count: 356, percent: 40.36 },
        { id: 3, title: "meh", count: 230, percent: 26.08 },
        { id: 1, title: "skip", count: 174, percent: 19.73 },
        { id: 5, title: "exceptional", count: 122, percent: 13.83 },
      ],
      ratings_count: 880,
      reviews_text_count: 2,
      added: 9805,
      added_by_status: {
        yet: 532,
        owned: 8478,
        beaten: 316,
        toplay: 23,
        dropped: 444,
        playing: 12,
      },
      metacritic: null,
      playtime: 1,
      suggestions_count: 196,
      updated: "2024-03-17T16:17:42",
      user_game: null,
      reviews_count: 882,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 6,
            name: "Linux",
            slug: "linux",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 76454,
            image_background:
              "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
          },
          released_at: "2004-11-01",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 5,
            name: "macOS",
            slug: "macos",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 103400,
            image_background:
              "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
          },
          released_at: "2004-11-01",
          requirements_en: {
            minimum:
              "<strong>Minimum: </strong>OS X version Leopard 10.5.8, Snow Leopard 10.6.3, 1GB RAM, NVIDIA GeForce 8 or higher, ATI X1600 or higher, or Intel HD 3000 or higher Mouse, Keyboard, Internet Connection",
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2004-11-01",
          requirements_en: {
            minimum:
              "\r\n\t\t\t<p><strong>Minimum:</strong> 1.2 GHz Processor, 256MB RAM, DirectX 7 level graphics card, Windows XP, Mouse, Keyboard, Internet Connection<br /></p>\r\n\t\t\t<p><strong>Recommended:</strong> 2.4 GHz Processor, 512MB RAM, DirectX 9 level graphics card, Windows XP, Mouse, Keyboard, Internet Connection<br /></p>\r\n\t\t\t",
          },
          requirements_ru: null,
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
        { platform: { id: 6, name: "Linux", slug: "linux" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
      ],
      stores: [
        {
          id: 13105,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 7,
          name: "Multiplayer",
          slug: "multiplayer",
          language: "eng",
          games_count: 37130,
          image_background:
            "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 18,
          name: "Co-op",
          slug: "co-op",
          language: "eng",
          games_count: 11072,
          image_background:
            "https://media.rawg.io/media/games/511/5118aff5091cb3efec399c808f8c598f.jpg",
        },
        {
          id: 411,
          name: "cooperative",
          slug: "cooperative",
          language: "eng",
          games_count: 4703,
          image_background:
            "https://media.rawg.io/media/games/73e/73eecb8909e0c39fb246f457b5d6cbbe.jpg",
        },
        {
          id: 8,
          name: "First-Person",
          slug: "first-person",
          language: "eng",
          games_count: 30626,
          image_background:
            "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
        },
        {
          id: 32,
          name: "Sci-fi",
          slug: "sci-fi",
          language: "eng",
          games_count: 18535,
          image_background:
            "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
        },
        {
          id: 30,
          name: "FPS",
          slug: "fps",
          language: "eng",
          games_count: 12893,
          image_background:
            "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
        },
        {
          id: 4,
          name: "Funny",
          slug: "funny",
          language: "eng",
          games_count: 24154,
          image_background:
            "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
        },
        {
          id: 79,
          name: "Free to Play",
          slug: "free-to-play",
          language: "eng",
          games_count: 6248,
          image_background:
            "https://media.rawg.io/media/games/dcb/dcbb67f371a9a28ea38ffd73ee0f53f3.jpg",
        },
        {
          id: 114,
          name: "Physics",
          slug: "physics",
          language: "eng",
          games_count: 18796,
          image_background:
            "https://media.rawg.io/media/screenshots/f5a/f5abab52c4d606551cd5ec3ab709e501.jpg",
        },
        {
          id: 40856,
          name: "Valve Anti-Cheat enabled",
          slug: "valve-anti-cheat-enabled",
          language: "eng",
          games_count: 105,
          image_background:
            "https://media.rawg.io/media/games/78d/78dfae12fb8c5b16cd78648553071e0a.jpg",
        },
        {
          id: 170,
          name: "Competitive",
          slug: "competitive",
          language: "eng",
          games_count: 1116,
          image_background:
            "https://media.rawg.io/media/games/9fb/9fbaea2168caea1f806546dfdaaeb1da.jpg",
        },
        {
          id: 40839,
          name: "Includes Source SDK",
          slug: "includes-source-sdk",
          language: "eng",
          games_count: 62,
          image_background:
            "https://media.rawg.io/media/screenshots/ae8/ae873e0da5973a9896bdf5ebce1e9bf4.jpg",
        },
      ],
      esrb_rating: { id: 4, name: "Mature", slug: "mature" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
        },
        {
          id: 98731,
          image:
            "https://media.rawg.io/media/screenshots/8e7/8e7039ef354447c969a9fe82329ca50e.jpg",
        },
        {
          id: 98732,
          image:
            "https://media.rawg.io/media/screenshots/0d0/0d0ff34c7bef12783cfe7c6dda7284b2.jpg",
        },
        {
          id: 98733,
          image:
            "https://media.rawg.io/media/screenshots/853/85349bbb51ad92fc351b61a89b6db19b.jpg",
        },
        {
          id: 98734,
          image:
            "https://media.rawg.io/media/screenshots/1fc/1fcc71ae3d931406dbc4b33e3446d457.jpg",
        },
        {
          id: 98735,
          image:
            "https://media.rawg.io/media/screenshots/c11/c11d517937d7261e9d357709ca3c3cc0.jpg",
        },
      ],
    },
    {
      id: 58134,
      slug: "marvels-spider-man",
      name: "Marvel's Spider-Man",
      released: "2018-09-07",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/9aa/9aa42d16d425fa6f179fc9dc2f763647.jpg",
      rating: 4.45,
      rating_top: 5,
      ratings: [
        { id: 5, title: "exceptional", count: 2065, percent: 58.32 },
        { id: 4, title: "recommended", count: 1185, percent: 33.47 },
        { id: 3, title: "meh", count: 208, percent: 5.87 },
        { id: 1, title: "skip", count: 83, percent: 2.34 },
      ],
      ratings_count: 3467,
      reviews_text_count: 56,
      added: 9794,
      added_by_status: {
        yet: 356,
        owned: 4824,
        beaten: 3187,
        toplay: 951,
        dropped: 210,
        playing: 266,
      },
      metacritic: 87,
      playtime: 6,
      suggestions_count: 583,
      updated: "2024-03-20T07:32:14",
      user_game: null,
      reviews_count: 3541,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 18,
            name: "PlayStation 4",
            slug: "playstation4",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 6767,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
          released_at: "2018-09-07",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2018-09-07",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 187,
            name: "PlayStation 5",
            slug: "playstation5",
            image: null,
            year_end: null,
            year_start: 2020,
            games_count: 1015,
            image_background:
              "https://media.rawg.io/media/games/909/909974d1c7863c2027241e265fe7011f.jpg",
          },
          released_at: "2018-09-07",
          requirements_en: null,
          requirements_ru: null,
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
      ],
      stores: [
        {
          id: 958559,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
        {
          id: 46837,
          store: {
            id: 3,
            name: "PlayStation Store",
            slug: "playstation-store",
            domain: "store.playstation.com",
            games_count: 7900,
            image_background:
              "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 36,
          name: "Open World",
          slug: "open-world",
          language: "eng",
          games_count: 7102,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        {
          id: 6,
          name: "Exploration",
          slug: "exploration",
          language: "eng",
          games_count: 21834,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
        {
          id: 69,
          name: "Action-Adventure",
          slug: "action-adventure",
          language: "eng",
          games_count: 15512,
          image_background:
            "https://media.rawg.io/media/games/253/2534a46f3da7fa7c315f1387515ca393.jpg",
        },
        {
          id: 37796,
          name: "exclusive",
          slug: "exclusive",
          language: "eng",
          games_count: 4496,
          image_background:
            "https://media.rawg.io/media/games/f24/f2493ea338fe7bd3c7d73750a85a0959.jpeg",
        },
        {
          id: 110,
          name: "Cinematic",
          slug: "cinematic",
          language: "eng",
          games_count: 1881,
          image_background:
            "https://media.rawg.io/media/games/9aa/9aa42d16d425fa6f179fc9dc2f763647.jpg",
        },
        {
          id: 203,
          name: "Beat 'em up",
          slug: "beat-em-up",
          language: "eng",
          games_count: 2933,
          image_background:
            "https://media.rawg.io/media/games/d1a/d1a1202a378607b6c635c8f18ace95dd.jpg",
        },
        {
          id: 37797,
          name: "true exclusive",
          slug: "true-exclusive",
          language: "eng",
          games_count: 3983,
          image_background:
            "https://media.rawg.io/media/screenshots/135/135a00cf33b0c860fcb366747e95b89b.jpg",
        },
        {
          id: 478,
          name: "3rd-Person Perspective",
          slug: "3rd-person-perspective",
          language: "eng",
          games_count: 87,
          image_background:
            "https://media.rawg.io/media/games/21c/21cc15d233117c6809ec86870559e105.jpg",
        },
        {
          id: 268,
          name: "Comic Book",
          slug: "comic-book",
          language: "eng",
          games_count: 1203,
          image_background:
            "https://media.rawg.io/media/games/a87/a8743bdee8627c55bb9f2f01b9136ac1.jpg",
        },
        {
          id: 234,
          name: "Superhero",
          slug: "superhero",
          language: "eng",
          games_count: 1319,
          image_background:
            "https://media.rawg.io/media/games/9e5/9e52a797f049e701d4eee84774a99007.jpg",
        },
        {
          id: 78,
          name: "America",
          slug: "america",
          language: "eng",
          games_count: 597,
          image_background:
            "https://media.rawg.io/media/games/909/909974d1c7863c2027241e265fe7011f.jpg",
        },
        {
          id: 43369,
          name: "new york",
          slug: "new-york-2",
          language: "eng",
          games_count: 2,
          image_background:
            "https://media.rawg.io/media/screenshots/e3c/e3cc9dcd0d4779fcdc08a01d150ea7f9.jpg",
        },
      ],
      esrb_rating: { id: 3, name: "Teen", slug: "teen" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/9aa/9aa42d16d425fa6f179fc9dc2f763647.jpg",
        },
        {
          id: 1325929,
          image:
            "https://media.rawg.io/media/screenshots/331/331ba5164c5c53a5d59aad3fe9771ac7.jpg",
        },
        {
          id: 1325930,
          image:
            "https://media.rawg.io/media/screenshots/a15/a15b42bd8a652a3733c6ad419ebb24bd.jpg",
        },
        {
          id: 1325931,
          image:
            "https://media.rawg.io/media/screenshots/150/150589c127b28f287f992c2bd426b443.jpg",
        },
        {
          id: 1325932,
          image:
            "https://media.rawg.io/media/screenshots/f52/f526988f895b554dccf68767557a8518.jpg",
        },
        {
          id: 1325958,
          image:
            "https://media.rawg.io/media/screenshots/745/74589db2dee21101d7af690976fca902.jpg",
        },
        {
          id: 1325959,
          image:
            "https://media.rawg.io/media/screenshots/090/09063845f2efe6d0b9bc908e2652c1e1.jpeg",
        },
      ],
    },
    {
      id: 4332,
      slug: "spec-ops-the-line",
      name: "Spec Ops: The Line",
      released: "2012-06-26",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/b49/b4912b5dbfc7ed8927b65f05b8507f6c.jpg",
      rating: 4.09,
      rating_top: 4,
      ratings: [
        { id: 4, title: "recommended", count: 907, percent: 47.99 },
        { id: 5, title: "exceptional", count: 675, percent: 35.71 },
        { id: 3, title: "meh", count: 212, percent: 11.22 },
        { id: 1, title: "skip", count: 96, percent: 5.08 },
      ],
      ratings_count: 1869,
      reviews_text_count: 12,
      added: 9781,
      added_by_status: {
        yet: 494,
        owned: 6907,
        beaten: 1822,
        toplay: 188,
        dropped: 332,
        playing: 38,
      },
      metacritic: 76,
      playtime: 5,
      suggestions_count: 532,
      updated: "2024-03-13T20:37:54",
      user_game: null,
      reviews_count: 1890,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 14,
            name: "Xbox 360",
            slug: "xbox360",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 2796,
            image_background:
              "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
          },
          released_at: "2012-06-26",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 16,
            name: "PlayStation 3",
            slug: "playstation3",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 3168,
            image_background:
              "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
          },
          released_at: "2012-06-26",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 1,
            name: "Xbox One",
            slug: "xbox-one",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5590,
            image_background:
              "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
          },
          released_at: "2012-06-26",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2012-06-26",
          requirements_en: {
            minimum:
              '<strong> Minimum</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows XP SP3<br>                </li><li><strong>Processor:</strong>  Intel Core 2 Duo @ 2Ghz / AMD Athlon 64 X2 equivalent<br>                </li><li><strong>Memory:</strong> 2 GB RAM<br>                </li><li><strong>Hard Disk Space:</strong>  6 GB free                <br>                </li><li><strong>Video Card:</strong> NVIDIA GeForce 8600 / ATI Radeon HD 2600XT (256 MB memory) <br>                </li><li><strong>DirectX®:</strong> 9.0<br>                </li><li><strong>Sound:</strong> DirectX Compatible<br></li><li><strong>Additional:</strong> Initial installation requires one-time internet connection for Steam authentication; software installations required (included with the game) include: STEAM Client, Microsoft Direct X, Visual C++ 2008 Redistributable, Visual C++ 2005 SP1 RedistributableCompatible      <br> </li></ul>',
            recommended:
              '<strong> Recommended</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows Vista/7<br>                </li><li><strong>Processor:</strong> 2.4 GHz Quad Core processor<br>                </li><li><strong>Memory:</strong> 3 GB RAM<br>                </li><li><strong>Hard Disk Space:</strong> 10 GB free               <br>                </li><li><strong>Video Card:</strong> NVIDIA GeForce 9800 GTX/ATI Radeon HD 4850 (512+ MB memory) <br>                </li><li><strong>DirectX®:</strong> 9.0<br>                </li><li><strong>Sound:</strong> DirectX Compatible     <br></li><li><strong>Additional:</strong> Initial installation requires one-time internet connection for Steam authentication; software installations required (included with the game) include: STEAM Client, Microsoft Direct X, Visual C++ 2008 Redistributable, Visual C++ 2005 SP1 RedistributableCompatible                  </li></ul>',
          },
          requirements_ru: {
            minimum:
              "Core 2 Duo/Athlon 64 X2 2 ГГц,2 Гб памяти,GeForce 8600/Radeon HD 2600,6 Гб на винчестере,интернет-соединение",
            recommended:
              "Core 2 Quad/Phenom X4 2.4 ГГц,3 Гб памяти,GeForce 9800 GTX/Radeon HD 4850,10 Гб на винчестере,интернет-соединение",
          },
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
        { platform: { id: 3, name: "Xbox", slug: "xbox" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
        {
          id: 2,
          name: "Shooter",
          slug: "shooter",
          games_count: 59459,
          image_background:
            "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
        },
      ],
      stores: [
        {
          id: 465902,
          store: {
            id: 2,
            name: "Xbox Store",
            slug: "xbox-store",
            domain: "microsoft.com",
            games_count: 4813,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
        },
        {
          id: 4660,
          store: {
            id: 3,
            name: "PlayStation Store",
            slug: "playstation-store",
            domain: "store.playstation.com",
            games_count: 7900,
            image_background:
              "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
          },
        },
        {
          id: 19791,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
        {
          id: 33912,
          store: {
            id: 7,
            name: "Xbox 360 Store",
            slug: "xbox360",
            domain: "marketplace.xbox.com",
            games_count: 1912,
            image_background:
              "https://media.rawg.io/media/games/d1a/d1a2e99ade53494c6330a0ed945fe823.jpg",
          },
        },
        {
          id: 320890,
          store: {
            id: 5,
            name: "GOG",
            slug: "gog",
            domain: "gog.com",
            games_count: 5817,
            image_background:
              "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 40847,
          name: "Steam Achievements",
          slug: "steam-achievements",
          language: "eng",
          games_count: 35702,
          image_background:
            "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
        },
        {
          id: 7,
          name: "Multiplayer",
          slug: "multiplayer",
          language: "eng",
          games_count: 37130,
          image_background:
            "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
        },
        {
          id: 40836,
          name: "Full controller support",
          slug: "full-controller-support",
          language: "eng",
          games_count: 16746,
          image_background:
            "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
        },
        {
          id: 40849,
          name: "Steam Cloud",
          slug: "steam-cloud",
          language: "eng",
          games_count: 16769,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 42,
          name: "Great Soundtrack",
          slug: "great-soundtrack",
          language: "eng",
          games_count: 3386,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 18,
          name: "Co-op",
          slug: "co-op",
          language: "eng",
          games_count: 11072,
          image_background:
            "https://media.rawg.io/media/games/511/5118aff5091cb3efec399c808f8c598f.jpg",
        },
        {
          id: 118,
          name: "Story Rich",
          slug: "story-rich",
          language: "eng",
          games_count: 20561,
          image_background:
            "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
        },
        {
          id: 149,
          name: "Third Person",
          slug: "third-person",
          language: "eng",
          games_count: 10861,
          image_background:
            "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
        },
        {
          id: 16,
          name: "Horror",
          slug: "horror",
          language: "eng",
          games_count: 43534,
          image_background:
            "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
        },
        {
          id: 40850,
          name: "Steam Leaderboards",
          slug: "steam-leaderboards",
          language: "eng",
          games_count: 6539,
          image_background:
            "https://media.rawg.io/media/games/59a/59a3ebcba3d08c51532c6ca877aff256.jpg",
        },
        {
          id: 150,
          name: "Third-Person Shooter",
          slug: "third-person-shooter",
          language: "eng",
          games_count: 3266,
          image_background:
            "https://media.rawg.io/media/games/ebd/ebdbb7eb52bd58b0e7fa4538d9757b60.jpg",
        },
        {
          id: 11669,
          name: "stats",
          slug: "stats",
          language: "eng",
          games_count: 4821,
          image_background:
            "https://media.rawg.io/media/games/f52/f5206d55f918edf8ee07803101106fa6.jpg",
        },
        {
          id: 41,
          name: "Dark",
          slug: "dark",
          language: "eng",
          games_count: 15192,
          image_background:
            "https://media.rawg.io/media/games/490/49016e06ae2103881ff6373248843069.jpg",
        },
        {
          id: 40832,
          name: "Cross-Platform Multiplayer",
          slug: "cross-platform-multiplayer",
          language: "eng",
          games_count: 2509,
          image_background:
            "https://media.rawg.io/media/screenshots/c97/c97b943741f5fbc936fe054d9d58851d.jpg",
        },
        {
          id: 70,
          name: "War",
          slug: "war",
          language: "eng",
          games_count: 9104,
          image_background:
            "https://media.rawg.io/media/games/2fd/2fd1b58116b10cc1f4442bee5593ca7c.jpg",
        },
        {
          id: 43,
          name: "Post-apocalyptic",
          slug: "post-apocalyptic",
          language: "eng",
          games_count: 3777,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        {
          id: 192,
          name: "Mature",
          slug: "mature",
          language: "eng",
          games_count: 2873,
          image_background:
            "https://media.rawg.io/media/games/849/849414b978db37d4563ff9e4b0d3a787.jpg",
        },
        {
          id: 81,
          name: "Military",
          slug: "military",
          language: "eng",
          games_count: 1727,
          image_background:
            "https://media.rawg.io/media/games/8bd/8bda6d876f3e241c6024022299553efd.jpg",
        },
        {
          id: 285,
          name: "Psychological",
          slug: "psychological",
          language: "eng",
          games_count: 1458,
          image_background:
            "https://media.rawg.io/media/games/643/6431f15213a854efd273943f2cf6a925.jpg",
        },
        {
          id: 283,
          name: "Based On A Novel",
          slug: "based-on-a-novel",
          language: "eng",
          games_count: 49,
          image_background:
            "https://media.rawg.io/media/screenshots/2b9/2b911fa5f17f537087631a4da9ede2af.jpg",
        },
        {
          id: 287,
          name: "Political",
          slug: "political",
          language: "eng",
          games_count: 577,
          image_background:
            "https://media.rawg.io/media/games/bc0/bc06a29ceac58652b684deefe7d56099.jpg",
        },
        {
          id: 19728,
          name: "european",
          slug: "european",
          language: "eng",
          games_count: 12,
          image_background:
            "https://media.rawg.io/media/screenshots/d14/d148f8ffc332b1d0f70be9bad91c2171.jpg",
        },
      ],
      esrb_rating: { id: 4, name: "Mature", slug: "mature" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/b49/b4912b5dbfc7ed8927b65f05b8507f6c.jpg",
        },
        {
          id: 163188,
          image:
            "https://media.rawg.io/media/screenshots/a17/a17ff71c8774a3b70375a869b3881244.jpg",
        },
        {
          id: 163189,
          image:
            "https://media.rawg.io/media/screenshots/e5a/e5aaa5d242144ab80ef8264c96516dcc.jpg",
        },
        {
          id: 163190,
          image:
            "https://media.rawg.io/media/screenshots/f36/f36e756c36d36fe8ffe73a4b39acbebf.jpg",
        },
        {
          id: 163191,
          image:
            "https://media.rawg.io/media/screenshots/437/437ad0efe43adcad4284f5f48d03559f.jpg",
        },
        {
          id: 163192,
          image:
            "https://media.rawg.io/media/screenshots/9e2/9e26af47a676b061f288ff269e91a8f1.jpg",
        },
        {
          id: 163193,
          image:
            "https://media.rawg.io/media/screenshots/2d9/2d9a5dd6a25c04e49accca5358782152.jpg",
        },
      ],
    },
    {
      id: 19487,
      slug: "alan-wake",
      name: "Alan Wake",
      released: "2010-05-14",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/5c0/5c0dd63002cb23f804aab327d40ef119.jpg",
      rating: 4.1,
      rating_top: 4,
      ratings: [
        { id: 4, title: "recommended", count: 1085, percent: 49.82 },
        { id: 5, title: "exceptional", count: 743, percent: 34.11 },
        { id: 3, title: "meh", count: 261, percent: 11.98 },
        { id: 1, title: "skip", count: 89, percent: 4.09 },
      ],
      ratings_count: 2161,
      reviews_text_count: 6,
      added: 9736,
      added_by_status: {
        yet: 572,
        owned: 6294,
        beaten: 1978,
        toplay: 293,
        dropped: 508,
        playing: 91,
      },
      metacritic: 83,
      playtime: 5,
      suggestions_count: 523,
      updated: "2024-03-20T07:25:55",
      user_game: null,
      reviews_count: 2178,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 14,
            name: "Xbox 360",
            slug: "xbox360",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 2796,
            image_background:
              "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
          },
          released_at: "2010-05-14",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 1,
            name: "Xbox One",
            slug: "xbox-one",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5590,
            image_background:
              "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
          },
          released_at: "2010-05-14",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2010-05-14",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br>\t\t\t\t\t\t\t\t\t\t\t\t<ul class="bb_ul"><li><strong>OS:</strong> Windows XP SP2<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Processor:</strong> Dual Core 2GHz Intel or 2.8GHz AMD<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Memory:</strong> 2 GB RAM<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Graphics:</strong> DirectX 10 compatible with 512MB RAM<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>DirectX®:</strong> 9.0c<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Hard Drive:</strong> 8 GB HD space<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Sound:</strong> DirectX 9.0c compatible<br>\t\t\t\t\t\t\t\t\t\t\t\t</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br>\t\t\t\t\t\t\t\t\t\t\t\t<ul class="bb_ul"><li><strong>OS:</strong> Windows 7<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Processor:</strong> Quad Core 2.66GHz Intel or 3.2GHz AMD<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Memory:</strong> 4 GB RAM<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Graphics:</strong> DirectX 10 compatible or later with 1GB RAM <br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>DirectX®:</strong> 10<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Hard Drive:</strong> 8 GB HD space<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Sound:</strong> DirectX 9.0c compatible<br>\t\t\t\t\t\t\t\t\t\t\t\t</li></ul>',
          },
          requirements_ru: null,
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 3, name: "Xbox", slug: "xbox" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
        {
          id: 2,
          name: "Shooter",
          slug: "shooter",
          games_count: 59459,
          image_background:
            "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
        },
        {
          id: 3,
          name: "Adventure",
          slug: "adventure",
          games_count: 137936,
          image_background:
            "https://media.rawg.io/media/games/0af/0af85e8edddfa55368e47c539914a220.jpg",
        },
      ],
      stores: [
        {
          id: 465958,
          store: {
            id: 2,
            name: "Xbox Store",
            slug: "xbox-store",
            domain: "microsoft.com",
            games_count: 4813,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
        },
        {
          id: 25873,
          store: {
            id: 5,
            name: "GOG",
            slug: "gog",
            domain: "gog.com",
            games_count: 5817,
            image_background:
              "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
          },
        },
        {
          id: 21833,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
        {
          id: 34002,
          store: {
            id: 7,
            name: "Xbox 360 Store",
            slug: "xbox360",
            domain: "marketplace.xbox.com",
            games_count: 1912,
            image_background:
              "https://media.rawg.io/media/games/d1a/d1a2e99ade53494c6330a0ed945fe823.jpg",
          },
        },
        {
          id: 339240,
          store: {
            id: 11,
            name: "Epic Games",
            slug: "epic-games",
            domain: "epicgames.com",
            games_count: 1319,
            image_background:
              "https://media.rawg.io/media/games/4be/4be6a6ad0364751a96229c56bf69be59.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 40847,
          name: "Steam Achievements",
          slug: "steam-achievements",
          language: "eng",
          games_count: 35702,
          image_background:
            "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
        },
        {
          id: 40836,
          name: "Full controller support",
          slug: "full-controller-support",
          language: "eng",
          games_count: 16746,
          image_background:
            "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
        },
        {
          id: 40849,
          name: "Steam Cloud",
          slug: "steam-cloud",
          language: "eng",
          games_count: 16769,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 7808,
          name: "steam-trading-cards",
          slug: "steam-trading-cards",
          language: "eng",
          games_count: 7569,
          image_background:
            "https://media.rawg.io/media/games/6fc/6fcf4cd3b17c288821388e6085bb0fc9.jpg",
        },
        {
          id: 42,
          name: "Great Soundtrack",
          slug: "great-soundtrack",
          language: "eng",
          games_count: 3386,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 118,
          name: "Story Rich",
          slug: "story-rich",
          language: "eng",
          games_count: 20561,
          image_background:
            "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
        },
        {
          id: 36,
          name: "Open World",
          slug: "open-world",
          language: "eng",
          games_count: 7102,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        {
          id: 149,
          name: "Third Person",
          slug: "third-person",
          language: "eng",
          games_count: 10861,
          image_background:
            "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
        },
        {
          id: 16,
          name: "Horror",
          slug: "horror",
          language: "eng",
          games_count: 43534,
          image_background:
            "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
        },
        {
          id: 1,
          name: "Survival",
          slug: "survival",
          language: "eng",
          games_count: 8231,
          image_background:
            "https://media.rawg.io/media/games/476/476178ef18ab0534771d099f51cdc694.jpg",
        },
        {
          id: 150,
          name: "Third-Person Shooter",
          slug: "third-person-shooter",
          language: "eng",
          games_count: 3266,
          image_background:
            "https://media.rawg.io/media/games/ebd/ebdbb7eb52bd58b0e7fa4538d9757b60.jpg",
        },
        {
          id: 41,
          name: "Dark",
          slug: "dark",
          language: "eng",
          games_count: 15192,
          image_background:
            "https://media.rawg.io/media/games/490/49016e06ae2103881ff6373248843069.jpg",
        },
        {
          id: 336,
          name: "controller support",
          slug: "controller-support",
          language: "eng",
          games_count: 293,
          image_background:
            "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
        },
        {
          id: 17,
          name: "Survival Horror",
          slug: "survival-horror",
          language: "eng",
          games_count: 8168,
          image_background:
            "https://media.rawg.io/media/games/ebd/ebdbb7eb52bd58b0e7fa4538d9757b60.jpg",
        },
        {
          id: 117,
          name: "Mystery",
          slug: "mystery",
          language: "eng",
          games_count: 12984,
          image_background:
            "https://media.rawg.io/media/games/a9a/a9a4e45ad8e653df2295e8410b7e96fd.jpg",
        },
        {
          id: 110,
          name: "Cinematic",
          slug: "cinematic",
          language: "eng",
          games_count: 1881,
          image_background:
            "https://media.rawg.io/media/games/9aa/9aa42d16d425fa6f179fc9dc2f763647.jpg",
        },
        {
          id: 232,
          name: "Episodic",
          slug: "episodic",
          language: "eng",
          games_count: 459,
          image_background:
            "https://media.rawg.io/media/games/a28/a289e23b4d4d84f26ab59125e3be4483.jpg",
        },
        {
          id: 200,
          name: "Narration",
          slug: "narration",
          language: "eng",
          games_count: 1896,
          image_background:
            "https://media.rawg.io/media/games/37a/37a9536e92cf8fe3b60045aa75dbd41f.jpg",
        },
        {
          id: 279,
          name: "Supernatural",
          slug: "supernatural",
          language: "eng",
          games_count: 1978,
          image_background:
            "https://media.rawg.io/media/games/a74/a7462cb98ad1ed06f646afd0a5a1454d.jpg",
        },
        {
          id: 183,
          name: "Thriller",
          slug: "thriller",
          language: "eng",
          games_count: 2325,
          image_background:
            "https://media.rawg.io/media/games/e77/e77ceadde635a080738bcde211687776.jpg",
        },
        {
          id: 7629,
          name: "enviroment",
          slug: "enviroment",
          language: "eng",
          games_count: 16,
          image_background:
            "https://media.rawg.io/media/screenshots/363/363eac26816e5f26a574415efebbd582.jpg",
        },
        {
          id: 36002,
          name: "psycholoical-horror",
          slug: "psycholoical-horror",
          language: "eng",
          games_count: 4,
          image_background:
            "https://media.rawg.io/media/games/5c0/5c0dd63002cb23f804aab327d40ef119.jpg",
        },
      ],
      esrb_rating: { id: 3, name: "Teen", slug: "teen" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/5c0/5c0dd63002cb23f804aab327d40ef119.jpg",
        },
        {
          id: 182489,
          image:
            "https://media.rawg.io/media/screenshots/8d7/8d7d24df1418efdaba45128e2c855f62.jpg",
        },
        {
          id: 182490,
          image:
            "https://media.rawg.io/media/screenshots/5ac/5ac6dd243c0ed41fb1a0b5734bff55f1.jpg",
        },
        {
          id: 182491,
          image:
            "https://media.rawg.io/media/screenshots/ad2/ad2e4f73ac71839178ea01ab66d8bf6c.jpg",
        },
        {
          id: 182492,
          image:
            "https://media.rawg.io/media/screenshots/b28/b28fd421d570931d83b27213538689df.jpg",
        },
        {
          id: 182493,
          image:
            "https://media.rawg.io/media/screenshots/d1e/d1e49ff4902b1bc964262ffc2e08043d.jpg",
        },
        {
          id: 182494,
          image:
            "https://media.rawg.io/media/screenshots/02f/02f6cb8534d5434e94559cdcd35aeef7.jpg",
        },
      ],
    },
    {
      id: 13668,
      slug: "amnesia-the-dark-descent",
      name: "Amnesia: The Dark Descent",
      released: "2010-09-08",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/b54/b54598d1d5cc31899f4f0a7e3122a7b0.jpg",
      rating: 3.65,
      rating_top: 4,
      ratings: [
        { id: 4, title: "recommended", count: 582, percent: 48.34 },
        { id: 5, title: "exceptional", count: 259, percent: 21.51 },
        { id: 3, title: "meh", count: 203, percent: 16.86 },
        { id: 1, title: "skip", count: 160, percent: 13.29 },
      ],
      ratings_count: 1185,
      reviews_text_count: 10,
      added: 9722,
      added_by_status: {
        yet: 744,
        owned: 7599,
        beaten: 741,
        toplay: 137,
        dropped: 468,
        playing: 33,
      },
      metacritic: 85,
      playtime: 2,
      suggestions_count: 438,
      updated: "2024-02-13T06:41:54",
      user_game: null,
      reviews_count: 1204,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 6,
            name: "Linux",
            slug: "linux",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 76454,
            image_background:
              "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
          },
          released_at: "2010-09-08",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 12.04 LTS, Mint 13 LTS, Fedora 16, fully updated<br>\t</li><li><strong>Processor:</strong> 2.0Ghz - Low budget CPUs such as Celeron or Duron needs to be at about twice the CPU speed. <br>\t</li><li><strong>Memory:</strong> 2 GB<br>\t</li><li><strong>Graphics:</strong> Radeon X1000/GeForce 6 - Integrated graphics and low budget cards might not work. <br>\t</li><li><strong>Hard Drive:</strong> 3 GB</li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 5,
            name: "macOS",
            slug: "macos",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 103400,
            image_background:
              "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
          },
          released_at: "2010-09-08",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t<ul class="bb_ul"><li><strong>OS:</strong> Mac OS X 10.5.8 or newer<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Processor:</strong> 2.0Ghz<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Memory:</strong> 1 GB RAM<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Graphics:</strong> Radeon X1000/GeForce 6 (Integrated Intel Graphics not supported)<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Hard Drive:</strong> 2GB space free<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2010-09-08",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows XP/Vista/7<br>\t\t\t\t\t\t\t\t</li><li><strong>Processor:</strong>  2.0Ghz - Low budget CPUs such as Celeron or Duron needs to be at about twice the CPU speed<br>\t\t\t\t\t\t\t\t</li><li><strong>Memory:</strong> 2 GB<br>\t\t\t\t\t\t\t\t</li><li><strong>Hard Drive:</strong> 3GB<br>\t\t\t\t\t\t\t\t</li><li><strong>Graphics:</strong> Radeon X1000/GF 6 - Integrated graphics and very low budget cards might not work.<br>\t\t\t\t\t\t\t</li></ul>',
          },
          requirements_ru: {
            minimum:
              "Pentium 4/Athlon XP 1.5 ГГц,1 Гб памяти,3D-ускоритель со 128 Мб памяти,2 Гб на винчестере",
            recommended:
              "Pentium 4/Athlon XP 2 ГГц,2 Гб памяти,3D-ускоритель с 256 Мб памяти,2 Гб на винчестере",
          },
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
        { platform: { id: 6, name: "Linux", slug: "linux" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
        {
          id: 3,
          name: "Adventure",
          slug: "adventure",
          games_count: 137936,
          image_background:
            "https://media.rawg.io/media/games/0af/0af85e8edddfa55368e47c539914a220.jpg",
        },
        {
          id: 51,
          name: "Indie",
          slug: "indie",
          games_count: 62257,
          image_background:
            "https://media.rawg.io/media/games/9dd/9ddabb34840ea9227556670606cf8ea3.jpg",
        },
      ],
      stores: [
        {
          id: 423652,
          store: {
            id: 11,
            name: "Epic Games",
            slug: "epic-games",
            domain: "epicgames.com",
            games_count: 1319,
            image_background:
              "https://media.rawg.io/media/games/4be/4be6a6ad0364751a96229c56bf69be59.jpg",
          },
        },
        {
          id: 15036,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
        {
          id: 25640,
          store: {
            id: 5,
            name: "GOG",
            slug: "gog",
            domain: "gog.com",
            games_count: 5817,
            image_background:
              "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 40836,
          name: "Full controller support",
          slug: "full-controller-support",
          language: "eng",
          games_count: 16746,
          image_background:
            "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 42,
          name: "Great Soundtrack",
          slug: "great-soundtrack",
          language: "eng",
          games_count: 3386,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 118,
          name: "Story Rich",
          slug: "story-rich",
          language: "eng",
          games_count: 20561,
          image_background:
            "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
        },
        {
          id: 8,
          name: "First-Person",
          slug: "first-person",
          language: "eng",
          games_count: 30626,
          image_background:
            "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
        },
        {
          id: 16,
          name: "Horror",
          slug: "horror",
          language: "eng",
          games_count: 43534,
          image_background:
            "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
        },
        {
          id: 6,
          name: "Exploration",
          slug: "exploration",
          language: "eng",
          games_count: 21834,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
        {
          id: 193,
          name: "Classic",
          slug: "classic",
          language: "eng",
          games_count: 1783,
          image_background:
            "https://media.rawg.io/media/games/960/960b601d9541cec776c5fa42a00bf6c4.jpg",
        },
        {
          id: 1,
          name: "Survival",
          slug: "survival",
          language: "eng",
          games_count: 8231,
          image_background:
            "https://media.rawg.io/media/games/476/476178ef18ab0534771d099f51cdc694.jpg",
        },
        {
          id: 15,
          name: "Stealth",
          slug: "stealth",
          language: "eng",
          games_count: 6100,
          image_background:
            "https://media.rawg.io/media/games/b7d/b7d3f1715fa8381a4e780173a197a615.jpg",
        },
        {
          id: 41,
          name: "Dark",
          slug: "dark",
          language: "eng",
          games_count: 15192,
          image_background:
            "https://media.rawg.io/media/games/490/49016e06ae2103881ff6373248843069.jpg",
        },
        {
          id: 62,
          name: "Moddable",
          slug: "moddable",
          language: "eng",
          games_count: 880,
          image_background:
            "https://media.rawg.io/media/games/c22/c22d804ac753c72f2617b3708a625dec.jpg",
        },
        {
          id: 17,
          name: "Survival Horror",
          slug: "survival-horror",
          language: "eng",
          games_count: 8168,
          image_background:
            "https://media.rawg.io/media/games/ebd/ebdbb7eb52bd58b0e7fa4538d9757b60.jpg",
        },
        {
          id: 40833,
          name: "Captions available",
          slug: "captions-available",
          language: "eng",
          games_count: 1311,
          image_background:
            "https://media.rawg.io/media/games/7a2/7a2500ee8b2c0e1ff268bb4479463dea.jpg",
        },
        {
          id: 40834,
          name: "Commentary available",
          slug: "commentary-available",
          language: "eng",
          games_count: 266,
          image_background:
            "https://media.rawg.io/media/games/594/5949baae74fe9e399adbce0c44e28783.jpg",
        },
        {
          id: 204,
          name: "Gothic",
          slug: "gothic",
          language: "eng",
          games_count: 856,
          image_background:
            "https://media.rawg.io/media/games/fd9/fd92f105dcd6491bc5d61135033d1f19.jpg",
        },
        {
          id: 47,
          name: "Lovecraftian",
          slug: "lovecraftian",
          language: "eng",
          games_count: 593,
          image_background:
            "https://media.rawg.io/media/screenshots/d2f/d2f5f87a6e6df6b988dac391605f0b60.jpg",
        },
      ],
      esrb_rating: { id: 4, name: "Mature", slug: "mature" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/b54/b54598d1d5cc31899f4f0a7e3122a7b0.jpg",
        },
        {
          id: 117208,
          image:
            "https://media.rawg.io/media/screenshots/32b/32bde7545dff888358a7ce620c7b3063.jpg",
        },
        {
          id: 117209,
          image:
            "https://media.rawg.io/media/screenshots/f59/f597e6857aab3b1ba098d713524d1690.jpg",
        },
        {
          id: 117210,
          image:
            "https://media.rawg.io/media/screenshots/5fc/5fce282fa41e7d73d0e8b0c35da74391.jpg",
        },
        {
          id: 117211,
          image:
            "https://media.rawg.io/media/screenshots/8f8/8f88209c1fdc529c3af746c08a44ba6c.jpg",
        },
        {
          id: 117212,
          image:
            "https://media.rawg.io/media/screenshots/ff7/ff776252fbd6de18cfd197b57d6e6aac.jpg",
        },
        {
          id: 117213,
          image:
            "https://media.rawg.io/media/screenshots/42d/42d3c11b04e589565e5a9d2feae5023c.jpg",
        },
      ],
    },
    {
      id: 3696,
      slug: "wolfenstein-the-new-order",
      name: "Wolfenstein: The New Order",
      released: "2014-05-19",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/c80/c80bcf321da44d69b18a06c04d942662.jpg",
      rating: 4.18,
      rating_top: 4,
      ratings: [
        { id: 4, title: "recommended", count: 1321, percent: 54.72 },
        { id: 5, title: "exceptional", count: 822, percent: 34.05 },
        { id: 3, title: "meh", count: 214, percent: 8.86 },
        { id: 1, title: "skip", count: 57, percent: 2.36 },
      ],
      ratings_count: 2396,
      reviews_text_count: 10,
      added: 9668,
      added_by_status: {
        yet: 471,
        owned: 6013,
        beaten: 2437,
        toplay: 303,
        dropped: 362,
        playing: 82,
      },
      metacritic: 81,
      playtime: 12,
      suggestions_count: 673,
      updated: "2024-03-20T11:28:13",
      user_game: null,
      reviews_count: 2414,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 1,
            name: "Xbox One",
            slug: "xbox-one",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5590,
            image_background:
              "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
          },
          released_at: "2014-05-19",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2014-05-19",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> 64-bit Windows 7/Windows 8<br></li><li><strong>Processor:</strong> Intel Core i7 or equivalent AMD<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> GeForce 460, ATI Radeon HD 6850<br></li><li><strong>Storage:</strong> 50 GB available space</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>Additional Notes:</strong> AMD Radeon users: Please install AMD Catalyst™ 14.4</li></ul>',
          },
          requirements_ru: {
            minimum:
              "Win 7 64,Core 2 Duo 2.66GHz/Phenom II X2 545,GeForce GTS 250/Radeon HD 6670,3 GB RAM,50 GB HDD",
            recommended:
              "Win 7 64,Core i7 2.80GHz/Phenom II X6 1090T,GeForce GTX 460/Radeon HD 6850,4 GB RAM,50 GB HDD",
          },
        },
        {
          platform: {
            id: 14,
            name: "Xbox 360",
            slug: "xbox360",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 2796,
            image_background:
              "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
          },
          released_at: "2014-05-19",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 16,
            name: "PlayStation 3",
            slug: "playstation3",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 3168,
            image_background:
              "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
          },
          released_at: "2014-05-19",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 18,
            name: "PlayStation 4",
            slug: "playstation4",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 6767,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
          released_at: "2014-05-19",
          requirements_en: null,
          requirements_ru: null,
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
        { platform: { id: 3, name: "Xbox", slug: "xbox" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
        {
          id: 2,
          name: "Shooter",
          slug: "shooter",
          games_count: 59459,
          image_background:
            "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
        },
      ],
      stores: [
        {
          id: 403368,
          store: {
            id: 5,
            name: "GOG",
            slug: "gog",
            domain: "gog.com",
            games_count: 5817,
            image_background:
              "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
          },
        },
        {
          id: 7879,
          store: {
            id: 2,
            name: "Xbox Store",
            slug: "xbox-store",
            domain: "microsoft.com",
            games_count: 4813,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
        },
        {
          id: 17137,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
        {
          id: 3999,
          store: {
            id: 3,
            name: "PlayStation Store",
            slug: "playstation-store",
            domain: "store.playstation.com",
            games_count: 7900,
            image_background:
              "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
          },
        },
        {
          id: 34545,
          store: {
            id: 7,
            name: "Xbox 360 Store",
            slug: "xbox360",
            domain: "marketplace.xbox.com",
            games_count: 1912,
            image_background:
              "https://media.rawg.io/media/games/d1a/d1a2e99ade53494c6330a0ed945fe823.jpg",
          },
        },
        {
          id: 660388,
          store: {
            id: 11,
            name: "Epic Games",
            slug: "epic-games",
            domain: "epicgames.com",
            games_count: 1319,
            image_background:
              "https://media.rawg.io/media/games/4be/4be6a6ad0364751a96229c56bf69be59.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 40847,
          name: "Steam Achievements",
          slug: "steam-achievements",
          language: "eng",
          games_count: 35702,
          image_background:
            "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
        },
        {
          id: 7,
          name: "Multiplayer",
          slug: "multiplayer",
          language: "eng",
          games_count: 37130,
          image_background:
            "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
        },
        {
          id: 13,
          name: "Atmospheric",
          slug: "atmospheric",
          language: "eng",
          games_count: 32044,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 42,
          name: "Great Soundtrack",
          slug: "great-soundtrack",
          language: "eng",
          games_count: 3386,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 118,
          name: "Story Rich",
          slug: "story-rich",
          language: "eng",
          games_count: 20561,
          image_background:
            "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
        },
        {
          id: 8,
          name: "First-Person",
          slug: "first-person",
          language: "eng",
          games_count: 30626,
          image_background:
            "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
        },
        {
          id: 32,
          name: "Sci-fi",
          slug: "sci-fi",
          language: "eng",
          games_count: 18535,
          image_background:
            "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
        },
        {
          id: 40845,
          name: "Partial Controller Support",
          slug: "partial-controller-support",
          language: "eng",
          games_count: 11229,
          image_background:
            "https://media.rawg.io/media/games/62c/62c7c8b28a27b83680b22fb9d33fc619.jpg",
        },
        {
          id: 16,
          name: "Horror",
          slug: "horror",
          language: "eng",
          games_count: 43534,
          image_background:
            "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
        },
        {
          id: 30,
          name: "FPS",
          slug: "fps",
          language: "eng",
          games_count: 12893,
          image_background:
            "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
        },
        {
          id: 26,
          name: "Gore",
          slug: "gore",
          language: "eng",
          games_count: 5515,
          image_background:
            "https://media.rawg.io/media/games/7f6/7f6cd70ba2ad57053b4847c13569f2d8.jpg",
        },
        {
          id: 40850,
          name: "Steam Leaderboards",
          slug: "steam-leaderboards",
          language: "eng",
          games_count: 6539,
          image_background:
            "https://media.rawg.io/media/games/59a/59a3ebcba3d08c51532c6ca877aff256.jpg",
        },
        {
          id: 15,
          name: "Stealth",
          slug: "stealth",
          language: "eng",
          games_count: 6100,
          image_background:
            "https://media.rawg.io/media/games/b7d/b7d3f1715fa8381a4e780173a197a615.jpg",
        },
        {
          id: 70,
          name: "War",
          slug: "war",
          language: "eng",
          games_count: 9104,
          image_background:
            "https://media.rawg.io/media/games/2fd/2fd1b58116b10cc1f4442bee5593ca7c.jpg",
        },
        {
          id: 89,
          name: "Historical",
          slug: "historical",
          language: "eng",
          games_count: 3005,
          image_background:
            "https://media.rawg.io/media/games/d07/d0790809a13027251b6d0f4dc7538c58.jpg",
        },
        {
          id: 119,
          name: "Dystopian",
          slug: "dystopian",
          language: "eng",
          games_count: 2001,
          image_background:
            "https://media.rawg.io/media/games/a0e/a0ef08621301a1eab5e04fa5c96978fa.jpeg",
        },
        {
          id: 197,
          name: "Robots",
          slug: "robots",
          language: "eng",
          games_count: 8053,
          image_background:
            "https://media.rawg.io/media/screenshots/e29/e29ce2369152515e9ae20c6e217cbe81.jpg",
        },
        {
          id: 105,
          name: "World War II",
          slug: "world-war-ii",
          language: "eng",
          games_count: 925,
          image_background:
            "https://media.rawg.io/media/games/fe6/fe68940746007261e34da4d9cce81423.jpg",
        },
        {
          id: 208,
          name: "Alternate History",
          slug: "alternate-history",
          language: "eng",
          games_count: 1675,
          image_background:
            "https://media.rawg.io/media/games/f14/f1422eacab98c5f85a5123da4e9d9e89.jpg",
        },
      ],
      esrb_rating: { id: 4, name: "Mature", slug: "mature" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/c80/c80bcf321da44d69b18a06c04d942662.jpg",
        },
        {
          id: 32371,
          image:
            "https://media.rawg.io/media/screenshots/23a/23af906d70f57be798bbd83da986c6db.jpg",
        },
        {
          id: 32372,
          image:
            "https://media.rawg.io/media/screenshots/b71/b716cf4d51be00d9561df3d1588383c4.jpg",
        },
        {
          id: 32373,
          image:
            "https://media.rawg.io/media/screenshots/439/4395d5364559ca15ac8e1becb100daea.jpg",
        },
        {
          id: 32374,
          image:
            "https://media.rawg.io/media/screenshots/0de/0de321f22cd1f5a7d0fb6b471f63c2d8.jpg",
        },
        {
          id: 32375,
          image:
            "https://media.rawg.io/media/screenshots/900/900e962d31b5ace3fb66bfd388d352cf.jpg",
        },
        {
          id: 32376,
          image:
            "https://media.rawg.io/media/screenshots/a1d/a1d6bd9f3617e0787da4b0f8389fe1f0.jpg",
        },
      ],
    },
    {
      id: 4386,
      slug: "saints-row-the-third",
      name: "Saints Row: The Third",
      released: "2011-11-15",
      tba: false,
      background_image:
        "https://media.rawg.io/media/games/d69/d69810315bd7e226ea2d21f9156af629.jpg",
      rating: 3.95,
      rating_top: 4,
      ratings: [
        { id: 4, title: "recommended", count: 1121, percent: 56.25 },
        { id: 5, title: "exceptional", count: 473, percent: 23.73 },
        { id: 3, title: "meh", count: 308, percent: 15.45 },
        { id: 1, title: "skip", count: 91, percent: 4.57 },
      ],
      ratings_count: 1978,
      reviews_text_count: 9,
      added: 9649,
      added_by_status: {
        yet: 287,
        owned: 6766,
        beaten: 1992,
        toplay: 113,
        dropped: 460,
        playing: 31,
      },
      metacritic: 84,
      playtime: 14,
      suggestions_count: 662,
      updated: "2024-03-20T07:31:09",
      user_game: null,
      reviews_count: 1993,
      saturated_color: "0f0f0f",
      dominant_color: "0f0f0f",
      platforms: [
        {
          platform: {
            id: 6,
            name: "Linux",
            slug: "linux",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 76454,
            image_background:
              "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
          },
          released_at: "2011-11-15",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 14.04.3 LTS, Ubuntu 15.04, Linux Mint 17.1, Steam OS Brewmaster<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>CPU:</strong> Intel Core i5-4570T (2.9 GHz) or equivalent\tor AMD Athlon II X2 270 (3.8 GHz) or equivalent\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t<br></li><li><strong>GPU:</strong>GeForce GT 630 (1024 MB) or equivalent or Radeon HD 6850 (1024 MB) or equivalent\t<br></li><li><strong>Recommended Graphics Drivers:</strong> Nvidia: 352, 355 or 358 series driver / AMD: AMD Catalyst (fglrx) 15.9 or better. MESA drivers and Intel Graphics are not currently supported.  \t\t<br></li><li><strong>RAM:</strong> 4096 MB<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Disk Space:</strong> Around 7 GB<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li></ul>',
            recommended:
              '<strong>Recommended:</strong><br><br><ul class="bb_ul"><li><strong>OS:</strong>Ubuntu 14.04.3 LTS, Ubuntu 15.04, Linux Mint 17.1, Steam OS Brewmaster<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>CPU:</strong> Intel Core i5-4670K (3.4 GHz) or equivalent or AMD Athlon X4 860K (3.7 GHz) or equivalent\t\t\t\t\t\t\t\t\t\t\t<br></li><li><strong>GPU:</strong>GeForce GTX 760 (2048 MB) or equivalent or Radeon HD 7950 (2048 MB) or equivalent\t\t<br></li><li><strong>Recommended Graphics Drivers:</strong> Nvidia: 352, 355 or 358 series driver / AMD: AMD Catalyst (fglrx) 15.9 or better. MESA drivers and Intel Graphics are not currently supported.  \t<br></li><li><strong>RAM:</strong>8192 MB<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Disk Space:</strong>Around 7 GB<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li></ul>',
          },
          requirements_ru: null,
        },
        {
          platform: {
            id: 4,
            name: "PC",
            slug: "pc",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 525904,
            image_background:
              "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
          },
          released_at: "2011-11-15",
          requirements_en: {
            minimum:
              '<strong>Minimum:</strong><br><br><ul class="bb_ul"><li><strong>OS:</strong> Windows® XP<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Processor:</strong> 2GHz Dual Core Processor (Intel® Core 2 Duo or AMD Athlon™ X2) or higher<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Memory:</strong> 2GB System RAM or more<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Graphics:</strong> 320MB Video RAM GPU w/ Shader Model 3.0 support.  NVIDIA® GeForce® 8800 series or better.  ATI Radeon™ HD3800 series or better<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>DirectX®:</strong> 9.0c<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Hard Drive:</strong> 10GB<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Sound:</strong> 100% DirectX® 9.0C compliant sound card or equivalent onboard sound<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Co-Op Play:</strong>NVIDIA® GeForce® 8800 cards require 640MB of Video RAM.  ATI Radeon™ HD3800 cards require 1GB of Video RAM<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li></ul><strong></strong>',
            recommended:
              '<strong>Recommended:</strong><br><br><ul class="bb_ul"><li><strong>OS:</strong> Microsoft® Windows® 7<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Processor:</strong> Any Quad Core Processor (Intel® Core i5 or AMD Phenom™ II X4) or 3.0+ Dual Core CPU<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Memory:</strong> 4GB System RAM or more<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Graphics:</strong> 1GB Video RAM GPU w/ Shader Model 4.0 support.  NVIDIA® GeForce® GTX 400 series or better.  ATI Radeon™ HD5000 series or better <br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>DirectX®:</strong> DirectX® 11<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Hard Drive:</strong> 10 GB<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Sound:</strong> 100% DirectX® 9.0C compliant sound card or equivalent onboard sound<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li></ul>',
          },
          requirements_ru: {
            minimum:
              "Core 2 Duo/Athlon X2 2 ГГц,2 Гб памяти,GeForce 8800/Radeon HD 3800,10 Гб на винчестере",
            recommended:
              "Core 2 Duo/Phenom II X4 3 ГГц,2 Гб памяти,GeForce GTX 460/Radeon HD 5850,10 Гб на винчестере",
          },
        },
        {
          platform: {
            id: 1,
            name: "Xbox One",
            slug: "xbox-one",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 5590,
            image_background:
              "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
          },
          released_at: "2011-11-15",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 14,
            name: "Xbox 360",
            slug: "xbox360",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 2796,
            image_background:
              "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
          },
          released_at: "2011-11-15",
          requirements_en: null,
          requirements_ru: null,
        },
        {
          platform: {
            id: 16,
            name: "PlayStation 3",
            slug: "playstation3",
            image: null,
            year_end: null,
            year_start: null,
            games_count: 3168,
            image_background:
              "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
          },
          released_at: "2011-11-15",
          requirements_en: null,
          requirements_ru: null,
        },
      ],
      parent_platforms: [
        { platform: { id: 1, name: "PC", slug: "pc" } },
        { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
        { platform: { id: 3, name: "Xbox", slug: "xbox" } },
        { platform: { id: 6, name: "Linux", slug: "linux" } },
      ],
      genres: [
        {
          id: 4,
          name: "Action",
          slug: "action",
          games_count: 178358,
          image_background:
            "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
        },
        {
          id: 3,
          name: "Adventure",
          slug: "adventure",
          games_count: 137936,
          image_background:
            "https://media.rawg.io/media/games/0af/0af85e8edddfa55368e47c539914a220.jpg",
        },
      ],
      stores: [
        {
          id: 467012,
          store: {
            id: 2,
            name: "Xbox Store",
            slug: "xbox-store",
            domain: "microsoft.com",
            games_count: 4813,
            image_background:
              "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
          },
        },
        {
          id: 4714,
          store: {
            id: 3,
            name: "PlayStation Store",
            slug: "playstation-store",
            domain: "store.playstation.com",
            games_count: 7900,
            image_background:
              "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
          },
        },
        {
          id: 11270,
          store: {
            id: 1,
            name: "Steam",
            slug: "steam",
            domain: "store.steampowered.com",
            games_count: 89063,
            image_background:
              "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
          },
        },
        {
          id: 33958,
          store: {
            id: 7,
            name: "Xbox 360 Store",
            slug: "xbox360",
            domain: "marketplace.xbox.com",
            games_count: 1912,
            image_background:
              "https://media.rawg.io/media/games/d1a/d1a2e99ade53494c6330a0ed945fe823.jpg",
          },
        },
      ],
      clip: null,
      tags: [
        {
          id: 31,
          name: "Singleplayer",
          slug: "singleplayer",
          language: "eng",
          games_count: 218199,
          image_background:
            "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
        },
        {
          id: 7,
          name: "Multiplayer",
          slug: "multiplayer",
          language: "eng",
          games_count: 37130,
          image_background:
            "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
        },
        {
          id: 42,
          name: "Great Soundtrack",
          slug: "great-soundtrack",
          language: "eng",
          games_count: 3386,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        {
          id: 24,
          name: "RPG",
          slug: "rpg",
          language: "eng",
          games_count: 19865,
          image_background:
            "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
        },
        {
          id: 18,
          name: "Co-op",
          slug: "co-op",
          language: "eng",
          games_count: 11072,
          image_background:
            "https://media.rawg.io/media/games/511/5118aff5091cb3efec399c808f8c598f.jpg",
        },
        {
          id: 36,
          name: "Open World",
          slug: "open-world",
          language: "eng",
          games_count: 7102,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        {
          id: 411,
          name: "cooperative",
          slug: "cooperative",
          language: "eng",
          games_count: 4703,
          image_background:
            "https://media.rawg.io/media/games/73e/73eecb8909e0c39fb246f457b5d6cbbe.jpg",
        },
        {
          id: 149,
          name: "Third Person",
          slug: "third-person",
          language: "eng",
          games_count: 10861,
          image_background:
            "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
        },
        {
          id: 40845,
          name: "Partial Controller Support",
          slug: "partial-controller-support",
          language: "eng",
          games_count: 11229,
          image_background:
            "https://media.rawg.io/media/games/62c/62c7c8b28a27b83680b22fb9d33fc619.jpg",
        },
        {
          id: 9,
          name: "Online Co-Op",
          slug: "online-co-op",
          language: "eng",
          games_count: 5203,
          image_background:
            "https://media.rawg.io/media/games/48c/48cb04ca483be865e3a83119c94e6097.jpg",
        },
        {
          id: 4,
          name: "Funny",
          slug: "funny",
          language: "eng",
          games_count: 24154,
          image_background:
            "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
        },
        {
          id: 37,
          name: "Sandbox",
          slug: "sandbox",
          language: "eng",
          games_count: 6607,
          image_background:
            "https://media.rawg.io/media/games/7cf/7cfc9220b401b7a300e409e539c9afd5.jpg",
        },
        {
          id: 189,
          name: "Female Protagonist",
          slug: "female-protagonist",
          language: "eng",
          games_count: 11837,
          image_background:
            "https://media.rawg.io/media/games/530/5302dd22a190e664531236ca724e8726.jpg",
        },
        {
          id: 123,
          name: "Comedy",
          slug: "comedy",
          language: "eng",
          games_count: 11960,
          image_background:
            "https://media.rawg.io/media/games/e3d/e3ddc524c6292a435d01d97cc5f42ea7.jpg",
        },
        {
          id: 150,
          name: "Third-Person Shooter",
          slug: "third-person-shooter",
          language: "eng",
          games_count: 3266,
          image_background:
            "https://media.rawg.io/media/games/ebd/ebdbb7eb52bd58b0e7fa4538d9757b60.jpg",
        },
        {
          id: 63,
          name: "Zombies",
          slug: "zombies",
          language: "eng",
          games_count: 10103,
          image_background:
            "https://media.rawg.io/media/games/cee/cee577e2097a59b77193fe2bce94667d.jpg",
        },
        {
          id: 121,
          name: "Character Customization",
          slug: "character-customization",
          language: "eng",
          games_count: 4284,
          image_background:
            "https://media.rawg.io/media/games/33b/33b825c76382931df0fd8ecddf5caebe.jpg",
        },
        {
          id: 144,
          name: "Crime",
          slug: "crime",
          language: "eng",
          games_count: 2718,
          image_background:
            "https://media.rawg.io/media/games/5fa/5fae5fec3c943179e09da67a4427d68f.jpg",
        },
        {
          id: 153,
          name: "Satire",
          slug: "satire",
          language: "eng",
          games_count: 1264,
          image_background:
            "https://media.rawg.io/media/screenshots/17a/17a94bcc1b994fa4e24729371d6988b6.jpg",
        },
      ],
      esrb_rating: { id: 4, name: "Mature", slug: "mature" },
      short_screenshots: [
        {
          id: -1,
          image:
            "https://media.rawg.io/media/games/d69/d69810315bd7e226ea2d21f9156af629.jpg",
        },
        {
          id: 79408,
          image:
            "https://media.rawg.io/media/screenshots/ac0/ac00b015e9c51f52a24631c44676f81b.jpg",
        },
        {
          id: 79409,
          image:
            "https://media.rawg.io/media/screenshots/4e8/4e8cb89dc1ac9b3a9b68ad34f1dbc744.jpg",
        },
        {
          id: 79410,
          image:
            "https://media.rawg.io/media/screenshots/cc2/cc205478d728ad862c0e9da48df5db05.jpg",
        },
        {
          id: 79411,
          image:
            "https://media.rawg.io/media/screenshots/6a4/6a4976a45c96960e72d05362375908e4.jpg",
        },
        {
          id: 79412,
          image:
            "https://media.rawg.io/media/screenshots/e26/e26e0f29195d3682e6e9e9f5a74f6250.jpg",
        },
        {
          id: 79413,
          image:
            "https://media.rawg.io/media/screenshots/215/215fbd506ac8c1a24cb2bf9b9a4d5b92.jpg",
        },
      ],
    },
  ],
};

const exampleData = [
  {
    id: 19709,
    slug: "half-life-2-episode-two",
    name: "Half-Life 2: Episode Two",
    released: "2007-10-10",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/198/1988a337305e008b41d7f536ce9b73f6.jpg",
    rating: 4.45,
    rating_top: 5,
    ratings: [
      { id: 5, title: "exceptional", count: 1144, percent: 59.4 },
      { id: 4, title: "recommended", count: 637, percent: 33.07 },
      { id: 3, title: "meh", count: 75, percent: 3.89 },
      { id: 1, title: "skip", count: 70, percent: 3.63 },
    ],
    ratings_count: 1918,
    reviews_text_count: 4,
    added: 10587,
    added_by_status: {
      yet: 592,
      owned: 7524,
      beaten: 2208,
      toplay: 105,
      dropped: 138,
      playing: 20,
    },
    metacritic: 90,
    playtime: 5,
    suggestions_count: 298,
    updated: "2024-03-17T16:15:31",
    user_game: null,
    reviews_count: 1926,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 21,
          name: "Android",
          slug: "android",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 52330,
          image_background:
            "https://media.rawg.io/media/games/530/5302dd22a190e664531236ca724e8726.jpg",
        },
        released_at: "2007-10-10",
        requirements_en: { minimum: "4.4 and up" },
        requirements_ru: null,
      },
      {
        platform: {
          id: 6,
          name: "Linux",
          slug: "linux",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 76454,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        released_at: "2007-10-10",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 5,
          name: "macOS",
          slug: "macos",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 103400,
          image_background:
            "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
        },
        released_at: "2007-10-10",
        requirements_en: {
          minimum:
            "<strong>Minimum: </strong>OS X version Leopard 10.5.8, Snow Leopard 10.6.3, 1GB RAM, NVIDIA GeForce 8 or higher, ATI X1600 or higher, or Intel HD 3000 or higher Mouse, Keyboard, Internet Connection",
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2007-10-10",
        requirements_en: {
          minimum:
            '<p><strong>Minimum: </strong>1.7 GHz Processor, 512MB RAM, DirectX&reg; 8.1 level Graphics Card (Requires support for SSE), Windows&reg; 7 (32/64-bit)/Vista/XP, Mouse, Keyboard, Internet Connection</p><br>\t\t\t<p><strong>Recommended: </strong>Pentium 4 processor (3.0GHz, or better), 1GB RAM, DirectX&reg; 9 level Graphics Card, Windows&reg; 7 (32/64-bit)/Vista/XP, Mouse, Keyboard, Internet Connection</p><br>\t\t\t<br><img src="gfx/apps_extras/OB/nvidia.jpg">',
        },
        requirements_ru: {
          minimum:
            "Pentium 4/Athlon XP 1.7 ГГц,512 Мб памяти,3D-ускоритель со 128 Мб памяти,7.5 Гб на винчестере,Интернет-соединение 128 Кб/c",
          recommended:
            "Core 2 Duo/Athlon 64 3 ГГц,1 Гб памяти,3D-ускоритель с 512 Мб памяти,12 Гб на винчестере,Интернет-соединение 512 Кб/c",
        },
      },
      {
        platform: {
          id: 14,
          name: "Xbox 360",
          slug: "xbox360",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 2796,
          image_background:
            "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
        },
        released_at: "2007-10-10",
        requirements_en: null,
        requirements_ru: null,
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 3, name: "Xbox", slug: "xbox" } },
      { platform: { id: 8, name: "Android", slug: "android" } },
      { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
      { platform: { id: 6, name: "Linux", slug: "linux" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
      {
        id: 2,
        name: "Shooter",
        slug: "shooter",
        games_count: 59459,
        image_background:
          "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
      },
      {
        id: 7,
        name: "Puzzle",
        slug: "puzzle",
        games_count: 97242,
        image_background:
          "https://media.rawg.io/media/games/9f4/9f418898f5415668ca47b5f4ab1ecfeb.jpg",
      },
    ],
    stores: [
      {
        id: 22130,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
      {
        id: 218334,
        store: {
          id: 8,
          name: "Google Play",
          slug: "google-play",
          domain: "play.google.com",
          games_count: 17079,
          image_background:
            "https://media.rawg.io/media/games/13a/13a528ac9cf48bbb6be5d35fe029336d.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 40847,
        name: "Steam Achievements",
        slug: "steam-achievements",
        language: "eng",
        games_count: 35702,
        image_background:
          "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
      },
      {
        id: 7,
        name: "Multiplayer",
        slug: "multiplayer",
        language: "eng",
        games_count: 37130,
        image_background:
          "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
      },
      {
        id: 40849,
        name: "Steam Cloud",
        slug: "steam-cloud",
        language: "eng",
        games_count: 16769,
        image_background:
          "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 42,
        name: "Great Soundtrack",
        slug: "great-soundtrack",
        language: "eng",
        games_count: 3386,
        image_background:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 118,
        name: "Story Rich",
        slug: "story-rich",
        language: "eng",
        games_count: 20561,
        image_background:
          "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
      },
      {
        id: 8,
        name: "First-Person",
        slug: "first-person",
        language: "eng",
        games_count: 30626,
        image_background:
          "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
      },
      {
        id: 32,
        name: "Sci-fi",
        slug: "sci-fi",
        language: "eng",
        games_count: 18535,
        image_background:
          "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
      },
      {
        id: 40845,
        name: "Partial Controller Support",
        slug: "partial-controller-support",
        language: "eng",
        games_count: 11229,
        image_background:
          "https://media.rawg.io/media/games/62c/62c7c8b28a27b83680b22fb9d33fc619.jpg",
      },
      {
        id: 16,
        name: "Horror",
        slug: "horror",
        language: "eng",
        games_count: 43534,
        image_background:
          "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
      },
      {
        id: 30,
        name: "FPS",
        slug: "fps",
        language: "eng",
        games_count: 12893,
        image_background:
          "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
      },
      {
        id: 193,
        name: "Classic",
        slug: "classic",
        language: "eng",
        games_count: 1783,
        image_background:
          "https://media.rawg.io/media/games/960/960b601d9541cec776c5fa42a00bf6c4.jpg",
      },
      {
        id: 11669,
        name: "stats",
        slug: "stats",
        language: "eng",
        games_count: 4821,
        image_background:
          "https://media.rawg.io/media/games/f52/f5206d55f918edf8ee07803101106fa6.jpg",
      },
      {
        id: 63,
        name: "Zombies",
        slug: "zombies",
        language: "eng",
        games_count: 10103,
        image_background:
          "https://media.rawg.io/media/games/cee/cee577e2097a59b77193fe2bce94667d.jpg",
      },
      {
        id: 43,
        name: "Post-apocalyptic",
        slug: "post-apocalyptic",
        language: "eng",
        games_count: 3777,
        image_background:
          "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
      },
      {
        id: 40833,
        name: "Captions available",
        slug: "captions-available",
        language: "eng",
        games_count: 1311,
        image_background:
          "https://media.rawg.io/media/games/7a2/7a2500ee8b2c0e1ff268bb4479463dea.jpg",
      },
      {
        id: 172,
        name: "Aliens",
        slug: "aliens",
        language: "eng",
        games_count: 6591,
        image_background:
          "https://media.rawg.io/media/games/ae1/ae1518c3dc1e847344661905fd2a8d16.jpg",
      },
      {
        id: 119,
        name: "Dystopian",
        slug: "dystopian",
        language: "eng",
        games_count: 2001,
        image_background:
          "https://media.rawg.io/media/games/a0e/a0ef08621301a1eab5e04fa5c96978fa.jpeg",
      },
      {
        id: 40834,
        name: "Commentary available",
        slug: "commentary-available",
        language: "eng",
        games_count: 266,
        image_background:
          "https://media.rawg.io/media/games/594/5949baae74fe9e399adbce0c44e28783.jpg",
      },
      {
        id: 232,
        name: "Episodic",
        slug: "episodic",
        language: "eng",
        games_count: 459,
        image_background:
          "https://media.rawg.io/media/games/a28/a289e23b4d4d84f26ab59125e3be4483.jpg",
      },
      {
        id: 40839,
        name: "Includes Source SDK",
        slug: "includes-source-sdk",
        language: "eng",
        games_count: 62,
        image_background:
          "https://media.rawg.io/media/screenshots/ae8/ae873e0da5973a9896bdf5ebce1e9bf4.jpg",
      },
      {
        id: 319,
        name: "Silent Protagonist",
        slug: "silent-protagonist",
        language: "eng",
        games_count: 91,
        image_background:
          "https://media.rawg.io/media/games/198/1988a337305e008b41d7f536ce9b73f6.jpg",
      },
    ],
    esrb_rating: { id: 4, name: "Mature", slug: "mature" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/198/1988a337305e008b41d7f536ce9b73f6.jpg",
      },
      {
        id: 185826,
        image:
          "https://media.rawg.io/media/screenshots/04b/04b62b9115ccd64ebac1e8d813c69d08.jpg",
      },
      {
        id: 185827,
        image:
          "https://media.rawg.io/media/screenshots/278/278f1654ade9d116ca1bb42bb37d94ba.jpg",
      },
      {
        id: 185828,
        image:
          "https://media.rawg.io/media/screenshots/0f2/0f2b40d5aa1eb776fdedcdb4d11aca8e.jpg",
      },
      {
        id: 185829,
        image:
          "https://media.rawg.io/media/screenshots/895/895f1c364465b338e2d1b41b0b4eff67.jpg",
      },
      {
        id: 185830,
        image:
          "https://media.rawg.io/media/screenshots/016/01638fb0f73a6acde1714a107e078807.jpg",
      },
    ],
  },
  {
    id: 4427,
    slug: "bioshock-2",
    name: "BioShock 2",
    released: "2010-02-09",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/157/15742f2f67eacff546738e1ab5c19d20.jpg",
    rating: 4.07,
    rating_top: 4,
    ratings: [
      { id: 4, title: "recommended", count: 992, percent: 52.57 },
      { id: 5, title: "exceptional", count: 602, percent: 31.9 },
      { id: 3, title: "meh", count: 205, percent: 10.86 },
      { id: 1, title: "skip", count: 88, percent: 4.66 },
    ],
    ratings_count: 1873,
    reviews_text_count: 8,
    added: 10577,
    added_by_status: {
      yet: 767,
      owned: 7292,
      beaten: 1830,
      toplay: 257,
      dropped: 388,
      playing: 43,
    },
    metacritic: 88,
    playtime: 3,
    suggestions_count: 645,
    updated: "2024-03-20T02:21:07",
    user_game: null,
    reviews_count: 1887,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 18,
          name: "PlayStation 4",
          slug: "playstation4",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 6767,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
        released_at: "2010-02-09",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 1,
          name: "Xbox One",
          slug: "xbox-one",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5590,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        released_at: "2010-02-09",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 7,
          name: "Nintendo Switch",
          slug: "nintendo-switch",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5406,
          image_background:
            "https://media.rawg.io/media/games/5eb/5eb49eb2fa0738fdb5bacea557b1bc57.jpg",
        },
        released_at: "2010-02-09",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2010-02-09",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows XP, Vista, Windows 7<br></li><li><strong>Processor:</strong> AMD Athlon 64 Processor 3800+ 2.4Ghz or better, Intel Pentium 4 530 3.0Ghz Processor or better<br></li><li><strong>Memory:</strong> 2GB<br></li><li><strong>Graphics:</strong> NVIDIA 7800GT 256MB graphics card or better, ATI Radeon X1900 256MB graphics card or better<br></li><li><strong>DirectX®:</strong> DirectX 9.0c<br></li><li><strong>Hard Drive:</strong> 11GB<br></li><li><strong>Sound:</strong> 100% DirectX 9.0C compliant sound card or onboard sound</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>Processor:</strong> AMD Athlon 64 X2 5200+ Dual Core 2.60Ghz, Intel Core 2 Duo E6420 Dual Core 2.13Ghz<br></li><li><strong>Memory:</strong> 3GB<br></li><li><strong>Graphics:</strong> NVIDIA 8800GT 512MB graphics card or better, ATI Radeon HD4830 512MB graphics card or better<br></li><li><strong>Other Requirements:</strong> Initial installation requires one-time internet connection; software installations required including Microsoft Visual C++2008 Runtime Libraries, Microsoft DirectX.</li></ul>',
        },
        requirements_ru: {
          minimum:
            "Pentium 4 3 ГГц/Athlon 64 3800+,2 Гб памяти,GeForce 7800 GT/Radeon X1900,11 Гб на винчестере,интернет-соединение",
          recommended:
            "Core 2 Duo/Athlon X2 2.5 ГГц,3 Гб памяти,GeForce 8800/Radeon HD 2600,11 Гб на винчестере,интернет-соединение",
        },
      },
      {
        platform: {
          id: 5,
          name: "macOS",
          slug: "macos",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 103400,
          image_background:
            "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
        },
        released_at: "2010-02-09",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 14,
          name: "Xbox 360",
          slug: "xbox360",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 2796,
          image_background:
            "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
        },
        released_at: "2010-02-09",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 16,
          name: "PlayStation 3",
          slug: "playstation3",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 3168,
          image_background:
            "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
        },
        released_at: "2010-02-09",
        requirements_en: null,
        requirements_ru: null,
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
      { platform: { id: 3, name: "Xbox", slug: "xbox" } },
      { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
      { platform: { id: 7, name: "Nintendo", slug: "nintendo" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
      {
        id: 2,
        name: "Shooter",
        slug: "shooter",
        games_count: 59459,
        image_background:
          "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
      },
    ],
    stores: [
      {
        id: 79405,
        store: {
          id: 4,
          name: "App Store",
          slug: "apple-appstore",
          domain: "apps.apple.com",
          games_count: 75436,
          image_background:
            "https://media.rawg.io/media/games/f46/f466571d536f2e3ea9e815ad17177501.jpg",
        },
      },
      {
        id: 440408,
        store: {
          id: 2,
          name: "Xbox Store",
          slug: "xbox-store",
          domain: "microsoft.com",
          games_count: 4813,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
      },
      {
        id: 19732,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
      {
        id: 4755,
        store: {
          id: 3,
          name: "PlayStation Store",
          slug: "playstation-store",
          domain: "store.playstation.com",
          games_count: 7900,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
      },
      {
        id: 33604,
        store: {
          id: 7,
          name: "Xbox 360 Store",
          slug: "xbox360",
          domain: "marketplace.xbox.com",
          games_count: 1912,
          image_background:
            "https://media.rawg.io/media/games/d1a/d1a2e99ade53494c6330a0ed945fe823.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 40847,
        name: "Steam Achievements",
        slug: "steam-achievements",
        language: "eng",
        games_count: 35702,
        image_background:
          "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
      },
      {
        id: 7,
        name: "Multiplayer",
        slug: "multiplayer",
        language: "eng",
        games_count: 37130,
        image_background:
          "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
      },
      {
        id: 40836,
        name: "Full controller support",
        slug: "full-controller-support",
        language: "eng",
        games_count: 16746,
        image_background:
          "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 42,
        name: "Great Soundtrack",
        slug: "great-soundtrack",
        language: "eng",
        games_count: 3386,
        image_background:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 24,
        name: "RPG",
        slug: "rpg",
        language: "eng",
        games_count: 19865,
        image_background:
          "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
      },
      {
        id: 118,
        name: "Story Rich",
        slug: "story-rich",
        language: "eng",
        games_count: 20561,
        image_background:
          "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
      },
      {
        id: 8,
        name: "First-Person",
        slug: "first-person",
        language: "eng",
        games_count: 30626,
        image_background:
          "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
      },
      {
        id: 32,
        name: "Sci-fi",
        slug: "sci-fi",
        language: "eng",
        games_count: 18535,
        image_background:
          "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
      },
      {
        id: 16,
        name: "Horror",
        slug: "horror",
        language: "eng",
        games_count: 43534,
        image_background:
          "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
      },
      {
        id: 30,
        name: "FPS",
        slug: "fps",
        language: "eng",
        games_count: 12893,
        image_background:
          "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
      },
      {
        id: 97,
        name: "Action RPG",
        slug: "action-rpg",
        language: "eng",
        games_count: 6501,
        image_background:
          "https://media.rawg.io/media/games/157/15742f2f67eacff546738e1ab5c19d20.jpg",
      },
      {
        id: 41,
        name: "Dark",
        slug: "dark",
        language: "eng",
        games_count: 15192,
        image_background:
          "https://media.rawg.io/media/games/490/49016e06ae2103881ff6373248843069.jpg",
      },
      {
        id: 119,
        name: "Dystopian",
        slug: "dystopian",
        language: "eng",
        games_count: 2001,
        image_background:
          "https://media.rawg.io/media/games/a0e/a0ef08621301a1eab5e04fa5c96978fa.jpeg",
      },
      {
        id: 154,
        name: "Steampunk",
        slug: "steampunk",
        language: "eng",
        games_count: 1169,
        image_background:
          "https://media.rawg.io/media/games/4e6/4e6e8e7f50c237d76f38f3c885dae3d2.jpg",
      },
      {
        id: 305,
        name: "Linear",
        slug: "linear",
        language: "eng",
        games_count: 5442,
        image_background:
          "https://media.rawg.io/media/games/569/569ea25d2b56bd05c7fa309ddabe81ff.jpg",
      },
      {
        id: 250,
        name: "Underwater",
        slug: "underwater",
        language: "eng",
        games_count: 2048,
        image_background:
          "https://media.rawg.io/media/games/386/3869a2540b5fd39776fd640686610d05.jpg",
      },
      {
        id: 281,
        name: "Philisophical",
        slug: "philisophical",
        language: "eng",
        games_count: 39,
        image_background:
          "https://media.rawg.io/media/screenshots/573/573f6403cf6de862ca5b886db8cf449a.jpg",
      },
    ],
    esrb_rating: null,
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/157/15742f2f67eacff546738e1ab5c19d20.jpg",
      },
      {
        id: 162566,
        image:
          "https://media.rawg.io/media/screenshots/a13/a130b342c9830f9c56d65c204638fe17.jpg",
      },
      {
        id: 162567,
        image:
          "https://media.rawg.io/media/screenshots/f38/f38a519f1545ef5cda66676c155cc5b8.jpg",
      },
      {
        id: 162568,
        image:
          "https://media.rawg.io/media/screenshots/996/996d1459d3f2ec1f03daba488d96c521.jpg",
      },
      {
        id: 162569,
        image:
          "https://media.rawg.io/media/screenshots/8da/8daab3b2c3d9e355f636e7d408a08315.jpg",
      },
      {
        id: 162570,
        image:
          "https://media.rawg.io/media/screenshots/f5c/f5c713b706c6b153b6a22ca1b08a1f5a.jpg",
      },
      {
        id: 162571,
        image:
          "https://media.rawg.io/media/screenshots/b7c/b7c32275a6be8134744322f1204ce176.jpg",
      },
    ],
  },
  {
    id: 9767,
    slug: "hollow-knight",
    name: "Hollow Knight",
    released: "2017-02-23",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
    rating: 4.4,
    rating_top: 5,
    ratings: [
      { id: 5, title: "exceptional", count: 1561, percent: 61.97 },
      { id: 4, title: "recommended", count: 649, percent: 25.76 },
      { id: 3, title: "meh", count: 192, percent: 7.62 },
      { id: 1, title: "skip", count: 117, percent: 4.64 },
    ],
    ratings_count: 2466,
    reviews_text_count: 33,
    added: 10557,
    added_by_status: {
      yet: 816,
      owned: 6234,
      beaten: 1521,
      toplay: 613,
      dropped: 859,
      playing: 514,
    },
    metacritic: 88,
    playtime: 6,
    suggestions_count: 503,
    updated: "2024-03-19T12:43:47",
    user_game: null,
    reviews_count: 2519,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 18,
          name: "PlayStation 4",
          slug: "playstation4",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 6767,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
        released_at: "2017-02-23",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 6,
          name: "Linux",
          slug: "linux",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 76454,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        released_at: "2017-02-23",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 16.04 LTS (64bit)<br></li><li><strong>Processor:</strong> Intel Core 2 Duo E5200<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> GeForce 9800GTX+ (1GB)<br></li><li><strong>Storage:</strong> 9 GB available space<br></li><li><strong>Additional Notes:</strong> 1080p, 16:9 recommended</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 16.04 LTS (64bit)<br></li><li><strong>Processor:</strong> Intel Core i5<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> GeForce GTX 560<br></li><li><strong>Storage:</strong> 9 GB available space<br></li><li><strong>Additional Notes:</strong> 1080p, 16:9 recommended</li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 5,
          name: "macOS",
          slug: "macos",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 103400,
          image_background:
            "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
        },
        released_at: "2017-02-23",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Mac OS 10.7 Lion<br></li><li><strong>Processor:</strong> Intel Core i3<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> GeForce GTX 470<br></li><li><strong>Storage:</strong> 9 GB available space<br></li><li><strong>Additional Notes:</strong> 1080p, 16:9 recommended</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Mac OS 10.12 Sierra<br></li><li><strong>Processor:</strong> Intel Core i5<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> GeForce GTX 570<br></li><li><strong>Storage:</strong> 9 GB available space<br></li><li><strong>Additional Notes:</strong> 1080p, 16:9 recommended</li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2017-02-23",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows 7<br></li><li><strong>Processor:</strong> Intel Core 2 Duo E5200<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> GeForce 9800GTX+ (1GB)<br></li><li><strong>DirectX:</strong> Version 10<br></li><li><strong>Storage:</strong> 9 GB available space<br></li><li><strong>Additional Notes:</strong> 1080p, 16:9 recommended</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows 10<br></li><li><strong>Processor:</strong> Intel Core i5<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> GeForce GTX 560<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Storage:</strong> 9 GB available space<br></li><li><strong>Additional Notes:</strong> 1080p, 16:9 recommended</li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 7,
          name: "Nintendo Switch",
          slug: "nintendo-switch",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5406,
          image_background:
            "https://media.rawg.io/media/games/5eb/5eb49eb2fa0738fdb5bacea557b1bc57.jpg",
        },
        released_at: "2017-02-23",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 1,
          name: "Xbox One",
          slug: "xbox-one",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5590,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        released_at: "2017-02-23",
        requirements_en: null,
        requirements_ru: null,
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
      { platform: { id: 3, name: "Xbox", slug: "xbox" } },
      { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
      { platform: { id: 6, name: "Linux", slug: "linux" } },
      { platform: { id: 7, name: "Nintendo", slug: "nintendo" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
      {
        id: 51,
        name: "Indie",
        slug: "indie",
        games_count: 62257,
        image_background:
          "https://media.rawg.io/media/games/9dd/9ddabb34840ea9227556670606cf8ea3.jpg",
      },
      {
        id: 83,
        name: "Platformer",
        slug: "platformer",
        games_count: 100773,
        image_background:
          "https://media.rawg.io/media/games/fc8/fc838d98c9b944e6a15176eabf40bee8.jpg",
      },
    ],
    stores: [
      {
        id: 10646,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
      {
        id: 26919,
        store: {
          id: 5,
          name: "GOG",
          slug: "gog",
          domain: "gog.com",
          games_count: 5817,
          image_background:
            "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
        },
      },
      {
        id: 48908,
        store: {
          id: 6,
          name: "Nintendo Store",
          slug: "nintendo",
          domain: "nintendo.com",
          games_count: 8966,
          image_background:
            "https://media.rawg.io/media/games/737/737ea5662211d2e0bbd6f5989189e4f1.jpg",
        },
      },
      {
        id: 296863,
        store: {
          id: 2,
          name: "Xbox Store",
          slug: "xbox-store",
          domain: "microsoft.com",
          games_count: 4813,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
      },
      {
        id: 296864,
        store: {
          id: 3,
          name: "PlayStation Store",
          slug: "playstation-store",
          domain: "store.playstation.com",
          games_count: 7900,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 40847,
        name: "Steam Achievements",
        slug: "steam-achievements",
        language: "eng",
        games_count: 35702,
        image_background:
          "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
      },
      {
        id: 40836,
        name: "Full controller support",
        slug: "full-controller-support",
        language: "eng",
        games_count: 16746,
        image_background:
          "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
      },
      {
        id: 40849,
        name: "Steam Cloud",
        slug: "steam-cloud",
        language: "eng",
        games_count: 16769,
        image_background:
          "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 7808,
        name: "steam-trading-cards",
        slug: "steam-trading-cards",
        language: "eng",
        games_count: 7569,
        image_background:
          "https://media.rawg.io/media/games/6fc/6fcf4cd3b17c288821388e6085bb0fc9.jpg",
      },
      {
        id: 42,
        name: "Great Soundtrack",
        slug: "great-soundtrack",
        language: "eng",
        games_count: 3386,
        image_background:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 118,
        name: "Story Rich",
        slug: "story-rich",
        language: "eng",
        games_count: 20561,
        image_background:
          "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
      },
      {
        id: 36,
        name: "Open World",
        slug: "open-world",
        language: "eng",
        games_count: 7102,
        image_background:
          "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
      },
      {
        id: 45,
        name: "2D",
        slug: "2d",
        language: "eng",
        games_count: 193065,
        image_background:
          "https://media.rawg.io/media/games/4cb/4cb855e8ef1578415a928e53c9f51867.png",
      },
      {
        id: 49,
        name: "Difficult",
        slug: "difficult",
        language: "eng",
        games_count: 13062,
        image_background:
          "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
      },
      {
        id: 6,
        name: "Exploration",
        slug: "exploration",
        language: "eng",
        games_count: 21834,
        image_background:
          "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
      },
      {
        id: 115,
        name: "Controller",
        slug: "controller",
        language: "eng",
        games_count: 10862,
        image_background:
          "https://media.rawg.io/media/games/295/295eb868c241e6ad32ac033b8e6a2ede.jpg",
      },
      {
        id: 336,
        name: "controller support",
        slug: "controller-support",
        language: "eng",
        games_count: 293,
        image_background:
          "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
      },
      {
        id: 88,
        name: "Cute",
        slug: "cute",
        language: "eng",
        games_count: 32125,
        image_background:
          "https://media.rawg.io/media/games/e44/e445335e611b4ccf03af71fffcbd30a4.jpg",
      },
      {
        id: 113,
        name: "Side Scroller",
        slug: "side-scroller",
        language: "eng",
        games_count: 10197,
        image_background:
          "https://media.rawg.io/media/screenshots/3f1/3f1c417b405a86ed7d92b903e0fcfd0c.jpg",
      },
      {
        id: 40,
        name: "Dark Fantasy",
        slug: "dark-fantasy",
        language: "eng",
        games_count: 3876,
        image_background:
          "https://media.rawg.io/media/games/29c/29c6c21cc0c78cff6f45d23631cc82f4.jpg",
      },
      {
        id: 218,
        name: "Multiple Endings",
        slug: "multiple-endings",
        language: "eng",
        games_count: 8219,
        image_background:
          "https://media.rawg.io/media/games/951/951572a3dd1e42544bd39a5d5b42d234.jpg",
      },
      {
        id: 259,
        name: "Metroidvania",
        slug: "metroidvania",
        language: "eng",
        games_count: 4229,
        image_background:
          "https://media.rawg.io/media/games/fc8/fc838d98c9b944e6a15176eabf40bee8.jpg",
      },
      {
        id: 258,
        name: "Hand-drawn",
        slug: "hand-drawn",
        language: "eng",
        games_count: 6601,
        image_background:
          "https://media.rawg.io/media/games/3ef/3eff92562640e452d3487c04ba6d7fae.jpg",
      },
      {
        id: 204,
        name: "Gothic",
        slug: "gothic",
        language: "eng",
        games_count: 856,
        image_background:
          "https://media.rawg.io/media/games/fd9/fd92f105dcd6491bc5d61135033d1f19.jpg",
      },
    ],
    esrb_rating: { id: 2, name: "Everyone 10+", slug: "everyone-10-plus" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
      },
      {
        id: 72619,
        image:
          "https://media.rawg.io/media/screenshots/6b3/6b309936c1fe07e9b7fa5e62a372790d.jpg",
      },
      {
        id: 72620,
        image:
          "https://media.rawg.io/media/screenshots/1a7/1a7a69db58c19d323f1dfbcc340d3f1e.jpg",
      },
      {
        id: 72621,
        image:
          "https://media.rawg.io/media/screenshots/723/7237d0c546b0d17a6a226f38823081d4.jpg",
      },
      {
        id: 72622,
        image:
          "https://media.rawg.io/media/screenshots/331/331095489397e7387681d921e8e472d4.jpg",
      },
      {
        id: 72623,
        image:
          "https://media.rawg.io/media/screenshots/5db/5db89e896496352c8f0a0a0bd545bd6d.jpg",
      },
      {
        id: 72624,
        image:
          "https://media.rawg.io/media/screenshots/843/843e06c9c5b26c309b47bd7075320dae.jpg",
      },
    ],
  },
  {
    id: 41,
    slug: "little-nightmares",
    name: "Little Nightmares",
    released: "2017-04-27",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
    rating: 4.04,
    rating_top: 4,
    ratings: [
      { id: 4, title: "recommended", count: 835, percent: 54.5 },
      { id: 5, title: "exceptional", count: 465, percent: 30.35 },
      { id: 3, title: "meh", count: 149, percent: 9.73 },
      { id: 1, title: "skip", count: 83, percent: 5.42 },
    ],
    ratings_count: 1502,
    reviews_text_count: 24,
    added: 10460,
    added_by_status: {
      yet: 750,
      owned: 7604,
      beaten: 1330,
      toplay: 367,
      dropped: 285,
      playing: 124,
    },
    metacritic: 81,
    playtime: 3,
    suggestions_count: 329,
    updated: "2024-03-14T15:13:47",
    user_game: null,
    reviews_count: 1532,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 18,
          name: "PlayStation 4",
          slug: "playstation4",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 6767,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
        released_at: "2017-04-27",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2017-04-27",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong>  Windows 7, 64-bit<br></li><li><strong>Processor:</strong>  Intel CPU Core i3<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> Nvidia GTX 460<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Network:</strong> Broadband Internet connection<br></li><li><strong>Storage:</strong> 10 GB available space<br></li><li><strong>Additional Notes:</strong> SSE4.2 required</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong>  Windows 7, 64-bit<br></li><li><strong>Processor:</strong>  Intel CPU Core i7<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong>  Nvidia GPU GeForce GTX 660<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Network:</strong> Broadband Internet connection<br></li><li><strong>Storage:</strong> 10 GB available space<br></li><li><strong>Additional Notes:</strong> SSE4.2 required</li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 1,
          name: "Xbox One",
          slug: "xbox-one",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5590,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        released_at: "2017-04-27",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 7,
          name: "Nintendo Switch",
          slug: "nintendo-switch",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5406,
          image_background:
            "https://media.rawg.io/media/games/5eb/5eb49eb2fa0738fdb5bacea557b1bc57.jpg",
        },
        released_at: "2017-04-27",
        requirements_en: null,
        requirements_ru: null,
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
      { platform: { id: 3, name: "Xbox", slug: "xbox" } },
      { platform: { id: 7, name: "Nintendo", slug: "nintendo" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
      {
        id: 83,
        name: "Platformer",
        slug: "platformer",
        games_count: 100773,
        image_background:
          "https://media.rawg.io/media/games/fc8/fc838d98c9b944e6a15176eabf40bee8.jpg",
      },
    ],
    stores: [
      {
        id: 41,
        store: {
          id: 3,
          name: "PlayStation Store",
          slug: "playstation-store",
          domain: "store.playstation.com",
          games_count: 7900,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
      },
      {
        id: 11257,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
      {
        id: 26963,
        store: {
          id: 5,
          name: "GOG",
          slug: "gog",
          domain: "gog.com",
          games_count: 5817,
          image_background:
            "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
        },
      },
      {
        id: 25310,
        store: {
          id: 2,
          name: "Xbox Store",
          slug: "xbox-store",
          domain: "microsoft.com",
          games_count: 4813,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 40847,
        name: "Steam Achievements",
        slug: "steam-achievements",
        language: "eng",
        games_count: 35702,
        image_background:
          "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
      },
      {
        id: 40836,
        name: "Full controller support",
        slug: "full-controller-support",
        language: "eng",
        games_count: 16746,
        image_background:
          "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
      },
      {
        id: 40849,
        name: "Steam Cloud",
        slug: "steam-cloud",
        language: "eng",
        games_count: 16769,
        image_background:
          "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 7808,
        name: "steam-trading-cards",
        slug: "steam-trading-cards",
        language: "eng",
        games_count: 7569,
        image_background:
          "https://media.rawg.io/media/games/6fc/6fcf4cd3b17c288821388e6085bb0fc9.jpg",
      },
      {
        id: 42,
        name: "Great Soundtrack",
        slug: "great-soundtrack",
        language: "eng",
        games_count: 3386,
        image_background:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 118,
        name: "Story Rich",
        slug: "story-rich",
        language: "eng",
        games_count: 20561,
        image_background:
          "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
      },
      {
        id: 149,
        name: "Third Person",
        slug: "third-person",
        language: "eng",
        games_count: 10861,
        image_background:
          "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
      },
      {
        id: 16,
        name: "Horror",
        slug: "horror",
        language: "eng",
        games_count: 43534,
        image_background:
          "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
      },
      {
        id: 189,
        name: "Female Protagonist",
        slug: "female-protagonist",
        language: "eng",
        games_count: 11837,
        image_background:
          "https://media.rawg.io/media/games/530/5302dd22a190e664531236ca724e8726.jpg",
      },
      {
        id: 15,
        name: "Stealth",
        slug: "stealth",
        language: "eng",
        games_count: 6100,
        image_background:
          "https://media.rawg.io/media/games/b7d/b7d3f1715fa8381a4e780173a197a615.jpg",
      },
      {
        id: 41,
        name: "Dark",
        slug: "dark",
        language: "eng",
        games_count: 15192,
        image_background:
          "https://media.rawg.io/media/games/490/49016e06ae2103881ff6373248843069.jpg",
      },
      {
        id: 336,
        name: "controller support",
        slug: "controller-support",
        language: "eng",
        games_count: 293,
        image_background:
          "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
      },
      {
        id: 17,
        name: "Survival Horror",
        slug: "survival-horror",
        language: "eng",
        games_count: 8168,
        image_background:
          "https://media.rawg.io/media/games/ebd/ebdbb7eb52bd58b0e7fa4538d9757b60.jpg",
      },
      {
        id: 111,
        name: "Short",
        slug: "short",
        language: "eng",
        games_count: 57680,
        image_background:
          "https://media.rawg.io/media/games/54a/54a3e4c617217848dc43c4de9989fe37.jpg",
      },
      {
        id: 334,
        name: "achievements",
        slug: "achievements",
        language: "eng",
        games_count: 4910,
        image_background:
          "https://media.rawg.io/media/games/8df/8df64136042eb6e2ed4fa910a6ad96ac.jpg",
      },
      {
        id: 83,
        name: "Puzzle-Platformer",
        slug: "puzzle-platformer",
        language: "eng",
        games_count: 9498,
        image_background:
          "https://media.rawg.io/media/games/a5a/a5abaa1b5cc1567b026fa3aa9fbd828e.jpg",
      },
      {
        id: 116,
        name: "2.5D",
        slug: "25d",
        language: "eng",
        games_count: 1748,
        image_background:
          "https://media.rawg.io/media/screenshots/fe0/fe0ccce931cdb5aaff9500a331e5c2eb.jpg",
      },
    ],
    esrb_rating: { id: 3, name: "Teen", slug: "teen" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 258,
        image:
          "https://media.rawg.io/media/screenshots/00a/00a53be1dcfba6c63ac6807637c4a45a.jpg",
      },
      {
        id: 259,
        image:
          "https://media.rawg.io/media/screenshots/78a/78aa666996cf9d2708f529c18784262f.jpg",
      },
      {
        id: 260,
        image:
          "https://media.rawg.io/media/screenshots/96d/96d356977066c0c2159ed7ef3f470cab.jpg",
      },
      {
        id: 261,
        image:
          "https://media.rawg.io/media/screenshots/3dc/3dc7c607ff7873eaa84a8d69eac26335.jpg",
      },
      {
        id: 262,
        image:
          "https://media.rawg.io/media/screenshots/a83/a83cce4aae86d8ed0b7c8d933231b9bf.jpg",
      },
      {
        id: 263,
        image:
          "https://media.rawg.io/media/screenshots/0c6/0c6642fb4a74893a4c950528791fe7dd.jpg",
      },
    ],
  },
  {
    id: 10035,
    slug: "hitman",
    name: "Hitman",
    released: "2016-03-11",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/16b/16b1b7b36e2042d1128d5a3e852b3b2f.jpg",
    rating: 3.92,
    rating_top: 4,
    ratings: [
      { id: 4, title: "recommended", count: 1107, percent: 56.02 },
      { id: 5, title: "exceptional", count: 462, percent: 23.38 },
      { id: 3, title: "meh", count: 298, percent: 15.08 },
      { id: 1, title: "skip", count: 109, percent: 5.52 },
    ],
    ratings_count: 1965,
    reviews_text_count: 3,
    added: 10276,
    added_by_status: {
      yet: 770,
      owned: 7177,
      beaten: 1166,
      toplay: 351,
      dropped: 681,
      playing: 131,
    },
    metacritic: 83,
    playtime: 7,
    suggestions_count: 572,
    updated: "2024-03-13T20:20:40",
    user_game: null,
    reviews_count: 1976,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 18,
          name: "PlayStation 4",
          slug: "playstation4",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 6767,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
        released_at: "2016-03-11",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 6,
          name: "Linux",
          slug: "linux",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 76454,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        released_at: "2016-03-11",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 16.04 or Steam OS 2.0<br></li><li><strong>Processor:</strong> Intel Core i5-2500K/AMD FX-8350<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> Nvidia GeForce GTX 680/AMD R9 270X graphics card or better*<br></li><li><strong>Storage:</strong> 67 GB available space<br></li><li><strong>Additional Notes:</strong> *NVIDIA graphics cards require driver version 375.26. AMD graphics cards require MESA 13.0.3 or better. Intel graphics cards will not be supported on release.</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 16.10 or Steam OS 2.0<br></li><li><strong>Processor:</strong> Intel Core i7 3770<br></li><li><strong>Memory:</strong> 16 GB RAM<br></li><li><strong>Graphics:</strong> Nvidia GeForce GTX 970/AMD R9 290 graphics card or better*<br></li><li><strong>Storage:</strong> 67 GB available space<br></li><li><strong>Additional Notes:</strong> *NVIDIA graphics cards require driver version 375.26. AMD graphics cards require MESA 13.0.3 or better. Intel graphics cards will not be supported on release.</li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 5,
          name: "macOS",
          slug: "macos",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 103400,
          image_background:
            "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
        },
        released_at: "2016-03-11",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> macOS 10.12.5<br></li><li><strong>Processor:</strong> 2.0Ghz Intel Core i5<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> 2GB AMD M290 or better (see notes for more details)<br></li><li><strong>Storage:</strong> 68 GB available space<br></li><li><strong>Additional Notes:</strong> <strong>The game is supported on the following Macs. To check your Mac model and when it was released, select About This Mac from the Apple menu on your menu bar.</strong>   <ul class="bb_ul">  <li>*    All 27&quot; iMacs released since Late 2014 </li><li>*    All 15&quot; MacBook Pros released since 2016  </li><li>*    All Mac Pros released since Late 2013 </li></ul>    ——— <strong>The following Macs are capable of running the game but do not consistently meet the standards required for official support.</strong> <ul class="bb_ul">  <li>*    All 15&quot; MacBook Pros with an AMD graphics card released in 2015 </li></ul> ——— <ul class="bb_ul"><strong>Please note: HITMAN for macOS will launch with support for AMD graphics cards only. We hope to announce more details regarding support for Nvidia and Intel graphics cards shortly.</strong></ul></li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2016-03-11",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> OS 64-bit Windows 7<br></li><li><strong>Processor:</strong> Intel CPU Core i5-2500K 3.3GHz / AMD CPU Phenom II X4 940<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> NVIDIA GeForce GTX 660 / Radeon HD 7870<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Storage:</strong> 50 GB available space</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> OS 64-bit Windows 7 / 64-bit Windows 8 (8.1) or Windows 10<br></li><li><strong>Processor:</strong> Intel CPU Core i7 3770 3,4 GHz / AMD CPU AMD FX-8350 4 GHz<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> Nvidia GPU GeForce GTX 770 / AMD GPU Radeon R9 290<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Storage:</strong> 50 GB available space</li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 1,
          name: "Xbox One",
          slug: "xbox-one",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5590,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        released_at: "2016-03-11",
        requirements_en: null,
        requirements_ru: null,
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
      { platform: { id: 3, name: "Xbox", slug: "xbox" } },
      { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
      { platform: { id: 6, name: "Linux", slug: "linux" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
      {
        id: 2,
        name: "Shooter",
        slug: "shooter",
        games_count: 59459,
        image_background:
          "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
      },
      {
        id: 14,
        name: "Simulation",
        slug: "simulation",
        games_count: 68547,
        image_background:
          "https://media.rawg.io/media/games/e40/e40cc9d1957b0a0ed7e389834457b524.jpg",
      },
    ],
    stores: [
      {
        id: 38172,
        store: {
          id: 3,
          name: "PlayStation Store",
          slug: "playstation-store",
          domain: "store.playstation.com",
          games_count: 7900,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
      },
      {
        id: 10947,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
      {
        id: 35171,
        store: {
          id: 2,
          name: "Xbox Store",
          slug: "xbox-store",
          domain: "microsoft.com",
          games_count: 4813,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
      },
      {
        id: 593095,
        store: {
          id: 11,
          name: "Epic Games",
          slug: "epic-games",
          domain: "epicgames.com",
          games_count: 1319,
          image_background:
            "https://media.rawg.io/media/games/4be/4be6a6ad0364751a96229c56bf69be59.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 40836,
        name: "Full controller support",
        slug: "full-controller-support",
        language: "eng",
        games_count: 16746,
        image_background:
          "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 42,
        name: "Great Soundtrack",
        slug: "great-soundtrack",
        language: "eng",
        games_count: 3386,
        image_background:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 118,
        name: "Story Rich",
        slug: "story-rich",
        language: "eng",
        games_count: 20561,
        image_background:
          "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
      },
      {
        id: 36,
        name: "Open World",
        slug: "open-world",
        language: "eng",
        games_count: 7102,
        image_background:
          "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
      },
      {
        id: 149,
        name: "Third Person",
        slug: "third-person",
        language: "eng",
        games_count: 10861,
        image_background:
          "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
      },
      {
        id: 37,
        name: "Sandbox",
        slug: "sandbox",
        language: "eng",
        games_count: 6607,
        image_background:
          "https://media.rawg.io/media/games/7cf/7cfc9220b401b7a300e409e539c9afd5.jpg",
      },
      {
        id: 15,
        name: "Stealth",
        slug: "stealth",
        language: "eng",
        games_count: 6100,
        image_background:
          "https://media.rawg.io/media/games/b7d/b7d3f1715fa8381a4e780173a197a615.jpg",
      },
      {
        id: 80,
        name: "Tactical",
        slug: "tactical",
        language: "eng",
        games_count: 4789,
        image_background:
          "https://media.rawg.io/media/games/997/997ab4d67e96fb20a4092383477d4463.jpg",
      },
      {
        id: 150,
        name: "Third-Person Shooter",
        slug: "third-person-shooter",
        language: "eng",
        games_count: 3266,
        image_background:
          "https://media.rawg.io/media/games/ebd/ebdbb7eb52bd58b0e7fa4538d9757b60.jpg",
      },
      {
        id: 44,
        name: "Nudity",
        slug: "nudity",
        language: "eng",
        games_count: 6072,
        image_background:
          "https://media.rawg.io/media/games/260/26023c855f1769a93411d6a7ea084632.jpeg",
      },
      {
        id: 144,
        name: "Crime",
        slug: "crime",
        language: "eng",
        games_count: 2718,
        image_background:
          "https://media.rawg.io/media/games/5fa/5fae5fec3c943179e09da67a4427d68f.jpg",
      },
      {
        id: 278,
        name: "Assassin",
        slug: "assassin",
        language: "eng",
        games_count: 807,
        image_background:
          "https://media.rawg.io/media/games/858/858c016de0cf7bc21a57dcc698a04a0c.jpg",
      },
      {
        id: 232,
        name: "Episodic",
        slug: "episodic",
        language: "eng",
        games_count: 459,
        image_background:
          "https://media.rawg.io/media/games/a28/a289e23b4d4d84f26ab59125e3be4483.jpg",
      },
      {
        id: 178,
        name: "Illuminati",
        slug: "illuminati",
        language: "eng",
        games_count: 315,
        image_background:
          "https://media.rawg.io/media/games/46b/46b62c37c8faf3268a0768b548e6cd9d.jpg",
      },
      {
        id: 216,
        name: "Heist",
        slug: "heist",
        language: "eng",
        games_count: 502,
        image_background:
          "https://media.rawg.io/media/games/7e0/7e09315d72e12b6cd8995ba18b5f70d0.jpg",
      },
    ],
    esrb_rating: { id: 5, name: "Adults Only", slug: "adults-only" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/16b/16b1b7b36e2042d1128d5a3e852b3b2f.jpg",
      },
      {
        id: 76082,
        image:
          "https://media.rawg.io/media/screenshots/ef6/ef6c0a92d08a99d2e405cac53c597d10.jpg",
      },
      {
        id: 76083,
        image:
          "https://media.rawg.io/media/screenshots/fb3/fb3c23014fcb24a28fb94af0a009906c.jpg",
      },
      {
        id: 76084,
        image:
          "https://media.rawg.io/media/screenshots/202/202293192d1b6245c8a1e252d9df604c.jpg",
      },
      {
        id: 76085,
        image:
          "https://media.rawg.io/media/screenshots/278/278bd63ee564982e816ec7fe802df420.jpg",
      },
      {
        id: 76086,
        image:
          "https://media.rawg.io/media/screenshots/002/0021b0e5db6959989d90979abba68c8d.jpg",
      },
      {
        id: 76087,
        image:
          "https://media.rawg.io/media/screenshots/c8a/c8a537532ab8ce8750293a1fc41ee7f8.jpg",
      },
    ],
  },
  {
    id: 2551,
    slug: "dark-souls-iii",
    name: "Dark Souls III",
    released: "2016-04-11",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/da1/da1b267764d77221f07a4386b6548e5a.jpg",
    rating: 4.4,
    rating_top: 5,
    ratings: [
      { id: 5, title: "exceptional", count: 1600, percent: 61.4 },
      { id: 4, title: "recommended", count: 698, percent: 26.78 },
      { id: 3, title: "meh", count: 185, percent: 7.1 },
      { id: 1, title: "skip", count: 123, percent: 4.72 },
    ],
    ratings_count: 2581,
    reviews_text_count: 17,
    added: 10229,
    added_by_status: {
      yet: 630,
      owned: 6201,
      beaten: 1960,
      toplay: 536,
      dropped: 633,
      playing: 269,
    },
    metacritic: 89,
    playtime: 29,
    suggestions_count: 708,
    updated: "2024-03-20T13:47:45",
    user_game: null,
    reviews_count: 2606,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 1,
          name: "Xbox One",
          slug: "xbox-one",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5590,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        released_at: "2016-04-11",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 18,
          name: "PlayStation 4",
          slug: "playstation4",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 6767,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
        released_at: "2016-04-11",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2016-04-11",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows 7 SP1 64bit, Windows 8.1 64bit Windows 10 64bit<br></li><li><strong>Processor:</strong> Intel Core i3-2100 / AMD® FX-6300<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> NVIDIA® GeForce GTX 750 Ti / ATI Radeon HD 7950<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Network:</strong> Broadband Internet connection<br></li><li><strong>Storage:</strong> 25 GB available space<br></li><li><strong>Sound Card:</strong> DirectX 11 sound device<br></li><li><strong>Additional Notes:</strong> Internet connection required for online play and product activation</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows 7 SP1 64bit, Windows 8.1 64bit Windows 10 64bit<br></li><li><strong>Processor:</strong> Intel Core i7-3770 / AMD® FX-8350<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> NVIDIA® GeForce GTX 970 / ATI Radeon R9 series<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Network:</strong> Broadband Internet connection<br></li><li><strong>Storage:</strong> 25 GB available space<br></li><li><strong>Sound Card:</strong> DirectX 11 sound device<br></li><li><strong>Additional Notes:</strong> Internet connection required for online play and product activation</li></ul>',
        },
        requirements_ru: null,
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
      { platform: { id: 3, name: "Xbox", slug: "xbox" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
      {
        id: 5,
        name: "RPG",
        slug: "role-playing-games-rpg",
        games_count: 55107,
        image_background:
          "https://media.rawg.io/media/games/618/618c2031a07bbff6b4f611f10b6bcdbc.jpg",
      },
    ],
    stores: [
      {
        id: 2679,
        store: {
          id: 3,
          name: "PlayStation Store",
          slug: "playstation-store",
          domain: "store.playstation.com",
          games_count: 7900,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
      },
      {
        id: 9067,
        store: {
          id: 2,
          name: "Xbox Store",
          slug: "xbox-store",
          domain: "microsoft.com",
          games_count: 4813,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
      },
      {
        id: 11140,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 40847,
        name: "Steam Achievements",
        slug: "steam-achievements",
        language: "eng",
        games_count: 35702,
        image_background:
          "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
      },
      {
        id: 7,
        name: "Multiplayer",
        slug: "multiplayer",
        language: "eng",
        games_count: 37130,
        image_background:
          "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
      },
      {
        id: 40836,
        name: "Full controller support",
        slug: "full-controller-support",
        language: "eng",
        games_count: 16746,
        image_background:
          "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 7808,
        name: "steam-trading-cards",
        slug: "steam-trading-cards",
        language: "eng",
        games_count: 7569,
        image_background:
          "https://media.rawg.io/media/games/6fc/6fcf4cd3b17c288821388e6085bb0fc9.jpg",
      },
      {
        id: 42,
        name: "Great Soundtrack",
        slug: "great-soundtrack",
        language: "eng",
        games_count: 3386,
        image_background:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 24,
        name: "RPG",
        slug: "rpg",
        language: "eng",
        games_count: 19865,
        image_background:
          "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
      },
      {
        id: 18,
        name: "Co-op",
        slug: "co-op",
        language: "eng",
        games_count: 11072,
        image_background:
          "https://media.rawg.io/media/games/511/5118aff5091cb3efec399c808f8c598f.jpg",
      },
      {
        id: 118,
        name: "Story Rich",
        slug: "story-rich",
        language: "eng",
        games_count: 20561,
        image_background:
          "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
      },
      {
        id: 36,
        name: "Open World",
        slug: "open-world",
        language: "eng",
        games_count: 7102,
        image_background:
          "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
      },
      {
        id: 411,
        name: "cooperative",
        slug: "cooperative",
        language: "eng",
        games_count: 4703,
        image_background:
          "https://media.rawg.io/media/games/73e/73eecb8909e0c39fb246f457b5d6cbbe.jpg",
      },
      {
        id: 149,
        name: "Third Person",
        slug: "third-person",
        language: "eng",
        games_count: 10861,
        image_background:
          "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
      },
      {
        id: 16,
        name: "Horror",
        slug: "horror",
        language: "eng",
        games_count: 43534,
        image_background:
          "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
      },
      {
        id: 49,
        name: "Difficult",
        slug: "difficult",
        language: "eng",
        games_count: 13062,
        image_background:
          "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
      },
      {
        id: 6,
        name: "Exploration",
        slug: "exploration",
        language: "eng",
        games_count: 21834,
        image_background:
          "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
      },
      {
        id: 97,
        name: "Action RPG",
        slug: "action-rpg",
        language: "eng",
        games_count: 6501,
        image_background:
          "https://media.rawg.io/media/games/157/15742f2f67eacff546738e1ab5c19d20.jpg",
      },
      {
        id: 157,
        name: "PvP",
        slug: "pvp",
        language: "eng",
        games_count: 8687,
        image_background:
          "https://media.rawg.io/media/games/737/737ea5662211d2e0bbd6f5989189e4f1.jpg",
      },
      {
        id: 121,
        name: "Character Customization",
        slug: "character-customization",
        language: "eng",
        games_count: 4284,
        image_background:
          "https://media.rawg.io/media/games/33b/33b825c76382931df0fd8ecddf5caebe.jpg",
      },
      {
        id: 5,
        name: "Replay Value",
        slug: "replay-value",
        language: "eng",
        games_count: 1548,
        image_background:
          "https://media.rawg.io/media/games/1f4/1f47a270b8f241e4676b14d39ec620f7.jpg",
      },
      {
        id: 40,
        name: "Dark Fantasy",
        slug: "dark-fantasy",
        language: "eng",
        games_count: 3876,
        image_background:
          "https://media.rawg.io/media/games/29c/29c6c21cc0c78cff6f45d23631cc82f4.jpg",
      },
      {
        id: 205,
        name: "Lore-Rich",
        slug: "lore-rich",
        language: "eng",
        games_count: 1145,
        image_background:
          "https://media.rawg.io/media/games/c24/c24f4434882ae9c2c8d9d38de82cb7a5.jpg",
      },
    ],
    esrb_rating: { id: 4, name: "Mature", slug: "mature" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/da1/da1b267764d77221f07a4386b6548e5a.jpg",
      },
      {
        id: 805127,
        image:
          "https://media.rawg.io/media/screenshots/d7c/d7c05cdfb30ec07147bcd0d3985ec54c.jpg",
      },
      {
        id: 805128,
        image:
          "https://media.rawg.io/media/screenshots/479/479cb74c874748ca70a3a14e79a0c232.jpg",
      },
      {
        id: 805129,
        image:
          "https://media.rawg.io/media/screenshots/070/0703edc6d3db345a3acf19b4e6e43ebd.jpg",
      },
      {
        id: 805130,
        image:
          "https://media.rawg.io/media/screenshots/e4f/e4feaf5b078949102e72780091eb12af.jpg",
      },
      {
        id: 805131,
        image:
          "https://media.rawg.io/media/screenshots/028/02863a140eea53c1f51fd790aca753aa.jpg",
      },
      {
        id: 805132,
        image:
          "https://media.rawg.io/media/screenshots/8d2/8d2c0bb90867031f9265ef62c01445c5.jpg",
      },
    ],
  },
  {
    id: 4252,
    slug: "mirrors-edge",
    name: "Mirror's Edge",
    released: "2008-11-11",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/8e4/8e4de3f54ac659e08a7ba6a2b731682a.jpg",
    rating: 4.07,
    rating_top: 4,
    ratings: [
      { id: 4, title: "recommended", count: 1221, percent: 53.23 },
      { id: 5, title: "exceptional", count: 689, percent: 30.03 },
      { id: 3, title: "meh", count: 312, percent: 13.6 },
      { id: 1, title: "skip", count: 72, percent: 3.14 },
    ],
    ratings_count: 2273,
    reviews_text_count: 12,
    added: 10202,
    added_by_status: {
      yet: 333,
      owned: 6549,
      beaten: 2336,
      toplay: 233,
      dropped: 710,
      playing: 41,
    },
    metacritic: 81,
    playtime: 3,
    suggestions_count: 343,
    updated: "2024-03-14T09:50:08",
    user_game: null,
    reviews_count: 2294,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 14,
          name: "Xbox 360",
          slug: "xbox360",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 2796,
          image_background:
            "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
        },
        released_at: "2008-11-11",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 16,
          name: "PlayStation 3",
          slug: "playstation3",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 3168,
          image_background:
            "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
        },
        released_at: "2008-11-11",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2008-11-11",
        requirements_en: {
          minimum:
            '<ul class="bb_ul"><li><strong>Supported OS:</strong> Microsoft Windows® XP SP2 or Vista                    <br>\t\t\t\t\t</li><li><strong>Processor:</strong> 3.0 GHz or faster                    <br>\t\t\t\t\t</li><li><strong>Memory:</strong> 1 GB RAM or more                    <br>\t\t\t\t\t</li><li><strong>Graphics:</strong> DirectX® 9.0c  compatible video card, Shader Model 3.0 required. Video card must have 256 MB, NVIDIA GeForce 6800 or better                    <br>\t\t\t\t\t</li><li><strong>Hard Drive:</strong> 8 GB free space <br>\t\t\t\t\t</li><li><strong>Sound:</strong> DirectX® 9.0c compatible sound card </li></ul><ul class="bb_ul"><li>INTERNET CONNECTION, ONLINE AUTHENTICATION AND ACCEPTANCE OF END USER LICENSE AGREEMENT REQUIRED TO PLAY. TO ACCESS ONLINE FEATURES, YOU MUST REGISTER ONLINE. ONLY ONE REGISTRATION IS AVAILABLE PER GAME. EA ONLINE TERMS &amp; CONDITIONS AND FEATURE UPDATES CAN BE FOUND AT <a href="https://steamcommunity.com/linkfilter/?url=http://www.ea.com" target="_blank" rel="noopener"  >www.ea.com</a>. YOU MUST BE 13+ TO REGISTER WITH EA ONLINE.</li></ul>',
        },
        requirements_ru: {
          minimum:
            "Pentium 4/Athlon XP 3 ГГц,1 Гб памяти,3D-ускоритель с 256 Мб памяти,8 Гб на винчестере",
          recommended:
            "Core 2 Duo/Athlon X2 2.5 ГГц,2 Гб памяти,3D-ускоритель с 512 Мб памяти,8 Гб на винчестере,интернет-соединение",
        },
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
      { platform: { id: 3, name: "Xbox", slug: "xbox" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
    ],
    stores: [
      {
        id: 4580,
        store: {
          id: 3,
          name: "PlayStation Store",
          slug: "playstation-store",
          domain: "store.playstation.com",
          games_count: 7900,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
      },
      {
        id: 21595,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
      {
        id: 26791,
        store: {
          id: 5,
          name: "GOG",
          slug: "gog",
          domain: "gog.com",
          games_count: 5817,
          image_background:
            "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
        },
      },
      {
        id: 34058,
        store: {
          id: 7,
          name: "Xbox 360 Store",
          slug: "xbox360",
          domain: "marketplace.xbox.com",
          games_count: 1912,
          image_background:
            "https://media.rawg.io/media/games/d1a/d1a2e99ade53494c6330a0ed945fe823.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 40836,
        name: "Full controller support",
        slug: "full-controller-support",
        language: "eng",
        games_count: 16746,
        image_background:
          "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 42,
        name: "Great Soundtrack",
        slug: "great-soundtrack",
        language: "eng",
        games_count: 3386,
        image_background:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 8,
        name: "First-Person",
        slug: "first-person",
        language: "eng",
        games_count: 30626,
        image_background:
          "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
      },
      {
        id: 32,
        name: "Sci-fi",
        slug: "sci-fi",
        language: "eng",
        games_count: 18535,
        image_background:
          "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
      },
      {
        id: 40845,
        name: "Partial Controller Support",
        slug: "partial-controller-support",
        language: "eng",
        games_count: 11229,
        image_background:
          "https://media.rawg.io/media/games/62c/62c7c8b28a27b83680b22fb9d33fc619.jpg",
      },
      {
        id: 193,
        name: "Classic",
        slug: "classic",
        language: "eng",
        games_count: 1783,
        image_background:
          "https://media.rawg.io/media/games/960/960b601d9541cec776c5fa42a00bf6c4.jpg",
      },
      {
        id: 189,
        name: "Female Protagonist",
        slug: "female-protagonist",
        language: "eng",
        games_count: 11837,
        image_background:
          "https://media.rawg.io/media/games/530/5302dd22a190e664531236ca724e8726.jpg",
      },
      {
        id: 111,
        name: "Short",
        slug: "short",
        language: "eng",
        games_count: 57680,
        image_background:
          "https://media.rawg.io/media/games/54a/54a3e4c617217848dc43c4de9989fe37.jpg",
      },
      {
        id: 165,
        name: "Colorful",
        slug: "colorful",
        language: "eng",
        games_count: 20827,
        image_background:
          "https://media.rawg.io/media/games/3e3/3e355e1b8a5ee47f4c76e28e3055236d.jpg",
      },
      {
        id: 167,
        name: "Futuristic",
        slug: "futuristic",
        language: "eng",
        games_count: 5050,
        image_background:
          "https://media.rawg.io/media/games/90f/90fd5e569bc4c4a666c588a732124908.jpg",
      },
      {
        id: 188,
        name: "Parkour",
        slug: "parkour",
        language: "eng",
        games_count: 3425,
        image_background:
          "https://media.rawg.io/media/games/c2e/c2eb6021a2596644b437e943612af25c.jpg",
      },
      {
        id: 119,
        name: "Dystopian",
        slug: "dystopian",
        language: "eng",
        games_count: 2001,
        image_background:
          "https://media.rawg.io/media/games/a0e/a0ef08621301a1eab5e04fa5c96978fa.jpeg",
      },
      {
        id: 166,
        name: "Stylized",
        slug: "stylized",
        language: "eng",
        games_count: 5738,
        image_background:
          "https://media.rawg.io/media/screenshots/f7f/f7fb42b2115c3730c00248dafe57e2d0.jpg",
      },
      {
        id: 280,
        name: "Time Attack",
        slug: "time-attack",
        language: "eng",
        games_count: 1670,
        image_background:
          "https://media.rawg.io/media/screenshots/3ac/3ace8d21a5d653c775844d591f3f14ef.jpg",
      },
      {
        id: 19728,
        name: "european",
        slug: "european",
        language: "eng",
        games_count: 12,
        image_background:
          "https://media.rawg.io/media/screenshots/d14/d148f8ffc332b1d0f70be9bad91c2171.jpg",
      },
    ],
    esrb_rating: { id: 3, name: "Teen", slug: "teen" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/8e4/8e4de3f54ac659e08a7ba6a2b731682a.jpg",
      },
      {
        id: 179965,
        image:
          "https://media.rawg.io/media/screenshots/38b/38bb5b035c811402248bbc19297d5183.jpg",
      },
      {
        id: 179966,
        image:
          "https://media.rawg.io/media/screenshots/4b6/4b6daf2d868abed65984f6ba308eb5f6.jpg",
      },
      {
        id: 179967,
        image:
          "https://media.rawg.io/media/screenshots/b07/b07d46f11697eb8ee5002d37d9ddc696.jpg",
      },
      {
        id: 179968,
        image:
          "https://media.rawg.io/media/screenshots/ddd/ddde2f1b028ded72097066b504db6f8b.jpg",
      },
      {
        id: 179969,
        image:
          "https://media.rawg.io/media/screenshots/a8b/a8b9fe7b7e04ff20405e40fb31cfe56e.jpg",
      },
      {
        id: 179970,
        image:
          "https://media.rawg.io/media/screenshots/741/741957fcdece6895a653bf51786838e9.jpg",
      },
    ],
  },
  {
    id: 3612,
    slug: "hotline-miami",
    name: "Hotline Miami",
    released: "2012-10-22",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/9fa/9fa63622543e5d4f6d99aa9d73b043de.jpg",
    rating: 4.38,
    rating_top: 5,
    ratings: [
      { id: 5, title: "exceptional", count: 1382, percent: 54.22 },
      { id: 4, title: "recommended", count: 911, percent: 35.74 },
      { id: 3, title: "meh", count: 174, percent: 6.83 },
      { id: 1, title: "skip", count: 82, percent: 3.22 },
    ],
    ratings_count: 2524,
    reviews_text_count: 12,
    added: 10128,
    added_by_status: {
      yet: 367,
      owned: 6672,
      beaten: 2196,
      toplay: 205,
      dropped: 607,
      playing: 81,
    },
    metacritic: 85,
    playtime: 5,
    suggestions_count: 294,
    updated: "2024-03-17T18:52:09",
    user_game: null,
    reviews_count: 2549,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 6,
          name: "Linux",
          slug: "linux",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 76454,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        released_at: "2012-10-22",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 7,
          name: "Nintendo Switch",
          slug: "nintendo-switch",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5406,
          image_background:
            "https://media.rawg.io/media/games/5eb/5eb49eb2fa0738fdb5bacea557b1bc57.jpg",
        },
        released_at: "2012-10-22",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 1,
          name: "Xbox One",
          slug: "xbox-one",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5590,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        released_at: "2012-10-22",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 18,
          name: "PlayStation 4",
          slug: "playstation4",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 6767,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
        released_at: "2012-10-22",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 19,
          name: "PS Vita",
          slug: "ps-vita",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 1444,
          image_background:
            "https://media.rawg.io/media/games/8d6/8d69eb6c32ed6acfd75f82d532144993.jpg",
        },
        released_at: "2012-10-22",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2012-10-22",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong>Microsoft® Windows® XP / Vista / 7<br>\t</li><li><strong>Processor:</strong>1.2GHz processor<br>\t</li><li><strong>Memory:</strong>512 MB RAM<br>\t</li><li><strong>Graphics:</strong>DirectX 8-compatible graphics card with at least 32MB of video memory<br>\t</li><li><strong>DirectX®:</strong>9.0c<br>\t</li><li><strong>Hard Drive:</strong>250 MB HD space<br>\t</li><li><strong>Additional:</strong>Microsoft Xbox 360 Controller or Direct Input compatible controller, OpenGL 2.x or better highly recommended.</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong>Microsoft® Windows® XP / Vista / 7<br>\t</li><li><strong>Processor:</strong>1.4GHz processor or faster<br>\t</li><li><strong>Memory:</strong>1 GB RAM<br>\t</li><li><strong>Graphics:</strong>DirectX 8-compatible graphics card with at least 32MB of video memory<br>\t</li><li><strong>DirectX®:</strong>9.0c<br>\t</li><li><strong>Hard Drive:</strong>250 MB HD space<br>\t</li><li><strong>Additional:</strong>Microsoft Xbox 360 Controller or Direct Input compatible controller</li></ul>',
        },
        requirements_ru: {
          minimum:
            "Pentium 3/Athlon 1.2 ГГц,512 Мб памяти,видеокарта с 32 Мб памяти,250 Мб на винчестере",
          recommended:
            "Pentium 4/Athlon XP 1.5 ГГц,1 Гб памяти,видеокарта с 32 Мб памяти,250 Мб на винчестере",
        },
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
      { platform: { id: 3, name: "Xbox", slug: "xbox" } },
      { platform: { id: 6, name: "Linux", slug: "linux" } },
      { platform: { id: 7, name: "Nintendo", slug: "nintendo" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
      {
        id: 2,
        name: "Shooter",
        slug: "shooter",
        games_count: 59459,
        image_background:
          "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
      },
      {
        id: 11,
        name: "Arcade",
        slug: "arcade",
        games_count: 22649,
        image_background:
          "https://media.rawg.io/media/games/7cf/7cf38d2d80267c121c6d0d361e9429ce.jpg",
      },
      {
        id: 51,
        name: "Indie",
        slug: "indie",
        games_count: 62257,
        image_background:
          "https://media.rawg.io/media/games/9dd/9ddabb34840ea9227556670606cf8ea3.jpg",
      },
    ],
    stores: [
      {
        id: 25947,
        store: {
          id: 5,
          name: "GOG",
          slug: "gog",
          domain: "gog.com",
          games_count: 5817,
          image_background:
            "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
        },
      },
      {
        id: 3905,
        store: {
          id: 3,
          name: "PlayStation Store",
          slug: "playstation-store",
          domain: "store.playstation.com",
          games_count: 7900,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
      },
      {
        id: 19491,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
      {
        id: 214847,
        store: {
          id: 8,
          name: "Google Play",
          slug: "google-play",
          domain: "play.google.com",
          games_count: 17079,
          image_background:
            "https://media.rawg.io/media/games/13a/13a528ac9cf48bbb6be5d35fe029336d.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 40847,
        name: "Steam Achievements",
        slug: "steam-achievements",
        language: "eng",
        games_count: 35702,
        image_background:
          "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 7808,
        name: "steam-trading-cards",
        slug: "steam-trading-cards",
        language: "eng",
        games_count: 7569,
        image_background:
          "https://media.rawg.io/media/games/6fc/6fcf4cd3b17c288821388e6085bb0fc9.jpg",
      },
      {
        id: 42,
        name: "Great Soundtrack",
        slug: "great-soundtrack",
        language: "eng",
        games_count: 3386,
        image_background:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 45,
        name: "2D",
        slug: "2d",
        language: "eng",
        games_count: 193065,
        image_background:
          "https://media.rawg.io/media/games/4cb/4cb855e8ef1578415a928e53c9f51867.png",
      },
      {
        id: 40845,
        name: "Partial Controller Support",
        slug: "partial-controller-support",
        language: "eng",
        games_count: 11229,
        image_background:
          "https://media.rawg.io/media/games/62c/62c7c8b28a27b83680b22fb9d33fc619.jpg",
      },
      {
        id: 26,
        name: "Gore",
        slug: "gore",
        language: "eng",
        games_count: 5515,
        image_background:
          "https://media.rawg.io/media/games/7f6/7f6cd70ba2ad57053b4847c13569f2d8.jpg",
      },
      {
        id: 49,
        name: "Difficult",
        slug: "difficult",
        language: "eng",
        games_count: 13062,
        image_background:
          "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
      },
      {
        id: 34,
        name: "Violent",
        slug: "violent",
        language: "eng",
        games_count: 6476,
        image_background:
          "https://media.rawg.io/media/games/511/5118aff5091cb3efec399c808f8c598f.jpg",
      },
      {
        id: 122,
        name: "Pixel Graphics",
        slug: "pixel-graphics",
        language: "eng",
        games_count: 92565,
        image_background:
          "https://media.rawg.io/media/games/f46/f466571d536f2e3ea9e815ad17177501.jpg",
      },
      {
        id: 74,
        name: "Retro",
        slug: "retro",
        language: "eng",
        games_count: 41033,
        image_background:
          "https://media.rawg.io/media/games/bbf/bbf8d74ab64440ad76294cff2f4d9cfa.jpg",
      },
      {
        id: 336,
        name: "controller support",
        slug: "controller-support",
        language: "eng",
        games_count: 293,
        image_background:
          "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
      },
      {
        id: 131,
        name: "Fast-Paced",
        slug: "fast-paced",
        language: "eng",
        games_count: 10466,
        image_background:
          "https://media.rawg.io/media/games/9fa/9fa63622543e5d4f6d99aa9d73b043de.jpg",
      },
      {
        id: 61,
        name: "Top-Down",
        slug: "top-down",
        language: "eng",
        games_count: 24503,
        image_background:
          "https://media.rawg.io/media/screenshots/2fc/2fc6994425146f9dba3133400b414e29.jpg",
      },
      {
        id: 337,
        name: "overlay",
        slug: "overlay",
        language: "eng",
        games_count: 217,
        image_background:
          "https://media.rawg.io/media/games/fad/fadc4be043ed07904012d47cd02671e4.jpg",
      },
      {
        id: 136,
        name: "Music",
        slug: "music",
        language: "eng",
        games_count: 25902,
        image_background:
          "https://media.rawg.io/media/games/f99/f9979698c43fd84c3ab69280576dd3af.jpg",
      },
      {
        id: 46,
        name: "Surreal",
        slug: "surreal",
        language: "eng",
        games_count: 5057,
        image_background:
          "https://media.rawg.io/media/screenshots/5a3/5a36930b16d9b1949d8c8c92b5a28d89.jpg",
      },
      {
        id: 236,
        name: "Top-Down Shooter",
        slug: "top-down-shooter",
        language: "eng",
        games_count: 2026,
        image_background:
          "https://media.rawg.io/media/screenshots/80a/80a0d07be6109fde6b92ac5283e7056c.jpg",
      },
      {
        id: 224,
        name: "1980s",
        slug: "1980s",
        language: "eng",
        games_count: 2265,
        image_background:
          "https://media.rawg.io/media/games/5e4/5e4bff02098b2b6f824c68286d5da1a6.jpg",
      },
      {
        id: 286,
        name: "Psychedelic",
        slug: "psychedelic",
        language: "eng",
        games_count: 2020,
        image_background:
          "https://media.rawg.io/media/games/9fa/9fa63622543e5d4f6d99aa9d73b043de.jpg",
      },
    ],
    esrb_rating: null,
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/9fa/9fa63622543e5d4f6d99aa9d73b043de.jpg",
      },
      {
        id: 32084,
        image:
          "https://media.rawg.io/media/screenshots/38b/38b9bb0de0a380434b78587e132b2e21.jpg",
      },
      {
        id: 32085,
        image:
          "https://media.rawg.io/media/screenshots/e7a/e7a11a669aa609b9b3b0c7aeab2dc804.jpg",
      },
      {
        id: 32086,
        image:
          "https://media.rawg.io/media/screenshots/88b/88b907144995e57c45ee043c59dd6810.jpg",
      },
      {
        id: 32087,
        image:
          "https://media.rawg.io/media/screenshots/53f/53fd44fd759bc571b1445898c50b418e.jpg",
      },
      {
        id: 32088,
        image:
          "https://media.rawg.io/media/screenshots/ddd/dddc7151559716c7c0dddb05874496da.jpg",
      },
      {
        id: 32089,
        image:
          "https://media.rawg.io/media/screenshots/b2a/b2a25c20510e3111a13d56a9751cadd7.jpg",
      },
    ],
  },
  {
    id: 18080,
    slug: "half-life",
    name: "Half-Life",
    released: "1998-11-08",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/6c5/6c55e22185876626881b76c11922b073.jpg",
    rating: 4.38,
    rating_top: 5,
    ratings: [
      { id: 5, title: "exceptional", count: 1268, percent: 55.66 },
      { id: 4, title: "recommended", count: 766, percent: 33.63 },
      { id: 3, title: "meh", count: 164, percent: 7.2 },
      { id: 1, title: "skip", count: 80, percent: 3.51 },
    ],
    ratings_count: 2259,
    reviews_text_count: 13,
    added: 10060,
    added_by_status: {
      yet: 396,
      owned: 6514,
      beaten: 2382,
      toplay: 168,
      dropped: 541,
      playing: 59,
    },
    metacritic: 96,
    playtime: 3,
    suggestions_count: 469,
    updated: "2024-03-17T16:17:16",
    user_game: null,
    reviews_count: 2278,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 106,
          name: "Dreamcast",
          slug: "dreamcast",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 362,
          image_background:
            "https://media.rawg.io/media/games/5b4/5b4af50ce68cf94dcab24f3dba33eaab.jpg",
        },
        released_at: "1998-11-08",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 6,
          name: "Linux",
          slug: "linux",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 76454,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        released_at: "1998-11-08",
        requirements_en: {
          minimum:
            "Minimum: Linux Ubuntu 12.04, Dual-core from Intel or AMD at 2.8 GHz, 1GB Memory, nVidia GeForce 8600/9600GT, ATI/AMD Radeaon HD2600/3600 (Graphic Drivers: nVidia 310, AMD 12.11), OpenGL 2.1, 4GB Hard Drive Space, OpenAL Compatible Sound Card",
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 5,
          name: "macOS",
          slug: "macos",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 103400,
          image_background:
            "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
        },
        released_at: "1998-11-08",
        requirements_en: {
          minimum:
            "Minimum: OS X  Snow Leopard 10.6.3, 1GB RAM, 4GB Hard Drive Space,NVIDIA GeForce 8 or higher, ATI X1600 or higher, or Intel HD 3000 or higher Mouse, Keyboard, Internet Connection",
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "1998-11-08",
        requirements_en: {
          minimum:
            "<p><strong>Minimum:</strong> 500 mhz processor, 96mb ram, 16mb video card, Windows XP, Mouse, Keyboard, Internet Connection<br /></p>\r\n\t\t\t<p><strong>Recommended:</strong> 800 mhz processor, 128mb ram, 32mb+ video card, Windows XP, Mouse, Keyboard, Internet Connection<br /></p>",
        },
        requirements_ru: {
          minimum: "Pentium 133, 32 Мб памяти",
          recommended: "Pentium II 333 МГц, 64 Мб памяти, 3D-ускоритель",
        },
      },
      {
        platform: {
          id: 15,
          name: "PlayStation 2",
          slug: "playstation2",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 2032,
          image_background:
            "https://media.rawg.io/media/games/615/615e9fc0a325e0d87b84dad029b8b7b9.jpg",
        },
        released_at: "1998-11-08",
        requirements_en: null,
        requirements_ru: null,
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
      { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
      { platform: { id: 6, name: "Linux", slug: "linux" } },
      { platform: { id: 11, name: "SEGA", slug: "sega" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
      {
        id: 2,
        name: "Shooter",
        slug: "shooter",
        games_count: 59459,
        image_background:
          "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
      },
    ],
    stores: [
      {
        id: 20250,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 7,
        name: "Multiplayer",
        slug: "multiplayer",
        language: "eng",
        games_count: 37130,
        image_background:
          "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 42,
        name: "Great Soundtrack",
        slug: "great-soundtrack",
        language: "eng",
        games_count: 3386,
        image_background:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 118,
        name: "Story Rich",
        slug: "story-rich",
        language: "eng",
        games_count: 20561,
        image_background:
          "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
      },
      {
        id: 8,
        name: "First-Person",
        slug: "first-person",
        language: "eng",
        games_count: 30626,
        image_background:
          "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
      },
      {
        id: 32,
        name: "Sci-fi",
        slug: "sci-fi",
        language: "eng",
        games_count: 18535,
        image_background:
          "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
      },
      {
        id: 30,
        name: "FPS",
        slug: "fps",
        language: "eng",
        games_count: 12893,
        image_background:
          "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
      },
      {
        id: 4,
        name: "Funny",
        slug: "funny",
        language: "eng",
        games_count: 24154,
        image_background:
          "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
      },
      {
        id: 49,
        name: "Difficult",
        slug: "difficult",
        language: "eng",
        games_count: 13062,
        image_background:
          "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
      },
      {
        id: 193,
        name: "Classic",
        slug: "classic",
        language: "eng",
        games_count: 1783,
        image_background:
          "https://media.rawg.io/media/games/960/960b601d9541cec776c5fa42a00bf6c4.jpg",
      },
      {
        id: 74,
        name: "Retro",
        slug: "retro",
        language: "eng",
        games_count: 41033,
        image_background:
          "https://media.rawg.io/media/games/bbf/bbf8d74ab64440ad76294cff2f4d9cfa.jpg",
      },
      {
        id: 62,
        name: "Moddable",
        slug: "moddable",
        language: "eng",
        games_count: 880,
        image_background:
          "https://media.rawg.io/media/games/c22/c22d804ac753c72f2617b3708a625dec.jpg",
      },
      {
        id: 172,
        name: "Aliens",
        slug: "aliens",
        language: "eng",
        games_count: 6591,
        image_background:
          "https://media.rawg.io/media/games/ae1/ae1518c3dc1e847344661905fd2a8d16.jpg",
      },
      {
        id: 40856,
        name: "Valve Anti-Cheat enabled",
        slug: "valve-anti-cheat-enabled",
        language: "eng",
        games_count: 105,
        image_background:
          "https://media.rawg.io/media/games/78d/78dfae12fb8c5b16cd78648553071e0a.jpg",
      },
      {
        id: 243,
        name: "1990's",
        slug: "1990s",
        language: "eng",
        games_count: 2299,
        image_background:
          "https://media.rawg.io/media/games/202/20253cf9a7bd4f3ddb26bf68cf9117ba.jpg",
      },
      {
        id: 305,
        name: "Linear",
        slug: "linear",
        language: "eng",
        games_count: 5442,
        image_background:
          "https://media.rawg.io/media/games/569/569ea25d2b56bd05c7fa309ddabe81ff.jpg",
      },
      {
        id: 319,
        name: "Silent Protagonist",
        slug: "silent-protagonist",
        language: "eng",
        games_count: 91,
        image_background:
          "https://media.rawg.io/media/games/198/1988a337305e008b41d7f536ce9b73f6.jpg",
      },
      {
        id: 62349,
        name: "vr mod",
        slug: "vr-mod",
        language: "eng",
        games_count: 17,
        image_background:
          "https://media.rawg.io/media/screenshots/1bb/1bb3f78f0fe43b5d5ca2f3da5b638840.jpg",
      },
    ],
    esrb_rating: { id: 4, name: "Mature", slug: "mature" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/6c5/6c55e22185876626881b76c11922b073.jpg",
      },
      {
        id: 167835,
        image:
          "https://media.rawg.io/media/screenshots/345/3458269ae8ea44a6b8c8268d39fe36a1.jpg",
      },
      {
        id: 167836,
        image:
          "https://media.rawg.io/media/screenshots/3c5/3c55c835054009de798c0a9fa886ef8b.jpg",
      },
      {
        id: 167837,
        image:
          "https://media.rawg.io/media/screenshots/700/70005f6c84708d988a287d406cbb038c.jpg",
      },
      {
        id: 167838,
        image:
          "https://media.rawg.io/media/screenshots/755/755a2b42257cf09c7f37e902dfa08400.jpg",
      },
      {
        id: 167839,
        image:
          "https://media.rawg.io/media/screenshots/921/9213ce16eba80f51b3562a3e5d322e02.jpg",
      },
      {
        id: 167840,
        image:
          "https://media.rawg.io/media/screenshots/948/948de39d02c1f08c8c98fa10e69e87af.jpg",
      },
    ],
  },
  {
    id: 3790,
    slug: "outlast",
    name: "Outlast",
    released: "2013-09-03",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/9dd/9ddabb34840ea9227556670606cf8ea3.jpg",
    rating: 3.73,
    rating_top: 4,
    ratings: [
      { id: 4, title: "recommended", count: 945, percent: 54.03 },
      { id: 3, title: "meh", count: 352, percent: 20.13 },
      { id: 5, title: "exceptional", count: 311, percent: 17.78 },
      { id: 1, title: "skip", count: 141, percent: 8.06 },
    ],
    ratings_count: 1738,
    reviews_text_count: 9,
    added: 10053,
    added_by_status: {
      yet: 595,
      owned: 7173,
      beaten: 1437,
      toplay: 181,
      dropped: 628,
      playing: 39,
    },
    metacritic: 80,
    playtime: 3,
    suggestions_count: 469,
    updated: "2024-03-17T10:46:00",
    user_game: null,
    reviews_count: 1749,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 6,
          name: "Linux",
          slug: "linux",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 76454,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        released_at: "2013-09-03",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 12 / Fedora 20 / Steam OS<br></li><li><strong>Processor:</strong> 2.2 GHz Dual Core CPU<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> GeForce GT 750M / 9800GTX / Radeon HD 3xxx. OpenGL 3.2+<br></li><li><strong>Storage:</strong> 5 GB available space<br></li><li><strong>Sound Card:</strong> Standard</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 14 / Fedora 21 / Steam OS<br></li><li><strong>Processor:</strong> 2.8 GHz Quad Core CPU<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> 1GB NVIDIA GTX 460 / ATI Radeon HD 6850. OpenGL 4.x+<br></li><li><strong>Storage:</strong> 5 GB available space<br></li><li><strong>Sound Card:</strong> Standard</li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 5,
          name: "macOS",
          slug: "macos",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 103400,
          image_background:
            "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
        },
        released_at: "2013-09-03",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> OS X 10.9<br></li><li><strong>Processor:</strong> 2.2 GHz Dual Core CPU<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> GeForce GT 750M / Intel Iris Pro 5200 / Radeon HD 3xxx<br></li><li><strong>Storage:</strong> 5 GB available space<br></li><li><strong>Sound Card:</strong> Standard</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> OS X 10.9<br></li><li><strong>Processor:</strong> 2.8 GHz Quad Core CPU<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> 1GB NVIDIA GTX 460 / ATI Radeon HD 6850<br></li><li><strong>Storage:</strong> 5 GB available space<br></li><li><strong>Sound Card:</strong> Standard</li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 7,
          name: "Nintendo Switch",
          slug: "nintendo-switch",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5406,
          image_background:
            "https://media.rawg.io/media/games/5eb/5eb49eb2fa0738fdb5bacea557b1bc57.jpg",
        },
        released_at: "2013-09-03",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2013-09-03",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows XP / Vista / 7 / 8 - 64 bits *<br></li><li><strong>Processor:</strong> 2.2 GHz Dual Core CPU<br></li><li><strong>Memory:</strong> 2 GB RAM<br></li><li><strong>Graphics:</strong> 512 MB NVIDIA GeForce 9800GTX / ATI Radeon HD 3xxx series<br></li><li><strong>DirectX:</strong> Version 9.0c<br></li><li><strong>Network:</strong> Broadband Internet connection<br></li><li><strong>Storage:</strong> 5 GB available space<br></li><li><strong>Sound Card:</strong> DirectX Compatible<br></li><li><strong>Additional Notes:</strong> * 32 bits systems are not officially supported, but should work if configured to provide 3Gb of user-mode address space. See <a href="https://steamcommunity.com/linkfilter/?url=http://msdn.microsoft.com/en-us/windows/bb613473" target="_blank" rel="noopener"  >http://msdn.microsoft.com/en-us/windows/bb613473</a> or <a href="http://steamcommunity.com/sharedfiles/filedetails/?id=175801311" target="_blank" rel="noreferrer"  id="dynamiclink_0" >http://steamcommunity.com/sharedfiles/filedetails/?id=175801311</a></li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows Vista / 7 / 8 - 64 bits<br></li><li><strong>Processor:</strong> 2.8 GHz Quad Core CPU<br></li><li><strong>Memory:</strong> 3 GB RAM<br></li><li><strong>Graphics:</strong> 1GB NVIDIA GTX 460 / ATI Radeon HD 6850 or better<br></li><li><strong>DirectX:</strong> Version 9.0c<br></li><li><strong>Network:</strong> Broadband Internet connection<br></li><li><strong>Storage:</strong> 5 GB available space<br></li><li><strong>Sound Card:</strong> DirectX Compatible</li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 1,
          name: "Xbox One",
          slug: "xbox-one",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5590,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        released_at: "2013-09-03",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 18,
          name: "PlayStation 4",
          slug: "playstation4",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 6767,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
        released_at: "2013-09-03",
        requirements_en: null,
        requirements_ru: null,
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
      { platform: { id: 3, name: "Xbox", slug: "xbox" } },
      { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
      { platform: { id: 6, name: "Linux", slug: "linux" } },
      { platform: { id: 7, name: "Nintendo", slug: "nintendo" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
      {
        id: 51,
        name: "Indie",
        slug: "indie",
        games_count: 62257,
        image_background:
          "https://media.rawg.io/media/games/9dd/9ddabb34840ea9227556670606cf8ea3.jpg",
      },
    ],
    stores: [
      {
        id: 863662,
        store: {
          id: 11,
          name: "Epic Games",
          slug: "epic-games",
          domain: "epicgames.com",
          games_count: 1319,
          image_background:
            "https://media.rawg.io/media/games/4be/4be6a6ad0364751a96229c56bf69be59.jpg",
        },
      },
      {
        id: 4100,
        store: {
          id: 3,
          name: "PlayStation Store",
          slug: "playstation-store",
          domain: "store.playstation.com",
          games_count: 7900,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
      },
      {
        id: 7939,
        store: {
          id: 2,
          name: "Xbox Store",
          slug: "xbox-store",
          domain: "microsoft.com",
          games_count: 4813,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
      },
      {
        id: 26098,
        store: {
          id: 5,
          name: "GOG",
          slug: "gog",
          domain: "gog.com",
          games_count: 5817,
          image_background:
            "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
        },
      },
      {
        id: 20159,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 40847,
        name: "Steam Achievements",
        slug: "steam-achievements",
        language: "eng",
        games_count: 35702,
        image_background:
          "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
      },
      {
        id: 40836,
        name: "Full controller support",
        slug: "full-controller-support",
        language: "eng",
        games_count: 16746,
        image_background:
          "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
      },
      {
        id: 40849,
        name: "Steam Cloud",
        slug: "steam-cloud",
        language: "eng",
        games_count: 16769,
        image_background:
          "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 7808,
        name: "steam-trading-cards",
        slug: "steam-trading-cards",
        language: "eng",
        games_count: 7569,
        image_background:
          "https://media.rawg.io/media/games/6fc/6fcf4cd3b17c288821388e6085bb0fc9.jpg",
      },
      {
        id: 8,
        name: "First-Person",
        slug: "first-person",
        language: "eng",
        games_count: 30626,
        image_background:
          "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
      },
      {
        id: 32,
        name: "Sci-fi",
        slug: "sci-fi",
        language: "eng",
        games_count: 18535,
        image_background:
          "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
      },
      {
        id: 16,
        name: "Horror",
        slug: "horror",
        language: "eng",
        games_count: 43534,
        image_background:
          "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
      },
      {
        id: 30,
        name: "FPS",
        slug: "fps",
        language: "eng",
        games_count: 12893,
        image_background:
          "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
      },
      {
        id: 26,
        name: "Gore",
        slug: "gore",
        language: "eng",
        games_count: 5515,
        image_background:
          "https://media.rawg.io/media/games/7f6/7f6cd70ba2ad57053b4847c13569f2d8.jpg",
      },
      {
        id: 49,
        name: "Difficult",
        slug: "difficult",
        language: "eng",
        games_count: 13062,
        image_background:
          "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
      },
      {
        id: 1,
        name: "Survival",
        slug: "survival",
        language: "eng",
        games_count: 8231,
        image_background:
          "https://media.rawg.io/media/games/476/476178ef18ab0534771d099f51cdc694.jpg",
      },
      {
        id: 15,
        name: "Stealth",
        slug: "stealth",
        language: "eng",
        games_count: 6100,
        image_background:
          "https://media.rawg.io/media/games/b7d/b7d3f1715fa8381a4e780173a197a615.jpg",
      },
      {
        id: 41,
        name: "Dark",
        slug: "dark",
        language: "eng",
        games_count: 15192,
        image_background:
          "https://media.rawg.io/media/games/490/49016e06ae2103881ff6373248843069.jpg",
      },
      {
        id: 44,
        name: "Nudity",
        slug: "nudity",
        language: "eng",
        games_count: 6072,
        image_background:
          "https://media.rawg.io/media/games/260/26023c855f1769a93411d6a7ea084632.jpeg",
      },
      {
        id: 336,
        name: "controller support",
        slug: "controller-support",
        language: "eng",
        games_count: 293,
        image_background:
          "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
      },
      {
        id: 17,
        name: "Survival Horror",
        slug: "survival-horror",
        language: "eng",
        games_count: 8168,
        image_background:
          "https://media.rawg.io/media/games/ebd/ebdbb7eb52bd58b0e7fa4538d9757b60.jpg",
      },
      {
        id: 40833,
        name: "Captions available",
        slug: "captions-available",
        language: "eng",
        games_count: 1311,
        image_background:
          "https://media.rawg.io/media/games/7a2/7a2500ee8b2c0e1ff268bb4479463dea.jpg",
      },
      {
        id: 91,
        name: "Walking Simulator",
        slug: "walking-simulator",
        language: "eng",
        games_count: 6811,
        image_background:
          "https://media.rawg.io/media/games/639/639ce7d7fecbb9f0717e9fbc1180f8f8.jpg",
      },
      {
        id: 188,
        name: "Parkour",
        slug: "parkour",
        language: "eng",
        games_count: 3425,
        image_background:
          "https://media.rawg.io/media/games/c2e/c2eb6021a2596644b437e943612af25c.jpg",
      },
      {
        id: 133,
        name: "3D Vision",
        slug: "3d-vision",
        language: "eng",
        games_count: 737,
        image_background:
          "https://media.rawg.io/media/games/7cf/7cf38d2d80267c121c6d0d361e9429ce.jpg",
      },
    ],
    esrb_rating: { id: 4, name: "Mature", slug: "mature" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/9dd/9ddabb34840ea9227556670606cf8ea3.jpg",
      },
      {
        id: 32637,
        image:
          "https://media.rawg.io/media/screenshots/83f/83ff600f8e2dd8507e7961d3e9f32126.jpg",
      },
      {
        id: 32638,
        image:
          "https://media.rawg.io/media/screenshots/283/283c90039e31e07f99979ccb445cf7b7.jpg",
      },
      {
        id: 32639,
        image:
          "https://media.rawg.io/media/screenshots/03f/03f4171763bda5824da07fc087cec609.jpg",
      },
      {
        id: 32640,
        image:
          "https://media.rawg.io/media/screenshots/37a/37acd5ef186c8e018cbd64751b21f064.jpg",
      },
      {
        id: 32641,
        image:
          "https://media.rawg.io/media/screenshots/242/2426226b9eb1a7de43b8bf01ecb2c291.jpg",
      },
      {
        id: 32642,
        image:
          "https://media.rawg.io/media/screenshots/d06/d06d3baecb6da0ac9e53b40ade32d5f2.jpg",
      },
    ],
  },
  {
    id: 1447,
    slug: "deus-ex-mankind-divided",
    name: "Deus Ex: Mankind Divided",
    released: "2016-08-22",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
    rating: 3.97,
    rating_top: 4,
    ratings: [
      { id: 4, title: "recommended", count: 882, percent: 44.77 },
      { id: 5, title: "exceptional", count: 627, percent: 31.83 },
      { id: 3, title: "meh", count: 345, percent: 17.51 },
      { id: 1, title: "skip", count: 116, percent: 5.89 },
    ],
    ratings_count: 1950,
    reviews_text_count: 12,
    added: 9977,
    added_by_status: {
      yet: 903,
      owned: 6701,
      beaten: 1311,
      toplay: 395,
      dropped: 548,
      playing: 119,
    },
    metacritic: 83,
    playtime: 8,
    suggestions_count: 630,
    updated: "2024-03-13T20:19:07",
    user_game: null,
    reviews_count: 1970,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 6,
          name: "Linux",
          slug: "linux",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 76454,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        released_at: "2016-08-22",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 16.10 64-bit, SteamOS 2.0<br></li><li><strong>Processor:</strong> Intel Core i3-4130, AMD FX8350<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> 2GB Nvidia 680 (driver version: 367.57) AMD GPUs are not supported<br></li><li><strong>Storage:</strong> 67 GB available space<br></li><li><strong>Additional Notes:</strong> AMD and Intel Graphics Cards are not supported at the time of release</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 16.10 64-bit, SteamOS 2.0<br></li><li><strong>Processor:</strong> Intel Core i7-3770K<br></li><li><strong>Memory:</strong> 16 GB RAM<br></li><li><strong>Graphics:</strong> 6GB Nvidia 1060 (driver version: 367.57)<br></li><li><strong>Storage:</strong> 70 GB available space<br></li><li><strong>Additional Notes:</strong> AMD and Intel Graphics Cards are not supported at the time of release</li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2016-08-22",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows 7.1SP1 or above (64-bit Operating System Required)<br></li><li><strong>Processor:</strong> Intel Core i3-2100 or AMD equivalent<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> AMD Radeon HD 7870 (2GB) or NVIDIA GeForce GTX 660 (2GB)<br></li><li><strong>Storage:</strong> 45 GB available space</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows 10 64-bit<br></li><li><strong>Processor:</strong> Intel Core i7-3770K or AMD FX 8350 Wraith<br></li><li><strong>Memory:</strong> 16 GB RAM<br></li><li><strong>Graphics:</strong> AMD Radeon RX 480 - 1920 x 1080 or NVIDIA GTX 970 - 1920 x 1080<br></li><li><strong>Storage:</strong> 55 GB available space<br></li><li><strong>Additional Notes:</strong> 55GB HD space includes DLC</li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 1,
          name: "Xbox One",
          slug: "xbox-one",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5590,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        released_at: "2016-08-22",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 5,
          name: "macOS",
          slug: "macos",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 103400,
          image_background:
            "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
        },
        released_at: "2016-08-22",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 18,
          name: "PlayStation 4",
          slug: "playstation4",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 6767,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
        released_at: "2016-08-22",
        requirements_en: null,
        requirements_ru: null,
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
      { platform: { id: 3, name: "Xbox", slug: "xbox" } },
      { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
      { platform: { id: 6, name: "Linux", slug: "linux" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
      {
        id: 5,
        name: "RPG",
        slug: "role-playing-games-rpg",
        games_count: 55107,
        image_background:
          "https://media.rawg.io/media/games/618/618c2031a07bbff6b4f611f10b6bcdbc.jpg",
      },
    ],
    stores: [
      {
        id: 428219,
        store: {
          id: 5,
          name: "GOG",
          slug: "gog",
          domain: "gog.com",
          games_count: 5817,
          image_background:
            "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
        },
      },
      {
        id: 1486,
        store: {
          id: 3,
          name: "PlayStation Store",
          slug: "playstation-store",
          domain: "store.playstation.com",
          games_count: 7900,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
      },
      {
        id: 11939,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
      {
        id: 7125,
        store: {
          id: 2,
          name: "Xbox Store",
          slug: "xbox-store",
          domain: "microsoft.com",
          games_count: 4813,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 40836,
        name: "Full controller support",
        slug: "full-controller-support",
        language: "eng",
        games_count: 16746,
        image_background:
          "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 42,
        name: "Great Soundtrack",
        slug: "great-soundtrack",
        language: "eng",
        games_count: 3386,
        image_background:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 24,
        name: "RPG",
        slug: "rpg",
        language: "eng",
        games_count: 19865,
        image_background:
          "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
      },
      {
        id: 118,
        name: "Story Rich",
        slug: "story-rich",
        language: "eng",
        games_count: 20561,
        image_background:
          "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
      },
      {
        id: 36,
        name: "Open World",
        slug: "open-world",
        language: "eng",
        games_count: 7102,
        image_background:
          "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
      },
      {
        id: 8,
        name: "First-Person",
        slug: "first-person",
        language: "eng",
        games_count: 30626,
        image_background:
          "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
      },
      {
        id: 149,
        name: "Third Person",
        slug: "third-person",
        language: "eng",
        games_count: 10861,
        image_background:
          "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
      },
      {
        id: 32,
        name: "Sci-fi",
        slug: "sci-fi",
        language: "eng",
        games_count: 18535,
        image_background:
          "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
      },
      {
        id: 30,
        name: "FPS",
        slug: "fps",
        language: "eng",
        games_count: 12893,
        image_background:
          "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
      },
      {
        id: 15,
        name: "Stealth",
        slug: "stealth",
        language: "eng",
        games_count: 6100,
        image_background:
          "https://media.rawg.io/media/games/b7d/b7d3f1715fa8381a4e780173a197a615.jpg",
      },
      {
        id: 167,
        name: "Futuristic",
        slug: "futuristic",
        language: "eng",
        games_count: 5050,
        image_background:
          "https://media.rawg.io/media/games/90f/90fd5e569bc4c4a666c588a732124908.jpg",
      },
      {
        id: 226,
        name: "Cyberpunk",
        slug: "cyberpunk",
        language: "eng",
        games_count: 4415,
        image_background:
          "https://media.rawg.io/media/games/a12/a12f806432cb385bc286f0935c49cd14.jpg",
      },
      {
        id: 119,
        name: "Dystopian",
        slug: "dystopian",
        language: "eng",
        games_count: 2001,
        image_background:
          "https://media.rawg.io/media/games/a0e/a0ef08621301a1eab5e04fa5c96978fa.jpeg",
      },
      {
        id: 178,
        name: "Illuminati",
        slug: "illuminati",
        language: "eng",
        games_count: 315,
        image_background:
          "https://media.rawg.io/media/games/46b/46b62c37c8faf3268a0768b548e6cd9d.jpg",
      },
      {
        id: 291,
        name: "Conspiracy",
        slug: "conspiracy",
        language: "eng",
        games_count: 720,
        image_background:
          "https://media.rawg.io/media/games/338/33848ebb15683ea022c08f39f01b810f.jpg",
      },
      {
        id: 301,
        name: "Transhumanism",
        slug: "transhumanism",
        language: "eng",
        games_count: 149,
        image_background:
          "https://media.rawg.io/media/games/ca7/ca7ca88681ad87eccd12e2acc65a2f6d.jpg",
      },
    ],
    esrb_rating: { id: 4, name: "Mature", slug: "mature" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
      },
      {
        id: 12861,
        image:
          "https://media.rawg.io/media/screenshots/d4c/d4ce2d053a78f5e05cea8c99be22b135.jpg",
      },
      {
        id: 12864,
        image:
          "https://media.rawg.io/media/screenshots/515/515438994fe978193980d9b259ad7c50.jpg",
      },
      {
        id: 12865,
        image:
          "https://media.rawg.io/media/screenshots/0a0/0a0fd428643d0491c96bf29840d18d02.jpg",
      },
      {
        id: 12866,
        image:
          "https://media.rawg.io/media/screenshots/a31/a31228f42e1e5025cb35c2ab1b21bc98.jpg",
      },
      {
        id: 12869,
        image:
          "https://media.rawg.io/media/screenshots/f06/f066b272534017b7f33b09cca803bd18.jpg",
      },
      {
        id: 12873,
        image:
          "https://media.rawg.io/media/screenshots/2ae/2ae9537fd36039072f8ce5f5da592b68.jpg",
      },
    ],
  },
  {
    id: 4161,
    slug: "far-cry-3",
    name: "Far Cry 3",
    released: "2012-11-28",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/15c/15c95a4915f88a3e89c821526afe05fc.jpg",
    rating: 4.24,
    rating_top: 4,
    ratings: [
      { id: 4, title: "recommended", count: 1383, percent: 49.23 },
      { id: 5, title: "exceptional", count: 1116, percent: 39.73 },
      { id: 3, title: "meh", count: 244, percent: 8.69 },
      { id: 1, title: "skip", count: 66, percent: 2.35 },
    ],
    ratings_count: 2777,
    reviews_text_count: 17,
    added: 9941,
    added_by_status: {
      yet: 309,
      owned: 5465,
      beaten: 3354,
      toplay: 205,
      dropped: 542,
      playing: 66,
    },
    metacritic: 88,
    playtime: 16,
    suggestions_count: 626,
    updated: "2024-03-19T23:35:25",
    user_game: null,
    reviews_count: 2809,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 1,
          name: "Xbox One",
          slug: "xbox-one",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5590,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        released_at: "2012-11-28",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 14,
          name: "Xbox 360",
          slug: "xbox360",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 2796,
          image_background:
            "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
        },
        released_at: "2012-11-28",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 16,
          name: "PlayStation 3",
          slug: "playstation3",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 3168,
          image_background:
            "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
        },
        released_at: "2012-11-28",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2012-11-28",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong>Windows XP, Windows Vista and Windows 7<br>\t</li><li><strong>Processor:</strong>Intel Core®2 Duo E6700 @ 2.6 GHz or AMD Athlon64 X2 6000+ @ 3.0Ghz or better<br>\t</li><li><strong>Memory:</strong>4 GB RAM<br>\t</li><li><strong>Graphics:</strong>512MB Video RAM (1GB Video RAM), DirectX9c (DirectX11) Shader Model 3.0 (Shader Model 5.0)<br>\t</li><li><strong>DirectX®:</strong>9.0c<br>\t</li><li><strong>Hard Drive:</strong>15 GB HD space<br>\t</li><li><strong>Sound:</strong>DirectX Compatible  (Recommended Surround Sound 5.1 capable)<br>\t</li><li><strong>Other Requirements:</strong>Broadband Internet connection<br>\t</li><li><strong>Additional:</strong>*Supported Video Cards at Time of Release: AMD Radeon™ HD 2900 / 3000 / 4000 / 5000 / 6000 / 7000 series, NVIDIA® GeForce® 8800 GTX / 9 / 200 / 400 / 500 / 600 series. Laptop versions of these cards may work, but are not supported. These chipsets are the only ones that will run this game. For the most up-to-date minimum requirement listings, please visit the FAQ on our support website at <a href="https://steamcommunity.com/linkfilter/?url=http://support.ubi.com" target="_blank" rel="noopener"  >http://support.ubi.com</a>.<br>Requires a UPlay account</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>Memory:</strong>4 GB RAM<br>\t</li><li><strong>Graphics:</strong>512MB Video RAM (1GB Video RAM), DirectX9c (DirectX11) Shader Model 3.0 (Shader Model 5.0)<br>\t</li><li><strong>DirectX®:</strong>11<br>\t</li><li><strong>Hard Drive:</strong>15 GB HD space<br>\t</li><li><strong>Sound:</strong>DirectX Compatible  (Recommended Surround Sound 5.1 capable)<br>\t</li><li><strong>Other Requirements:</strong>Broadband Internet connection<br>\t</li><li><strong>Additional:</strong>*Supported Video Cards at Time of Release: AMD Radeon™ HD 2900 / 3000 / 4000 / 5000 / 6000 / 7000 series, NVIDIA® GeForce® 8800 GTX / 9 / 200 / 400 / 500 / 600 series. Laptop versions of these cards may work, but are not supported. These chipsets are the only ones that will run this game. For the most up-to-date minimum requirement listings, please visit the FAQ on our support website at <a href="https://steamcommunity.com/linkfilter/?url=http://support.ubi.com" target="_blank" rel="noopener"  >http://support.ubi.com</a>.<br>Requires a UPlay account</li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 18,
          name: "PlayStation 4",
          slug: "playstation4",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 6767,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
        released_at: "2012-11-28",
        requirements_en: null,
        requirements_ru: null,
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
      { platform: { id: 3, name: "Xbox", slug: "xbox" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
      {
        id: 2,
        name: "Shooter",
        slug: "shooter",
        games_count: 59459,
        image_background:
          "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
      },
    ],
    stores: [
      {
        id: 465959,
        store: {
          id: 2,
          name: "Xbox Store",
          slug: "xbox-store",
          domain: "microsoft.com",
          games_count: 4813,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
      },
      {
        id: 4486,
        store: {
          id: 3,
          name: "PlayStation Store",
          slug: "playstation-store",
          domain: "store.playstation.com",
          games_count: 7900,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
      },
      {
        id: 19655,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
      {
        id: 33907,
        store: {
          id: 7,
          name: "Xbox 360 Store",
          slug: "xbox360",
          domain: "marketplace.xbox.com",
          games_count: 1912,
          image_background:
            "https://media.rawg.io/media/games/d1a/d1a2e99ade53494c6330a0ed945fe823.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 7,
        name: "Multiplayer",
        slug: "multiplayer",
        language: "eng",
        games_count: 37130,
        image_background:
          "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 24,
        name: "RPG",
        slug: "rpg",
        language: "eng",
        games_count: 19865,
        image_background:
          "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
      },
      {
        id: 18,
        name: "Co-op",
        slug: "co-op",
        language: "eng",
        games_count: 11072,
        image_background:
          "https://media.rawg.io/media/games/511/5118aff5091cb3efec399c808f8c598f.jpg",
      },
      {
        id: 118,
        name: "Story Rich",
        slug: "story-rich",
        language: "eng",
        games_count: 20561,
        image_background:
          "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
      },
      {
        id: 36,
        name: "Open World",
        slug: "open-world",
        language: "eng",
        games_count: 7102,
        image_background:
          "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
      },
      {
        id: 411,
        name: "cooperative",
        slug: "cooperative",
        language: "eng",
        games_count: 4703,
        image_background:
          "https://media.rawg.io/media/games/73e/73eecb8909e0c39fb246f457b5d6cbbe.jpg",
      },
      {
        id: 8,
        name: "First-Person",
        slug: "first-person",
        language: "eng",
        games_count: 30626,
        image_background:
          "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
      },
      {
        id: 40845,
        name: "Partial Controller Support",
        slug: "partial-controller-support",
        language: "eng",
        games_count: 11229,
        image_background:
          "https://media.rawg.io/media/games/62c/62c7c8b28a27b83680b22fb9d33fc619.jpg",
      },
      {
        id: 30,
        name: "FPS",
        slug: "fps",
        language: "eng",
        games_count: 12893,
        image_background:
          "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
      },
      {
        id: 9,
        name: "Online Co-Op",
        slug: "online-co-op",
        language: "eng",
        games_count: 5203,
        image_background:
          "https://media.rawg.io/media/games/48c/48cb04ca483be865e3a83119c94e6097.jpg",
      },
      {
        id: 37,
        name: "Sandbox",
        slug: "sandbox",
        language: "eng",
        games_count: 6607,
        image_background:
          "https://media.rawg.io/media/games/7cf/7cfc9220b401b7a300e409e539c9afd5.jpg",
      },
      {
        id: 1,
        name: "Survival",
        slug: "survival",
        language: "eng",
        games_count: 8231,
        image_background:
          "https://media.rawg.io/media/games/476/476178ef18ab0534771d099f51cdc694.jpg",
      },
      {
        id: 15,
        name: "Stealth",
        slug: "stealth",
        language: "eng",
        games_count: 6100,
        image_background:
          "https://media.rawg.io/media/games/b7d/b7d3f1715fa8381a4e780173a197a615.jpg",
      },
      {
        id: 44,
        name: "Nudity",
        slug: "nudity",
        language: "eng",
        games_count: 6072,
        image_background:
          "https://media.rawg.io/media/games/260/26023c855f1769a93411d6a7ea084632.jpeg",
      },
      {
        id: 165,
        name: "Colorful",
        slug: "colorful",
        language: "eng",
        games_count: 20827,
        image_background:
          "https://media.rawg.io/media/games/3e3/3e355e1b8a5ee47f4c76e28e3055236d.jpg",
      },
      {
        id: 125,
        name: "Crafting",
        slug: "crafting",
        language: "eng",
        games_count: 3896,
        image_background:
          "https://media.rawg.io/media/games/152/152e788b7504aa2753c86dae912fb34c.jpg",
      },
      {
        id: 181,
        name: "Hunting",
        slug: "hunting",
        language: "eng",
        games_count: 901,
        image_background:
          "https://media.rawg.io/media/screenshots/599/59905f4cc9614a53f36989a26098d13c.jpg",
      },
    ],
    esrb_rating: { id: 4, name: "Mature", slug: "mature" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/15c/15c95a4915f88a3e89c821526afe05fc.jpg",
      },
      {
        id: 161732,
        image:
          "https://media.rawg.io/media/screenshots/98e/98e4c2a0c3e84b3d2718f8801bba0fcc.jpg",
      },
      {
        id: 161733,
        image:
          "https://media.rawg.io/media/screenshots/87e/87e9ca5542b3e8da43d488c9252e20fe.jpg",
      },
      {
        id: 161734,
        image:
          "https://media.rawg.io/media/screenshots/0a9/0a9ff8bbfa49024159bcd884ce7128c3.jpg",
      },
      {
        id: 161735,
        image:
          "https://media.rawg.io/media/screenshots/971/971d3582a42ede482b9d90b47b50ac32.jpg",
      },
      {
        id: 161736,
        image:
          "https://media.rawg.io/media/screenshots/174/174333d080f475d5718219ef6e01d3f4.jpg",
      },
      {
        id: 161737,
        image:
          "https://media.rawg.io/media/screenshots/03d/03dcdb4292bcc6ccc2719c88202f4965.jpg",
      },
    ],
  },
  {
    id: 10754,
    slug: "bioshock-remastered",
    name: "BioShock Remastered",
    released: "2016-09-15",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/be0/be01c3d7d8795a45615da139322ca080.jpg",
    rating: 4.24,
    rating_top: 5,
    ratings: [
      { id: 5, title: "exceptional", count: 642, percent: 46.76 },
      { id: 4, title: "recommended", count: 554, percent: 40.35 },
      { id: 3, title: "meh", count: 109, percent: 7.94 },
      { id: 1, title: "skip", count: 68, percent: 4.95 },
    ],
    ratings_count: 1359,
    reviews_text_count: 8,
    added: 9866,
    added_by_status: {
      yet: 699,
      owned: 7420,
      beaten: 1185,
      toplay: 158,
      dropped: 299,
      playing: 105,
    },
    metacritic: null,
    playtime: 4,
    suggestions_count: 258,
    updated: "2024-03-11T16:06:18",
    user_game: null,
    reviews_count: 1373,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 18,
          name: "PlayStation 4",
          slug: "playstation4",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 6767,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
        released_at: "2016-09-15",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 7,
          name: "Nintendo Switch",
          slug: "nintendo-switch",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5406,
          image_background:
            "https://media.rawg.io/media/games/5eb/5eb49eb2fa0738fdb5bacea557b1bc57.jpg",
        },
        released_at: "2016-09-15",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 5,
          name: "macOS",
          slug: "macos",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 103400,
          image_background:
            "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
        },
        released_at: "2016-09-15",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2016-09-15",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows 7 Service Pack 1 64-bit. Platform Update for Windows 7 SP1 and Windows Server 2008 R2 SP1<br></li><li><strong>Processor:</strong> Intel E6750 Core 2 Duo 2.66 GHz / AMD Athlon X2 2.7 GHZ<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> 2GB AMD Radeon HD 7770 / 2GB NVIDIA GeForce GTX 670<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Storage:</strong> 25 GB available space<br></li><li><strong>Sound Card:</strong> DirectX Compatible Sound Device<br></li><li><strong>Additional Notes:</strong> Other Requirements: Software installations required including DirectX and Microsoft Visual C++ 2010 &amp; 2012 Redistributable Package. <strong>Some system components such as mobile chipsets, integrated, and AGP graphics cards may be incompatible. Unlisted specifications may not be supported by publisher. </strong></li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows 7 Service Pack 1 64-bit. Platform Update for Windows 7 SP1 and Windows Server 2008 R2 SP1<br></li><li><strong>Processor:</strong> 3GHz Quad-Core<br></li><li><strong>Memory:</strong> 8 GB RAM<br></li><li><strong>Graphics:</strong> 2GB ATI Radeon HD 7970, 2GB NVIDIA GeForce GTX 770 or better<br></li><li><strong>DirectX:</strong> Version 11<br></li><li><strong>Storage:</strong> 25 GB available space<br></li><li><strong>Sound Card:</strong> DirectX Compatible Sound Device</li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 1,
          name: "Xbox One",
          slug: "xbox-one",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5590,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        released_at: "2016-09-15",
        requirements_en: null,
        requirements_ru: null,
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
      { platform: { id: 3, name: "Xbox", slug: "xbox" } },
      { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
      { platform: { id: 7, name: "Nintendo", slug: "nintendo" } },
    ],
    genres: [
      {
        id: 2,
        name: "Shooter",
        slug: "shooter",
        games_count: 59459,
        image_background:
          "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
      },
    ],
    stores: [
      {
        id: 828867,
        store: {
          id: 11,
          name: "Epic Games",
          slug: "epic-games",
          domain: "epicgames.com",
          games_count: 1319,
          image_background:
            "https://media.rawg.io/media/games/4be/4be6a6ad0364751a96229c56bf69be59.jpg",
        },
      },
      {
        id: 440395,
        store: {
          id: 3,
          name: "PlayStation Store",
          slug: "playstation-store",
          domain: "store.playstation.com",
          games_count: 7900,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
      },
      {
        id: 525094,
        store: {
          id: 6,
          name: "Nintendo Store",
          slug: "nintendo",
          domain: "nintendo.com",
          games_count: 8966,
          image_background:
            "https://media.rawg.io/media/games/737/737ea5662211d2e0bbd6f5989189e4f1.jpg",
        },
      },
      {
        id: 11782,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
      {
        id: 315633,
        store: {
          id: 4,
          name: "App Store",
          slug: "apple-appstore",
          domain: "apps.apple.com",
          games_count: 75436,
          image_background:
            "https://media.rawg.io/media/games/f46/f466571d536f2e3ea9e815ad17177501.jpg",
        },
      },
      {
        id: 276209,
        store: {
          id: 5,
          name: "GOG",
          slug: "gog",
          domain: "gog.com",
          games_count: 5817,
          image_background:
            "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 40847,
        name: "Steam Achievements",
        slug: "steam-achievements",
        language: "eng",
        games_count: 35702,
        image_background:
          "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
      },
      {
        id: 40836,
        name: "Full controller support",
        slug: "full-controller-support",
        language: "eng",
        games_count: 16746,
        image_background:
          "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
      },
      {
        id: 40849,
        name: "Steam Cloud",
        slug: "steam-cloud",
        language: "eng",
        games_count: 16769,
        image_background:
          "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 42,
        name: "Great Soundtrack",
        slug: "great-soundtrack",
        language: "eng",
        games_count: 3386,
        image_background:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 24,
        name: "RPG",
        slug: "rpg",
        language: "eng",
        games_count: 19865,
        image_background:
          "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
      },
      {
        id: 118,
        name: "Story Rich",
        slug: "story-rich",
        language: "eng",
        games_count: 20561,
        image_background:
          "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
      },
      {
        id: 8,
        name: "First-Person",
        slug: "first-person",
        language: "eng",
        games_count: 30626,
        image_background:
          "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
      },
      {
        id: 32,
        name: "Sci-fi",
        slug: "sci-fi",
        language: "eng",
        games_count: 18535,
        image_background:
          "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
      },
      {
        id: 16,
        name: "Horror",
        slug: "horror",
        language: "eng",
        games_count: 43534,
        image_background:
          "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
      },
      {
        id: 30,
        name: "FPS",
        slug: "fps",
        language: "eng",
        games_count: 12893,
        image_background:
          "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
      },
      {
        id: 193,
        name: "Classic",
        slug: "classic",
        language: "eng",
        games_count: 1783,
        image_background:
          "https://media.rawg.io/media/games/960/960b601d9541cec776c5fa42a00bf6c4.jpg",
      },
      {
        id: 41,
        name: "Dark",
        slug: "dark",
        language: "eng",
        games_count: 15192,
        image_background:
          "https://media.rawg.io/media/games/490/49016e06ae2103881ff6373248843069.jpg",
      },
      {
        id: 119,
        name: "Dystopian",
        slug: "dystopian",
        language: "eng",
        games_count: 2001,
        image_background:
          "https://media.rawg.io/media/games/a0e/a0ef08621301a1eab5e04fa5c96978fa.jpeg",
      },
      {
        id: 271,
        name: "Remake",
        slug: "remake",
        language: "eng",
        games_count: 1695,
        image_background:
          "https://media.rawg.io/media/games/297/297a51aa1f0999016d5a35e2e1d6d8ab.jpg",
      },
      {
        id: 154,
        name: "Steampunk",
        slug: "steampunk",
        language: "eng",
        games_count: 1169,
        image_background:
          "https://media.rawg.io/media/games/4e6/4e6e8e7f50c237d76f38f3c885dae3d2.jpg",
      },
      {
        id: 40834,
        name: "Commentary available",
        slug: "commentary-available",
        language: "eng",
        games_count: 266,
        image_background:
          "https://media.rawg.io/media/games/594/5949baae74fe9e399adbce0c44e28783.jpg",
      },
      {
        id: 208,
        name: "Alternate History",
        slug: "alternate-history",
        language: "eng",
        games_count: 1675,
        image_background:
          "https://media.rawg.io/media/games/f14/f1422eacab98c5f85a5123da4e9d9e89.jpg",
      },
      {
        id: 250,
        name: "Underwater",
        slug: "underwater",
        language: "eng",
        games_count: 2048,
        image_background:
          "https://media.rawg.io/media/games/386/3869a2540b5fd39776fd640686610d05.jpg",
      },
    ],
    esrb_rating: { id: 4, name: "Mature", slug: "mature" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/be0/be01c3d7d8795a45615da139322ca080.jpg",
      },
      {
        id: 84960,
        image:
          "https://media.rawg.io/media/screenshots/c70/c709280e11aabec614d0aafb5779114a.jpg",
      },
      {
        id: 84961,
        image:
          "https://media.rawg.io/media/screenshots/414/41463563e721aa62c605cd0bf8350af3.jpg",
      },
      {
        id: 84962,
        image:
          "https://media.rawg.io/media/screenshots/76d/76d11a76541bcfa63de3ef8fe5a5a668.jpg",
      },
      {
        id: 84963,
        image:
          "https://media.rawg.io/media/screenshots/25a/25ac07efdbf90a2d7e626353ebadc565.jpg",
      },
      {
        id: 84964,
        image:
          "https://media.rawg.io/media/screenshots/0f6/0f69943b1d81710ba2fdcac27d248ca9.jpg",
      },
    ],
  },
  {
    id: 11936,
    slug: "half-life-2-deathmatch",
    name: "Half-Life 2: Deathmatch",
    released: "2004-11-01",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
    rating: 3.29,
    rating_top: 4,
    ratings: [
      { id: 4, title: "recommended", count: 356, percent: 40.36 },
      { id: 3, title: "meh", count: 230, percent: 26.08 },
      { id: 1, title: "skip", count: 174, percent: 19.73 },
      { id: 5, title: "exceptional", count: 122, percent: 13.83 },
    ],
    ratings_count: 880,
    reviews_text_count: 2,
    added: 9805,
    added_by_status: {
      yet: 532,
      owned: 8478,
      beaten: 316,
      toplay: 23,
      dropped: 444,
      playing: 12,
    },
    metacritic: null,
    playtime: 1,
    suggestions_count: 196,
    updated: "2024-03-17T16:17:42",
    user_game: null,
    reviews_count: 882,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 6,
          name: "Linux",
          slug: "linux",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 76454,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        released_at: "2004-11-01",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 5,
          name: "macOS",
          slug: "macos",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 103400,
          image_background:
            "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
        },
        released_at: "2004-11-01",
        requirements_en: {
          minimum:
            "<strong>Minimum: </strong>OS X version Leopard 10.5.8, Snow Leopard 10.6.3, 1GB RAM, NVIDIA GeForce 8 or higher, ATI X1600 or higher, or Intel HD 3000 or higher Mouse, Keyboard, Internet Connection",
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2004-11-01",
        requirements_en: {
          minimum:
            "\r\n\t\t\t<p><strong>Minimum:</strong> 1.2 GHz Processor, 256MB RAM, DirectX 7 level graphics card, Windows XP, Mouse, Keyboard, Internet Connection<br /></p>\r\n\t\t\t<p><strong>Recommended:</strong> 2.4 GHz Processor, 512MB RAM, DirectX 9 level graphics card, Windows XP, Mouse, Keyboard, Internet Connection<br /></p>\r\n\t\t\t",
        },
        requirements_ru: null,
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
      { platform: { id: 6, name: "Linux", slug: "linux" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
    ],
    stores: [
      {
        id: 13105,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 7,
        name: "Multiplayer",
        slug: "multiplayer",
        language: "eng",
        games_count: 37130,
        image_background:
          "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 18,
        name: "Co-op",
        slug: "co-op",
        language: "eng",
        games_count: 11072,
        image_background:
          "https://media.rawg.io/media/games/511/5118aff5091cb3efec399c808f8c598f.jpg",
      },
      {
        id: 411,
        name: "cooperative",
        slug: "cooperative",
        language: "eng",
        games_count: 4703,
        image_background:
          "https://media.rawg.io/media/games/73e/73eecb8909e0c39fb246f457b5d6cbbe.jpg",
      },
      {
        id: 8,
        name: "First-Person",
        slug: "first-person",
        language: "eng",
        games_count: 30626,
        image_background:
          "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
      },
      {
        id: 32,
        name: "Sci-fi",
        slug: "sci-fi",
        language: "eng",
        games_count: 18535,
        image_background:
          "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
      },
      {
        id: 30,
        name: "FPS",
        slug: "fps",
        language: "eng",
        games_count: 12893,
        image_background:
          "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
      },
      {
        id: 4,
        name: "Funny",
        slug: "funny",
        language: "eng",
        games_count: 24154,
        image_background:
          "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
      },
      {
        id: 79,
        name: "Free to Play",
        slug: "free-to-play",
        language: "eng",
        games_count: 6248,
        image_background:
          "https://media.rawg.io/media/games/dcb/dcbb67f371a9a28ea38ffd73ee0f53f3.jpg",
      },
      {
        id: 114,
        name: "Physics",
        slug: "physics",
        language: "eng",
        games_count: 18796,
        image_background:
          "https://media.rawg.io/media/screenshots/f5a/f5abab52c4d606551cd5ec3ab709e501.jpg",
      },
      {
        id: 40856,
        name: "Valve Anti-Cheat enabled",
        slug: "valve-anti-cheat-enabled",
        language: "eng",
        games_count: 105,
        image_background:
          "https://media.rawg.io/media/games/78d/78dfae12fb8c5b16cd78648553071e0a.jpg",
      },
      {
        id: 170,
        name: "Competitive",
        slug: "competitive",
        language: "eng",
        games_count: 1116,
        image_background:
          "https://media.rawg.io/media/games/9fb/9fbaea2168caea1f806546dfdaaeb1da.jpg",
      },
      {
        id: 40839,
        name: "Includes Source SDK",
        slug: "includes-source-sdk",
        language: "eng",
        games_count: 62,
        image_background:
          "https://media.rawg.io/media/screenshots/ae8/ae873e0da5973a9896bdf5ebce1e9bf4.jpg",
      },
    ],
    esrb_rating: { id: 4, name: "Mature", slug: "mature" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
      },
      {
        id: 98731,
        image:
          "https://media.rawg.io/media/screenshots/8e7/8e7039ef354447c969a9fe82329ca50e.jpg",
      },
      {
        id: 98732,
        image:
          "https://media.rawg.io/media/screenshots/0d0/0d0ff34c7bef12783cfe7c6dda7284b2.jpg",
      },
      {
        id: 98733,
        image:
          "https://media.rawg.io/media/screenshots/853/85349bbb51ad92fc351b61a89b6db19b.jpg",
      },
      {
        id: 98734,
        image:
          "https://media.rawg.io/media/screenshots/1fc/1fcc71ae3d931406dbc4b33e3446d457.jpg",
      },
      {
        id: 98735,
        image:
          "https://media.rawg.io/media/screenshots/c11/c11d517937d7261e9d357709ca3c3cc0.jpg",
      },
    ],
  },
  {
    id: 58134,
    slug: "marvels-spider-man",
    name: "Marvel's Spider-Man",
    released: "2018-09-07",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/9aa/9aa42d16d425fa6f179fc9dc2f763647.jpg",
    rating: 4.45,
    rating_top: 5,
    ratings: [
      { id: 5, title: "exceptional", count: 2065, percent: 58.32 },
      { id: 4, title: "recommended", count: 1185, percent: 33.47 },
      { id: 3, title: "meh", count: 208, percent: 5.87 },
      { id: 1, title: "skip", count: 83, percent: 2.34 },
    ],
    ratings_count: 3467,
    reviews_text_count: 56,
    added: 9794,
    added_by_status: {
      yet: 356,
      owned: 4824,
      beaten: 3187,
      toplay: 951,
      dropped: 210,
      playing: 266,
    },
    metacritic: 87,
    playtime: 6,
    suggestions_count: 583,
    updated: "2024-03-20T07:32:14",
    user_game: null,
    reviews_count: 3541,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 18,
          name: "PlayStation 4",
          slug: "playstation4",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 6767,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
        released_at: "2018-09-07",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2018-09-07",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 187,
          name: "PlayStation 5",
          slug: "playstation5",
          image: null,
          year_end: null,
          year_start: 2020,
          games_count: 1015,
          image_background:
            "https://media.rawg.io/media/games/909/909974d1c7863c2027241e265fe7011f.jpg",
        },
        released_at: "2018-09-07",
        requirements_en: null,
        requirements_ru: null,
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
    ],
    stores: [
      {
        id: 958559,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
      {
        id: 46837,
        store: {
          id: 3,
          name: "PlayStation Store",
          slug: "playstation-store",
          domain: "store.playstation.com",
          games_count: 7900,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 36,
        name: "Open World",
        slug: "open-world",
        language: "eng",
        games_count: 7102,
        image_background:
          "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
      },
      {
        id: 6,
        name: "Exploration",
        slug: "exploration",
        language: "eng",
        games_count: 21834,
        image_background:
          "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
      },
      {
        id: 69,
        name: "Action-Adventure",
        slug: "action-adventure",
        language: "eng",
        games_count: 15512,
        image_background:
          "https://media.rawg.io/media/games/253/2534a46f3da7fa7c315f1387515ca393.jpg",
      },
      {
        id: 37796,
        name: "exclusive",
        slug: "exclusive",
        language: "eng",
        games_count: 4496,
        image_background:
          "https://media.rawg.io/media/games/f24/f2493ea338fe7bd3c7d73750a85a0959.jpeg",
      },
      {
        id: 110,
        name: "Cinematic",
        slug: "cinematic",
        language: "eng",
        games_count: 1881,
        image_background:
          "https://media.rawg.io/media/games/9aa/9aa42d16d425fa6f179fc9dc2f763647.jpg",
      },
      {
        id: 203,
        name: "Beat 'em up",
        slug: "beat-em-up",
        language: "eng",
        games_count: 2933,
        image_background:
          "https://media.rawg.io/media/games/d1a/d1a1202a378607b6c635c8f18ace95dd.jpg",
      },
      {
        id: 37797,
        name: "true exclusive",
        slug: "true-exclusive",
        language: "eng",
        games_count: 3983,
        image_background:
          "https://media.rawg.io/media/screenshots/135/135a00cf33b0c860fcb366747e95b89b.jpg",
      },
      {
        id: 478,
        name: "3rd-Person Perspective",
        slug: "3rd-person-perspective",
        language: "eng",
        games_count: 87,
        image_background:
          "https://media.rawg.io/media/games/21c/21cc15d233117c6809ec86870559e105.jpg",
      },
      {
        id: 268,
        name: "Comic Book",
        slug: "comic-book",
        language: "eng",
        games_count: 1203,
        image_background:
          "https://media.rawg.io/media/games/a87/a8743bdee8627c55bb9f2f01b9136ac1.jpg",
      },
      {
        id: 234,
        name: "Superhero",
        slug: "superhero",
        language: "eng",
        games_count: 1319,
        image_background:
          "https://media.rawg.io/media/games/9e5/9e52a797f049e701d4eee84774a99007.jpg",
      },
      {
        id: 78,
        name: "America",
        slug: "america",
        language: "eng",
        games_count: 597,
        image_background:
          "https://media.rawg.io/media/games/909/909974d1c7863c2027241e265fe7011f.jpg",
      },
      {
        id: 43369,
        name: "new york",
        slug: "new-york-2",
        language: "eng",
        games_count: 2,
        image_background:
          "https://media.rawg.io/media/screenshots/e3c/e3cc9dcd0d4779fcdc08a01d150ea7f9.jpg",
      },
    ],
    esrb_rating: { id: 3, name: "Teen", slug: "teen" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/9aa/9aa42d16d425fa6f179fc9dc2f763647.jpg",
      },
      {
        id: 1325929,
        image:
          "https://media.rawg.io/media/screenshots/331/331ba5164c5c53a5d59aad3fe9771ac7.jpg",
      },
      {
        id: 1325930,
        image:
          "https://media.rawg.io/media/screenshots/a15/a15b42bd8a652a3733c6ad419ebb24bd.jpg",
      },
      {
        id: 1325931,
        image:
          "https://media.rawg.io/media/screenshots/150/150589c127b28f287f992c2bd426b443.jpg",
      },
      {
        id: 1325932,
        image:
          "https://media.rawg.io/media/screenshots/f52/f526988f895b554dccf68767557a8518.jpg",
      },
      {
        id: 1325958,
        image:
          "https://media.rawg.io/media/screenshots/745/74589db2dee21101d7af690976fca902.jpg",
      },
      {
        id: 1325959,
        image:
          "https://media.rawg.io/media/screenshots/090/09063845f2efe6d0b9bc908e2652c1e1.jpeg",
      },
    ],
  },
  {
    id: 4332,
    slug: "spec-ops-the-line",
    name: "Spec Ops: The Line",
    released: "2012-06-26",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/b49/b4912b5dbfc7ed8927b65f05b8507f6c.jpg",
    rating: 4.09,
    rating_top: 4,
    ratings: [
      { id: 4, title: "recommended", count: 907, percent: 47.99 },
      { id: 5, title: "exceptional", count: 675, percent: 35.71 },
      { id: 3, title: "meh", count: 212, percent: 11.22 },
      { id: 1, title: "skip", count: 96, percent: 5.08 },
    ],
    ratings_count: 1869,
    reviews_text_count: 12,
    added: 9781,
    added_by_status: {
      yet: 494,
      owned: 6907,
      beaten: 1822,
      toplay: 188,
      dropped: 332,
      playing: 38,
    },
    metacritic: 76,
    playtime: 5,
    suggestions_count: 532,
    updated: "2024-03-13T20:37:54",
    user_game: null,
    reviews_count: 1890,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 14,
          name: "Xbox 360",
          slug: "xbox360",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 2796,
          image_background:
            "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
        },
        released_at: "2012-06-26",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 16,
          name: "PlayStation 3",
          slug: "playstation3",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 3168,
          image_background:
            "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
        },
        released_at: "2012-06-26",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 1,
          name: "Xbox One",
          slug: "xbox-one",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5590,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        released_at: "2012-06-26",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2012-06-26",
        requirements_en: {
          minimum:
            '<strong> Minimum</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows XP SP3<br>                </li><li><strong>Processor:</strong>  Intel Core 2 Duo @ 2Ghz / AMD Athlon 64 X2 equivalent<br>                </li><li><strong>Memory:</strong> 2 GB RAM<br>                </li><li><strong>Hard Disk Space:</strong>  6 GB free                <br>                </li><li><strong>Video Card:</strong> NVIDIA GeForce 8600 / ATI Radeon HD 2600XT (256 MB memory) <br>                </li><li><strong>DirectX®:</strong> 9.0<br>                </li><li><strong>Sound:</strong> DirectX Compatible<br></li><li><strong>Additional:</strong> Initial installation requires one-time internet connection for Steam authentication; software installations required (included with the game) include: STEAM Client, Microsoft Direct X, Visual C++ 2008 Redistributable, Visual C++ 2005 SP1 RedistributableCompatible      <br> </li></ul>',
          recommended:
            '<strong> Recommended</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows Vista/7<br>                </li><li><strong>Processor:</strong> 2.4 GHz Quad Core processor<br>                </li><li><strong>Memory:</strong> 3 GB RAM<br>                </li><li><strong>Hard Disk Space:</strong> 10 GB free               <br>                </li><li><strong>Video Card:</strong> NVIDIA GeForce 9800 GTX/ATI Radeon HD 4850 (512+ MB memory) <br>                </li><li><strong>DirectX®:</strong> 9.0<br>                </li><li><strong>Sound:</strong> DirectX Compatible     <br></li><li><strong>Additional:</strong> Initial installation requires one-time internet connection for Steam authentication; software installations required (included with the game) include: STEAM Client, Microsoft Direct X, Visual C++ 2008 Redistributable, Visual C++ 2005 SP1 RedistributableCompatible                  </li></ul>',
        },
        requirements_ru: {
          minimum:
            "Core 2 Duo/Athlon 64 X2 2 ГГц,2 Гб памяти,GeForce 8600/Radeon HD 2600,6 Гб на винчестере,интернет-соединение",
          recommended:
            "Core 2 Quad/Phenom X4 2.4 ГГц,3 Гб памяти,GeForce 9800 GTX/Radeon HD 4850,10 Гб на винчестере,интернет-соединение",
        },
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
      { platform: { id: 3, name: "Xbox", slug: "xbox" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
      {
        id: 2,
        name: "Shooter",
        slug: "shooter",
        games_count: 59459,
        image_background:
          "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
      },
    ],
    stores: [
      {
        id: 465902,
        store: {
          id: 2,
          name: "Xbox Store",
          slug: "xbox-store",
          domain: "microsoft.com",
          games_count: 4813,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
      },
      {
        id: 4660,
        store: {
          id: 3,
          name: "PlayStation Store",
          slug: "playstation-store",
          domain: "store.playstation.com",
          games_count: 7900,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
      },
      {
        id: 19791,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
      {
        id: 33912,
        store: {
          id: 7,
          name: "Xbox 360 Store",
          slug: "xbox360",
          domain: "marketplace.xbox.com",
          games_count: 1912,
          image_background:
            "https://media.rawg.io/media/games/d1a/d1a2e99ade53494c6330a0ed945fe823.jpg",
        },
      },
      {
        id: 320890,
        store: {
          id: 5,
          name: "GOG",
          slug: "gog",
          domain: "gog.com",
          games_count: 5817,
          image_background:
            "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 40847,
        name: "Steam Achievements",
        slug: "steam-achievements",
        language: "eng",
        games_count: 35702,
        image_background:
          "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
      },
      {
        id: 7,
        name: "Multiplayer",
        slug: "multiplayer",
        language: "eng",
        games_count: 37130,
        image_background:
          "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
      },
      {
        id: 40836,
        name: "Full controller support",
        slug: "full-controller-support",
        language: "eng",
        games_count: 16746,
        image_background:
          "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
      },
      {
        id: 40849,
        name: "Steam Cloud",
        slug: "steam-cloud",
        language: "eng",
        games_count: 16769,
        image_background:
          "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 42,
        name: "Great Soundtrack",
        slug: "great-soundtrack",
        language: "eng",
        games_count: 3386,
        image_background:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 18,
        name: "Co-op",
        slug: "co-op",
        language: "eng",
        games_count: 11072,
        image_background:
          "https://media.rawg.io/media/games/511/5118aff5091cb3efec399c808f8c598f.jpg",
      },
      {
        id: 118,
        name: "Story Rich",
        slug: "story-rich",
        language: "eng",
        games_count: 20561,
        image_background:
          "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
      },
      {
        id: 149,
        name: "Third Person",
        slug: "third-person",
        language: "eng",
        games_count: 10861,
        image_background:
          "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
      },
      {
        id: 16,
        name: "Horror",
        slug: "horror",
        language: "eng",
        games_count: 43534,
        image_background:
          "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
      },
      {
        id: 40850,
        name: "Steam Leaderboards",
        slug: "steam-leaderboards",
        language: "eng",
        games_count: 6539,
        image_background:
          "https://media.rawg.io/media/games/59a/59a3ebcba3d08c51532c6ca877aff256.jpg",
      },
      {
        id: 150,
        name: "Third-Person Shooter",
        slug: "third-person-shooter",
        language: "eng",
        games_count: 3266,
        image_background:
          "https://media.rawg.io/media/games/ebd/ebdbb7eb52bd58b0e7fa4538d9757b60.jpg",
      },
      {
        id: 11669,
        name: "stats",
        slug: "stats",
        language: "eng",
        games_count: 4821,
        image_background:
          "https://media.rawg.io/media/games/f52/f5206d55f918edf8ee07803101106fa6.jpg",
      },
      {
        id: 41,
        name: "Dark",
        slug: "dark",
        language: "eng",
        games_count: 15192,
        image_background:
          "https://media.rawg.io/media/games/490/49016e06ae2103881ff6373248843069.jpg",
      },
      {
        id: 40832,
        name: "Cross-Platform Multiplayer",
        slug: "cross-platform-multiplayer",
        language: "eng",
        games_count: 2509,
        image_background:
          "https://media.rawg.io/media/screenshots/c97/c97b943741f5fbc936fe054d9d58851d.jpg",
      },
      {
        id: 70,
        name: "War",
        slug: "war",
        language: "eng",
        games_count: 9104,
        image_background:
          "https://media.rawg.io/media/games/2fd/2fd1b58116b10cc1f4442bee5593ca7c.jpg",
      },
      {
        id: 43,
        name: "Post-apocalyptic",
        slug: "post-apocalyptic",
        language: "eng",
        games_count: 3777,
        image_background:
          "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
      },
      {
        id: 192,
        name: "Mature",
        slug: "mature",
        language: "eng",
        games_count: 2873,
        image_background:
          "https://media.rawg.io/media/games/849/849414b978db37d4563ff9e4b0d3a787.jpg",
      },
      {
        id: 81,
        name: "Military",
        slug: "military",
        language: "eng",
        games_count: 1727,
        image_background:
          "https://media.rawg.io/media/games/8bd/8bda6d876f3e241c6024022299553efd.jpg",
      },
      {
        id: 285,
        name: "Psychological",
        slug: "psychological",
        language: "eng",
        games_count: 1458,
        image_background:
          "https://media.rawg.io/media/games/643/6431f15213a854efd273943f2cf6a925.jpg",
      },
      {
        id: 283,
        name: "Based On A Novel",
        slug: "based-on-a-novel",
        language: "eng",
        games_count: 49,
        image_background:
          "https://media.rawg.io/media/screenshots/2b9/2b911fa5f17f537087631a4da9ede2af.jpg",
      },
      {
        id: 287,
        name: "Political",
        slug: "political",
        language: "eng",
        games_count: 577,
        image_background:
          "https://media.rawg.io/media/games/bc0/bc06a29ceac58652b684deefe7d56099.jpg",
      },
      {
        id: 19728,
        name: "european",
        slug: "european",
        language: "eng",
        games_count: 12,
        image_background:
          "https://media.rawg.io/media/screenshots/d14/d148f8ffc332b1d0f70be9bad91c2171.jpg",
      },
    ],
    esrb_rating: { id: 4, name: "Mature", slug: "mature" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/b49/b4912b5dbfc7ed8927b65f05b8507f6c.jpg",
      },
      {
        id: 163188,
        image:
          "https://media.rawg.io/media/screenshots/a17/a17ff71c8774a3b70375a869b3881244.jpg",
      },
      {
        id: 163189,
        image:
          "https://media.rawg.io/media/screenshots/e5a/e5aaa5d242144ab80ef8264c96516dcc.jpg",
      },
      {
        id: 163190,
        image:
          "https://media.rawg.io/media/screenshots/f36/f36e756c36d36fe8ffe73a4b39acbebf.jpg",
      },
      {
        id: 163191,
        image:
          "https://media.rawg.io/media/screenshots/437/437ad0efe43adcad4284f5f48d03559f.jpg",
      },
      {
        id: 163192,
        image:
          "https://media.rawg.io/media/screenshots/9e2/9e26af47a676b061f288ff269e91a8f1.jpg",
      },
      {
        id: 163193,
        image:
          "https://media.rawg.io/media/screenshots/2d9/2d9a5dd6a25c04e49accca5358782152.jpg",
      },
    ],
  },
  {
    id: 19487,
    slug: "alan-wake",
    name: "Alan Wake",
    released: "2010-05-14",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/5c0/5c0dd63002cb23f804aab327d40ef119.jpg",
    rating: 4.1,
    rating_top: 4,
    ratings: [
      { id: 4, title: "recommended", count: 1085, percent: 49.82 },
      { id: 5, title: "exceptional", count: 743, percent: 34.11 },
      { id: 3, title: "meh", count: 261, percent: 11.98 },
      { id: 1, title: "skip", count: 89, percent: 4.09 },
    ],
    ratings_count: 2161,
    reviews_text_count: 6,
    added: 9736,
    added_by_status: {
      yet: 572,
      owned: 6294,
      beaten: 1978,
      toplay: 293,
      dropped: 508,
      playing: 91,
    },
    metacritic: 83,
    playtime: 5,
    suggestions_count: 523,
    updated: "2024-03-20T07:25:55",
    user_game: null,
    reviews_count: 2178,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 14,
          name: "Xbox 360",
          slug: "xbox360",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 2796,
          image_background:
            "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
        },
        released_at: "2010-05-14",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 1,
          name: "Xbox One",
          slug: "xbox-one",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5590,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        released_at: "2010-05-14",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2010-05-14",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br>\t\t\t\t\t\t\t\t\t\t\t\t<ul class="bb_ul"><li><strong>OS:</strong> Windows XP SP2<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Processor:</strong> Dual Core 2GHz Intel or 2.8GHz AMD<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Memory:</strong> 2 GB RAM<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Graphics:</strong> DirectX 10 compatible with 512MB RAM<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>DirectX®:</strong> 9.0c<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Hard Drive:</strong> 8 GB HD space<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Sound:</strong> DirectX 9.0c compatible<br>\t\t\t\t\t\t\t\t\t\t\t\t</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br>\t\t\t\t\t\t\t\t\t\t\t\t<ul class="bb_ul"><li><strong>OS:</strong> Windows 7<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Processor:</strong> Quad Core 2.66GHz Intel or 3.2GHz AMD<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Memory:</strong> 4 GB RAM<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Graphics:</strong> DirectX 10 compatible or later with 1GB RAM <br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>DirectX®:</strong> 10<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Hard Drive:</strong> 8 GB HD space<br>\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Sound:</strong> DirectX 9.0c compatible<br>\t\t\t\t\t\t\t\t\t\t\t\t</li></ul>',
        },
        requirements_ru: null,
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 3, name: "Xbox", slug: "xbox" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
      {
        id: 2,
        name: "Shooter",
        slug: "shooter",
        games_count: 59459,
        image_background:
          "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
      },
      {
        id: 3,
        name: "Adventure",
        slug: "adventure",
        games_count: 137936,
        image_background:
          "https://media.rawg.io/media/games/0af/0af85e8edddfa55368e47c539914a220.jpg",
      },
    ],
    stores: [
      {
        id: 465958,
        store: {
          id: 2,
          name: "Xbox Store",
          slug: "xbox-store",
          domain: "microsoft.com",
          games_count: 4813,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
      },
      {
        id: 25873,
        store: {
          id: 5,
          name: "GOG",
          slug: "gog",
          domain: "gog.com",
          games_count: 5817,
          image_background:
            "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
        },
      },
      {
        id: 21833,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
      {
        id: 34002,
        store: {
          id: 7,
          name: "Xbox 360 Store",
          slug: "xbox360",
          domain: "marketplace.xbox.com",
          games_count: 1912,
          image_background:
            "https://media.rawg.io/media/games/d1a/d1a2e99ade53494c6330a0ed945fe823.jpg",
        },
      },
      {
        id: 339240,
        store: {
          id: 11,
          name: "Epic Games",
          slug: "epic-games",
          domain: "epicgames.com",
          games_count: 1319,
          image_background:
            "https://media.rawg.io/media/games/4be/4be6a6ad0364751a96229c56bf69be59.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 40847,
        name: "Steam Achievements",
        slug: "steam-achievements",
        language: "eng",
        games_count: 35702,
        image_background:
          "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
      },
      {
        id: 40836,
        name: "Full controller support",
        slug: "full-controller-support",
        language: "eng",
        games_count: 16746,
        image_background:
          "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
      },
      {
        id: 40849,
        name: "Steam Cloud",
        slug: "steam-cloud",
        language: "eng",
        games_count: 16769,
        image_background:
          "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 7808,
        name: "steam-trading-cards",
        slug: "steam-trading-cards",
        language: "eng",
        games_count: 7569,
        image_background:
          "https://media.rawg.io/media/games/6fc/6fcf4cd3b17c288821388e6085bb0fc9.jpg",
      },
      {
        id: 42,
        name: "Great Soundtrack",
        slug: "great-soundtrack",
        language: "eng",
        games_count: 3386,
        image_background:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 118,
        name: "Story Rich",
        slug: "story-rich",
        language: "eng",
        games_count: 20561,
        image_background:
          "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
      },
      {
        id: 36,
        name: "Open World",
        slug: "open-world",
        language: "eng",
        games_count: 7102,
        image_background:
          "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
      },
      {
        id: 149,
        name: "Third Person",
        slug: "third-person",
        language: "eng",
        games_count: 10861,
        image_background:
          "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
      },
      {
        id: 16,
        name: "Horror",
        slug: "horror",
        language: "eng",
        games_count: 43534,
        image_background:
          "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
      },
      {
        id: 1,
        name: "Survival",
        slug: "survival",
        language: "eng",
        games_count: 8231,
        image_background:
          "https://media.rawg.io/media/games/476/476178ef18ab0534771d099f51cdc694.jpg",
      },
      {
        id: 150,
        name: "Third-Person Shooter",
        slug: "third-person-shooter",
        language: "eng",
        games_count: 3266,
        image_background:
          "https://media.rawg.io/media/games/ebd/ebdbb7eb52bd58b0e7fa4538d9757b60.jpg",
      },
      {
        id: 41,
        name: "Dark",
        slug: "dark",
        language: "eng",
        games_count: 15192,
        image_background:
          "https://media.rawg.io/media/games/490/49016e06ae2103881ff6373248843069.jpg",
      },
      {
        id: 336,
        name: "controller support",
        slug: "controller-support",
        language: "eng",
        games_count: 293,
        image_background:
          "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
      },
      {
        id: 17,
        name: "Survival Horror",
        slug: "survival-horror",
        language: "eng",
        games_count: 8168,
        image_background:
          "https://media.rawg.io/media/games/ebd/ebdbb7eb52bd58b0e7fa4538d9757b60.jpg",
      },
      {
        id: 117,
        name: "Mystery",
        slug: "mystery",
        language: "eng",
        games_count: 12984,
        image_background:
          "https://media.rawg.io/media/games/a9a/a9a4e45ad8e653df2295e8410b7e96fd.jpg",
      },
      {
        id: 110,
        name: "Cinematic",
        slug: "cinematic",
        language: "eng",
        games_count: 1881,
        image_background:
          "https://media.rawg.io/media/games/9aa/9aa42d16d425fa6f179fc9dc2f763647.jpg",
      },
      {
        id: 232,
        name: "Episodic",
        slug: "episodic",
        language: "eng",
        games_count: 459,
        image_background:
          "https://media.rawg.io/media/games/a28/a289e23b4d4d84f26ab59125e3be4483.jpg",
      },
      {
        id: 200,
        name: "Narration",
        slug: "narration",
        language: "eng",
        games_count: 1896,
        image_background:
          "https://media.rawg.io/media/games/37a/37a9536e92cf8fe3b60045aa75dbd41f.jpg",
      },
      {
        id: 279,
        name: "Supernatural",
        slug: "supernatural",
        language: "eng",
        games_count: 1978,
        image_background:
          "https://media.rawg.io/media/games/a74/a7462cb98ad1ed06f646afd0a5a1454d.jpg",
      },
      {
        id: 183,
        name: "Thriller",
        slug: "thriller",
        language: "eng",
        games_count: 2325,
        image_background:
          "https://media.rawg.io/media/games/e77/e77ceadde635a080738bcde211687776.jpg",
      },
      {
        id: 7629,
        name: "enviroment",
        slug: "enviroment",
        language: "eng",
        games_count: 16,
        image_background:
          "https://media.rawg.io/media/screenshots/363/363eac26816e5f26a574415efebbd582.jpg",
      },
      {
        id: 36002,
        name: "psycholoical-horror",
        slug: "psycholoical-horror",
        language: "eng",
        games_count: 4,
        image_background:
          "https://media.rawg.io/media/games/5c0/5c0dd63002cb23f804aab327d40ef119.jpg",
      },
    ],
    esrb_rating: { id: 3, name: "Teen", slug: "teen" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/5c0/5c0dd63002cb23f804aab327d40ef119.jpg",
      },
      {
        id: 182489,
        image:
          "https://media.rawg.io/media/screenshots/8d7/8d7d24df1418efdaba45128e2c855f62.jpg",
      },
      {
        id: 182490,
        image:
          "https://media.rawg.io/media/screenshots/5ac/5ac6dd243c0ed41fb1a0b5734bff55f1.jpg",
      },
      {
        id: 182491,
        image:
          "https://media.rawg.io/media/screenshots/ad2/ad2e4f73ac71839178ea01ab66d8bf6c.jpg",
      },
      {
        id: 182492,
        image:
          "https://media.rawg.io/media/screenshots/b28/b28fd421d570931d83b27213538689df.jpg",
      },
      {
        id: 182493,
        image:
          "https://media.rawg.io/media/screenshots/d1e/d1e49ff4902b1bc964262ffc2e08043d.jpg",
      },
      {
        id: 182494,
        image:
          "https://media.rawg.io/media/screenshots/02f/02f6cb8534d5434e94559cdcd35aeef7.jpg",
      },
    ],
  },
  {
    id: 13668,
    slug: "amnesia-the-dark-descent",
    name: "Amnesia: The Dark Descent",
    released: "2010-09-08",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/b54/b54598d1d5cc31899f4f0a7e3122a7b0.jpg",
    rating: 3.65,
    rating_top: 4,
    ratings: [
      { id: 4, title: "recommended", count: 582, percent: 48.34 },
      { id: 5, title: "exceptional", count: 259, percent: 21.51 },
      { id: 3, title: "meh", count: 203, percent: 16.86 },
      { id: 1, title: "skip", count: 160, percent: 13.29 },
    ],
    ratings_count: 1185,
    reviews_text_count: 10,
    added: 9722,
    added_by_status: {
      yet: 744,
      owned: 7599,
      beaten: 741,
      toplay: 137,
      dropped: 468,
      playing: 33,
    },
    metacritic: 85,
    playtime: 2,
    suggestions_count: 438,
    updated: "2024-02-13T06:41:54",
    user_game: null,
    reviews_count: 1204,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 6,
          name: "Linux",
          slug: "linux",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 76454,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        released_at: "2010-09-08",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 12.04 LTS, Mint 13 LTS, Fedora 16, fully updated<br>\t</li><li><strong>Processor:</strong> 2.0Ghz - Low budget CPUs such as Celeron or Duron needs to be at about twice the CPU speed. <br>\t</li><li><strong>Memory:</strong> 2 GB<br>\t</li><li><strong>Graphics:</strong> Radeon X1000/GeForce 6 - Integrated graphics and low budget cards might not work. <br>\t</li><li><strong>Hard Drive:</strong> 3 GB</li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 5,
          name: "macOS",
          slug: "macos",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 103400,
          image_background:
            "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
        },
        released_at: "2010-09-08",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t<ul class="bb_ul"><li><strong>OS:</strong> Mac OS X 10.5.8 or newer<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Processor:</strong> 2.0Ghz<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Memory:</strong> 1 GB RAM<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Graphics:</strong> Radeon X1000/GeForce 6 (Integrated Intel Graphics not supported)<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Hard Drive:</strong> 2GB space free<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2010-09-08",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> Windows XP/Vista/7<br>\t\t\t\t\t\t\t\t</li><li><strong>Processor:</strong>  2.0Ghz - Low budget CPUs such as Celeron or Duron needs to be at about twice the CPU speed<br>\t\t\t\t\t\t\t\t</li><li><strong>Memory:</strong> 2 GB<br>\t\t\t\t\t\t\t\t</li><li><strong>Hard Drive:</strong> 3GB<br>\t\t\t\t\t\t\t\t</li><li><strong>Graphics:</strong> Radeon X1000/GF 6 - Integrated graphics and very low budget cards might not work.<br>\t\t\t\t\t\t\t</li></ul>',
        },
        requirements_ru: {
          minimum:
            "Pentium 4/Athlon XP 1.5 ГГц,1 Гб памяти,3D-ускоритель со 128 Мб памяти,2 Гб на винчестере",
          recommended:
            "Pentium 4/Athlon XP 2 ГГц,2 Гб памяти,3D-ускоритель с 256 Мб памяти,2 Гб на винчестере",
        },
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 5, name: "Apple Macintosh", slug: "mac" } },
      { platform: { id: 6, name: "Linux", slug: "linux" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
      {
        id: 3,
        name: "Adventure",
        slug: "adventure",
        games_count: 137936,
        image_background:
          "https://media.rawg.io/media/games/0af/0af85e8edddfa55368e47c539914a220.jpg",
      },
      {
        id: 51,
        name: "Indie",
        slug: "indie",
        games_count: 62257,
        image_background:
          "https://media.rawg.io/media/games/9dd/9ddabb34840ea9227556670606cf8ea3.jpg",
      },
    ],
    stores: [
      {
        id: 423652,
        store: {
          id: 11,
          name: "Epic Games",
          slug: "epic-games",
          domain: "epicgames.com",
          games_count: 1319,
          image_background:
            "https://media.rawg.io/media/games/4be/4be6a6ad0364751a96229c56bf69be59.jpg",
        },
      },
      {
        id: 15036,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
      {
        id: 25640,
        store: {
          id: 5,
          name: "GOG",
          slug: "gog",
          domain: "gog.com",
          games_count: 5817,
          image_background:
            "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 40836,
        name: "Full controller support",
        slug: "full-controller-support",
        language: "eng",
        games_count: 16746,
        image_background:
          "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 42,
        name: "Great Soundtrack",
        slug: "great-soundtrack",
        language: "eng",
        games_count: 3386,
        image_background:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 118,
        name: "Story Rich",
        slug: "story-rich",
        language: "eng",
        games_count: 20561,
        image_background:
          "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
      },
      {
        id: 8,
        name: "First-Person",
        slug: "first-person",
        language: "eng",
        games_count: 30626,
        image_background:
          "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
      },
      {
        id: 16,
        name: "Horror",
        slug: "horror",
        language: "eng",
        games_count: 43534,
        image_background:
          "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
      },
      {
        id: 6,
        name: "Exploration",
        slug: "exploration",
        language: "eng",
        games_count: 21834,
        image_background:
          "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
      },
      {
        id: 193,
        name: "Classic",
        slug: "classic",
        language: "eng",
        games_count: 1783,
        image_background:
          "https://media.rawg.io/media/games/960/960b601d9541cec776c5fa42a00bf6c4.jpg",
      },
      {
        id: 1,
        name: "Survival",
        slug: "survival",
        language: "eng",
        games_count: 8231,
        image_background:
          "https://media.rawg.io/media/games/476/476178ef18ab0534771d099f51cdc694.jpg",
      },
      {
        id: 15,
        name: "Stealth",
        slug: "stealth",
        language: "eng",
        games_count: 6100,
        image_background:
          "https://media.rawg.io/media/games/b7d/b7d3f1715fa8381a4e780173a197a615.jpg",
      },
      {
        id: 41,
        name: "Dark",
        slug: "dark",
        language: "eng",
        games_count: 15192,
        image_background:
          "https://media.rawg.io/media/games/490/49016e06ae2103881ff6373248843069.jpg",
      },
      {
        id: 62,
        name: "Moddable",
        slug: "moddable",
        language: "eng",
        games_count: 880,
        image_background:
          "https://media.rawg.io/media/games/c22/c22d804ac753c72f2617b3708a625dec.jpg",
      },
      {
        id: 17,
        name: "Survival Horror",
        slug: "survival-horror",
        language: "eng",
        games_count: 8168,
        image_background:
          "https://media.rawg.io/media/games/ebd/ebdbb7eb52bd58b0e7fa4538d9757b60.jpg",
      },
      {
        id: 40833,
        name: "Captions available",
        slug: "captions-available",
        language: "eng",
        games_count: 1311,
        image_background:
          "https://media.rawg.io/media/games/7a2/7a2500ee8b2c0e1ff268bb4479463dea.jpg",
      },
      {
        id: 40834,
        name: "Commentary available",
        slug: "commentary-available",
        language: "eng",
        games_count: 266,
        image_background:
          "https://media.rawg.io/media/games/594/5949baae74fe9e399adbce0c44e28783.jpg",
      },
      {
        id: 204,
        name: "Gothic",
        slug: "gothic",
        language: "eng",
        games_count: 856,
        image_background:
          "https://media.rawg.io/media/games/fd9/fd92f105dcd6491bc5d61135033d1f19.jpg",
      },
      {
        id: 47,
        name: "Lovecraftian",
        slug: "lovecraftian",
        language: "eng",
        games_count: 593,
        image_background:
          "https://media.rawg.io/media/screenshots/d2f/d2f5f87a6e6df6b988dac391605f0b60.jpg",
      },
    ],
    esrb_rating: { id: 4, name: "Mature", slug: "mature" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/b54/b54598d1d5cc31899f4f0a7e3122a7b0.jpg",
      },
      {
        id: 117208,
        image:
          "https://media.rawg.io/media/screenshots/32b/32bde7545dff888358a7ce620c7b3063.jpg",
      },
      {
        id: 117209,
        image:
          "https://media.rawg.io/media/screenshots/f59/f597e6857aab3b1ba098d713524d1690.jpg",
      },
      {
        id: 117210,
        image:
          "https://media.rawg.io/media/screenshots/5fc/5fce282fa41e7d73d0e8b0c35da74391.jpg",
      },
      {
        id: 117211,
        image:
          "https://media.rawg.io/media/screenshots/8f8/8f88209c1fdc529c3af746c08a44ba6c.jpg",
      },
      {
        id: 117212,
        image:
          "https://media.rawg.io/media/screenshots/ff7/ff776252fbd6de18cfd197b57d6e6aac.jpg",
      },
      {
        id: 117213,
        image:
          "https://media.rawg.io/media/screenshots/42d/42d3c11b04e589565e5a9d2feae5023c.jpg",
      },
    ],
  },
  {
    id: 3696,
    slug: "wolfenstein-the-new-order",
    name: "Wolfenstein: The New Order",
    released: "2014-05-19",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/c80/c80bcf321da44d69b18a06c04d942662.jpg",
    rating: 4.18,
    rating_top: 4,
    ratings: [
      { id: 4, title: "recommended", count: 1321, percent: 54.72 },
      { id: 5, title: "exceptional", count: 822, percent: 34.05 },
      { id: 3, title: "meh", count: 214, percent: 8.86 },
      { id: 1, title: "skip", count: 57, percent: 2.36 },
    ],
    ratings_count: 2396,
    reviews_text_count: 10,
    added: 9668,
    added_by_status: {
      yet: 471,
      owned: 6013,
      beaten: 2437,
      toplay: 303,
      dropped: 362,
      playing: 82,
    },
    metacritic: 81,
    playtime: 12,
    suggestions_count: 673,
    updated: "2024-03-20T11:28:13",
    user_game: null,
    reviews_count: 2414,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 1,
          name: "Xbox One",
          slug: "xbox-one",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5590,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        released_at: "2014-05-19",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2014-05-19",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><ul class="bb_ul"><li><strong>OS:</strong> 64-bit Windows 7/Windows 8<br></li><li><strong>Processor:</strong> Intel Core i7 or equivalent AMD<br></li><li><strong>Memory:</strong> 4 GB RAM<br></li><li><strong>Graphics:</strong> GeForce 460, ATI Radeon HD 6850<br></li><li><strong>Storage:</strong> 50 GB available space</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><ul class="bb_ul"><li><strong>Additional Notes:</strong> AMD Radeon users: Please install AMD Catalyst™ 14.4</li></ul>',
        },
        requirements_ru: {
          minimum:
            "Win 7 64,Core 2 Duo 2.66GHz/Phenom II X2 545,GeForce GTS 250/Radeon HD 6670,3 GB RAM,50 GB HDD",
          recommended:
            "Win 7 64,Core i7 2.80GHz/Phenom II X6 1090T,GeForce GTX 460/Radeon HD 6850,4 GB RAM,50 GB HDD",
        },
      },
      {
        platform: {
          id: 14,
          name: "Xbox 360",
          slug: "xbox360",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 2796,
          image_background:
            "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
        },
        released_at: "2014-05-19",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 16,
          name: "PlayStation 3",
          slug: "playstation3",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 3168,
          image_background:
            "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
        },
        released_at: "2014-05-19",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 18,
          name: "PlayStation 4",
          slug: "playstation4",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 6767,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
        released_at: "2014-05-19",
        requirements_en: null,
        requirements_ru: null,
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
      { platform: { id: 3, name: "Xbox", slug: "xbox" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
      {
        id: 2,
        name: "Shooter",
        slug: "shooter",
        games_count: 59459,
        image_background:
          "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
      },
    ],
    stores: [
      {
        id: 403368,
        store: {
          id: 5,
          name: "GOG",
          slug: "gog",
          domain: "gog.com",
          games_count: 5817,
          image_background:
            "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
        },
      },
      {
        id: 7879,
        store: {
          id: 2,
          name: "Xbox Store",
          slug: "xbox-store",
          domain: "microsoft.com",
          games_count: 4813,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
      },
      {
        id: 17137,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
      {
        id: 3999,
        store: {
          id: 3,
          name: "PlayStation Store",
          slug: "playstation-store",
          domain: "store.playstation.com",
          games_count: 7900,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
      },
      {
        id: 34545,
        store: {
          id: 7,
          name: "Xbox 360 Store",
          slug: "xbox360",
          domain: "marketplace.xbox.com",
          games_count: 1912,
          image_background:
            "https://media.rawg.io/media/games/d1a/d1a2e99ade53494c6330a0ed945fe823.jpg",
        },
      },
      {
        id: 660388,
        store: {
          id: 11,
          name: "Epic Games",
          slug: "epic-games",
          domain: "epicgames.com",
          games_count: 1319,
          image_background:
            "https://media.rawg.io/media/games/4be/4be6a6ad0364751a96229c56bf69be59.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 40847,
        name: "Steam Achievements",
        slug: "steam-achievements",
        language: "eng",
        games_count: 35702,
        image_background:
          "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
      },
      {
        id: 7,
        name: "Multiplayer",
        slug: "multiplayer",
        language: "eng",
        games_count: 37130,
        image_background:
          "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
      },
      {
        id: 13,
        name: "Atmospheric",
        slug: "atmospheric",
        language: "eng",
        games_count: 32044,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 42,
        name: "Great Soundtrack",
        slug: "great-soundtrack",
        language: "eng",
        games_count: 3386,
        image_background:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 118,
        name: "Story Rich",
        slug: "story-rich",
        language: "eng",
        games_count: 20561,
        image_background:
          "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
      },
      {
        id: 8,
        name: "First-Person",
        slug: "first-person",
        language: "eng",
        games_count: 30626,
        image_background:
          "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
      },
      {
        id: 32,
        name: "Sci-fi",
        slug: "sci-fi",
        language: "eng",
        games_count: 18535,
        image_background:
          "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
      },
      {
        id: 40845,
        name: "Partial Controller Support",
        slug: "partial-controller-support",
        language: "eng",
        games_count: 11229,
        image_background:
          "https://media.rawg.io/media/games/62c/62c7c8b28a27b83680b22fb9d33fc619.jpg",
      },
      {
        id: 16,
        name: "Horror",
        slug: "horror",
        language: "eng",
        games_count: 43534,
        image_background:
          "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
      },
      {
        id: 30,
        name: "FPS",
        slug: "fps",
        language: "eng",
        games_count: 12893,
        image_background:
          "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
      },
      {
        id: 26,
        name: "Gore",
        slug: "gore",
        language: "eng",
        games_count: 5515,
        image_background:
          "https://media.rawg.io/media/games/7f6/7f6cd70ba2ad57053b4847c13569f2d8.jpg",
      },
      {
        id: 40850,
        name: "Steam Leaderboards",
        slug: "steam-leaderboards",
        language: "eng",
        games_count: 6539,
        image_background:
          "https://media.rawg.io/media/games/59a/59a3ebcba3d08c51532c6ca877aff256.jpg",
      },
      {
        id: 15,
        name: "Stealth",
        slug: "stealth",
        language: "eng",
        games_count: 6100,
        image_background:
          "https://media.rawg.io/media/games/b7d/b7d3f1715fa8381a4e780173a197a615.jpg",
      },
      {
        id: 70,
        name: "War",
        slug: "war",
        language: "eng",
        games_count: 9104,
        image_background:
          "https://media.rawg.io/media/games/2fd/2fd1b58116b10cc1f4442bee5593ca7c.jpg",
      },
      {
        id: 89,
        name: "Historical",
        slug: "historical",
        language: "eng",
        games_count: 3005,
        image_background:
          "https://media.rawg.io/media/games/d07/d0790809a13027251b6d0f4dc7538c58.jpg",
      },
      {
        id: 119,
        name: "Dystopian",
        slug: "dystopian",
        language: "eng",
        games_count: 2001,
        image_background:
          "https://media.rawg.io/media/games/a0e/a0ef08621301a1eab5e04fa5c96978fa.jpeg",
      },
      {
        id: 197,
        name: "Robots",
        slug: "robots",
        language: "eng",
        games_count: 8053,
        image_background:
          "https://media.rawg.io/media/screenshots/e29/e29ce2369152515e9ae20c6e217cbe81.jpg",
      },
      {
        id: 105,
        name: "World War II",
        slug: "world-war-ii",
        language: "eng",
        games_count: 925,
        image_background:
          "https://media.rawg.io/media/games/fe6/fe68940746007261e34da4d9cce81423.jpg",
      },
      {
        id: 208,
        name: "Alternate History",
        slug: "alternate-history",
        language: "eng",
        games_count: 1675,
        image_background:
          "https://media.rawg.io/media/games/f14/f1422eacab98c5f85a5123da4e9d9e89.jpg",
      },
    ],
    esrb_rating: { id: 4, name: "Mature", slug: "mature" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/c80/c80bcf321da44d69b18a06c04d942662.jpg",
      },
      {
        id: 32371,
        image:
          "https://media.rawg.io/media/screenshots/23a/23af906d70f57be798bbd83da986c6db.jpg",
      },
      {
        id: 32372,
        image:
          "https://media.rawg.io/media/screenshots/b71/b716cf4d51be00d9561df3d1588383c4.jpg",
      },
      {
        id: 32373,
        image:
          "https://media.rawg.io/media/screenshots/439/4395d5364559ca15ac8e1becb100daea.jpg",
      },
      {
        id: 32374,
        image:
          "https://media.rawg.io/media/screenshots/0de/0de321f22cd1f5a7d0fb6b471f63c2d8.jpg",
      },
      {
        id: 32375,
        image:
          "https://media.rawg.io/media/screenshots/900/900e962d31b5ace3fb66bfd388d352cf.jpg",
      },
      {
        id: 32376,
        image:
          "https://media.rawg.io/media/screenshots/a1d/a1d6bd9f3617e0787da4b0f8389fe1f0.jpg",
      },
    ],
  },
  {
    id: 4386,
    slug: "saints-row-the-third",
    name: "Saints Row: The Third",
    released: "2011-11-15",
    tba: false,
    background_image:
      "https://media.rawg.io/media/games/d69/d69810315bd7e226ea2d21f9156af629.jpg",
    rating: 3.95,
    rating_top: 4,
    ratings: [
      { id: 4, title: "recommended", count: 1121, percent: 56.25 },
      { id: 5, title: "exceptional", count: 473, percent: 23.73 },
      { id: 3, title: "meh", count: 308, percent: 15.45 },
      { id: 1, title: "skip", count: 91, percent: 4.57 },
    ],
    ratings_count: 1978,
    reviews_text_count: 9,
    added: 9649,
    added_by_status: {
      yet: 287,
      owned: 6766,
      beaten: 1992,
      toplay: 113,
      dropped: 460,
      playing: 31,
    },
    metacritic: 84,
    playtime: 14,
    suggestions_count: 662,
    updated: "2024-03-20T07:31:09",
    user_game: null,
    reviews_count: 1993,
    saturated_color: "0f0f0f",
    dominant_color: "0f0f0f",
    platforms: [
      {
        platform: {
          id: 6,
          name: "Linux",
          slug: "linux",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 76454,
          image_background:
            "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
        },
        released_at: "2011-11-15",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><br><ul class="bb_ul"><li><strong>OS:</strong> Ubuntu 14.04.3 LTS, Ubuntu 15.04, Linux Mint 17.1, Steam OS Brewmaster<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>CPU:</strong> Intel Core i5-4570T (2.9 GHz) or equivalent\tor AMD Athlon II X2 270 (3.8 GHz) or equivalent\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t<br></li><li><strong>GPU:</strong>GeForce GT 630 (1024 MB) or equivalent or Radeon HD 6850 (1024 MB) or equivalent\t<br></li><li><strong>Recommended Graphics Drivers:</strong> Nvidia: 352, 355 or 358 series driver / AMD: AMD Catalyst (fglrx) 15.9 or better. MESA drivers and Intel Graphics are not currently supported.  \t\t<br></li><li><strong>RAM:</strong> 4096 MB<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Disk Space:</strong> Around 7 GB<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li></ul>',
          recommended:
            '<strong>Recommended:</strong><br><br><ul class="bb_ul"><li><strong>OS:</strong>Ubuntu 14.04.3 LTS, Ubuntu 15.04, Linux Mint 17.1, Steam OS Brewmaster<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>CPU:</strong> Intel Core i5-4670K (3.4 GHz) or equivalent or AMD Athlon X4 860K (3.7 GHz) or equivalent\t\t\t\t\t\t\t\t\t\t\t<br></li><li><strong>GPU:</strong>GeForce GTX 760 (2048 MB) or equivalent or Radeon HD 7950 (2048 MB) or equivalent\t\t<br></li><li><strong>Recommended Graphics Drivers:</strong> Nvidia: 352, 355 or 358 series driver / AMD: AMD Catalyst (fglrx) 15.9 or better. MESA drivers and Intel Graphics are not currently supported.  \t<br></li><li><strong>RAM:</strong>8192 MB<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Disk Space:</strong>Around 7 GB<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li></ul>',
        },
        requirements_ru: null,
      },
      {
        platform: {
          id: 4,
          name: "PC",
          slug: "pc",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 525904,
          image_background:
            "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
        },
        released_at: "2011-11-15",
        requirements_en: {
          minimum:
            '<strong>Minimum:</strong><br><br><ul class="bb_ul"><li><strong>OS:</strong> Windows® XP<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Processor:</strong> 2GHz Dual Core Processor (Intel® Core 2 Duo or AMD Athlon™ X2) or higher<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Memory:</strong> 2GB System RAM or more<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Graphics:</strong> 320MB Video RAM GPU w/ Shader Model 3.0 support.  NVIDIA® GeForce® 8800 series or better.  ATI Radeon™ HD3800 series or better<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>DirectX®:</strong> 9.0c<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Hard Drive:</strong> 10GB<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Sound:</strong> 100% DirectX® 9.0C compliant sound card or equivalent onboard sound<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Co-Op Play:</strong>NVIDIA® GeForce® 8800 cards require 640MB of Video RAM.  ATI Radeon™ HD3800 cards require 1GB of Video RAM<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li></ul><strong></strong>',
          recommended:
            '<strong>Recommended:</strong><br><br><ul class="bb_ul"><li><strong>OS:</strong> Microsoft® Windows® 7<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Processor:</strong> Any Quad Core Processor (Intel® Core i5 or AMD Phenom™ II X4) or 3.0+ Dual Core CPU<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Memory:</strong> 4GB System RAM or more<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Graphics:</strong> 1GB Video RAM GPU w/ Shader Model 4.0 support.  NVIDIA® GeForce® GTX 400 series or better.  ATI Radeon™ HD5000 series or better <br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>DirectX®:</strong> DirectX® 11<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Hard Drive:</strong> 10 GB<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li><li><strong>Sound:</strong> 100% DirectX® 9.0C compliant sound card or equivalent onboard sound<br>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</li></ul>',
        },
        requirements_ru: {
          minimum:
            "Core 2 Duo/Athlon X2 2 ГГц,2 Гб памяти,GeForce 8800/Radeon HD 3800,10 Гб на винчестере",
          recommended:
            "Core 2 Duo/Phenom II X4 3 ГГц,2 Гб памяти,GeForce GTX 460/Radeon HD 5850,10 Гб на винчестере",
        },
      },
      {
        platform: {
          id: 1,
          name: "Xbox One",
          slug: "xbox-one",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 5590,
          image_background:
            "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
        },
        released_at: "2011-11-15",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 14,
          name: "Xbox 360",
          slug: "xbox360",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 2796,
          image_background:
            "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
        },
        released_at: "2011-11-15",
        requirements_en: null,
        requirements_ru: null,
      },
      {
        platform: {
          id: 16,
          name: "PlayStation 3",
          slug: "playstation3",
          image: null,
          year_end: null,
          year_start: null,
          games_count: 3168,
          image_background:
            "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
        },
        released_at: "2011-11-15",
        requirements_en: null,
        requirements_ru: null,
      },
    ],
    parent_platforms: [
      { platform: { id: 1, name: "PC", slug: "pc" } },
      { platform: { id: 2, name: "PlayStation", slug: "playstation" } },
      { platform: { id: 3, name: "Xbox", slug: "xbox" } },
      { platform: { id: 6, name: "Linux", slug: "linux" } },
    ],
    genres: [
      {
        id: 4,
        name: "Action",
        slug: "action",
        games_count: 178358,
        image_background:
          "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
      },
      {
        id: 3,
        name: "Adventure",
        slug: "adventure",
        games_count: 137936,
        image_background:
          "https://media.rawg.io/media/games/0af/0af85e8edddfa55368e47c539914a220.jpg",
      },
    ],
    stores: [
      {
        id: 467012,
        store: {
          id: 2,
          name: "Xbox Store",
          slug: "xbox-store",
          domain: "microsoft.com",
          games_count: 4813,
          image_background:
            "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
        },
      },
      {
        id: 4714,
        store: {
          id: 3,
          name: "PlayStation Store",
          slug: "playstation-store",
          domain: "store.playstation.com",
          games_count: 7900,
          image_background:
            "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
        },
      },
      {
        id: 11270,
        store: {
          id: 1,
          name: "Steam",
          slug: "steam",
          domain: "store.steampowered.com",
          games_count: 89063,
          image_background:
            "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
        },
      },
      {
        id: 33958,
        store: {
          id: 7,
          name: "Xbox 360 Store",
          slug: "xbox360",
          domain: "marketplace.xbox.com",
          games_count: 1912,
          image_background:
            "https://media.rawg.io/media/games/d1a/d1a2e99ade53494c6330a0ed945fe823.jpg",
        },
      },
    ],
    clip: null,
    tags: [
      {
        id: 31,
        name: "Singleplayer",
        slug: "singleplayer",
        language: "eng",
        games_count: 218199,
        image_background:
          "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
      },
      {
        id: 7,
        name: "Multiplayer",
        slug: "multiplayer",
        language: "eng",
        games_count: 37130,
        image_background:
          "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
      },
      {
        id: 42,
        name: "Great Soundtrack",
        slug: "great-soundtrack",
        language: "eng",
        games_count: 3386,
        image_background:
          "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
      },
      {
        id: 24,
        name: "RPG",
        slug: "rpg",
        language: "eng",
        games_count: 19865,
        image_background:
          "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
      },
      {
        id: 18,
        name: "Co-op",
        slug: "co-op",
        language: "eng",
        games_count: 11072,
        image_background:
          "https://media.rawg.io/media/games/511/5118aff5091cb3efec399c808f8c598f.jpg",
      },
      {
        id: 36,
        name: "Open World",
        slug: "open-world",
        language: "eng",
        games_count: 7102,
        image_background:
          "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
      },
      {
        id: 411,
        name: "cooperative",
        slug: "cooperative",
        language: "eng",
        games_count: 4703,
        image_background:
          "https://media.rawg.io/media/games/73e/73eecb8909e0c39fb246f457b5d6cbbe.jpg",
      },
      {
        id: 149,
        name: "Third Person",
        slug: "third-person",
        language: "eng",
        games_count: 10861,
        image_background:
          "https://media.rawg.io/media/games/995/9951d9d55323d08967640f7b9ab3e342.jpg",
      },
      {
        id: 40845,
        name: "Partial Controller Support",
        slug: "partial-controller-support",
        language: "eng",
        games_count: 11229,
        image_background:
          "https://media.rawg.io/media/games/62c/62c7c8b28a27b83680b22fb9d33fc619.jpg",
      },
      {
        id: 9,
        name: "Online Co-Op",
        slug: "online-co-op",
        language: "eng",
        games_count: 5203,
        image_background:
          "https://media.rawg.io/media/games/48c/48cb04ca483be865e3a83119c94e6097.jpg",
      },
      {
        id: 4,
        name: "Funny",
        slug: "funny",
        language: "eng",
        games_count: 24154,
        image_background:
          "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
      },
      {
        id: 37,
        name: "Sandbox",
        slug: "sandbox",
        language: "eng",
        games_count: 6607,
        image_background:
          "https://media.rawg.io/media/games/7cf/7cfc9220b401b7a300e409e539c9afd5.jpg",
      },
      {
        id: 189,
        name: "Female Protagonist",
        slug: "female-protagonist",
        language: "eng",
        games_count: 11837,
        image_background:
          "https://media.rawg.io/media/games/530/5302dd22a190e664531236ca724e8726.jpg",
      },
      {
        id: 123,
        name: "Comedy",
        slug: "comedy",
        language: "eng",
        games_count: 11960,
        image_background:
          "https://media.rawg.io/media/games/e3d/e3ddc524c6292a435d01d97cc5f42ea7.jpg",
      },
      {
        id: 150,
        name: "Third-Person Shooter",
        slug: "third-person-shooter",
        language: "eng",
        games_count: 3266,
        image_background:
          "https://media.rawg.io/media/games/ebd/ebdbb7eb52bd58b0e7fa4538d9757b60.jpg",
      },
      {
        id: 63,
        name: "Zombies",
        slug: "zombies",
        language: "eng",
        games_count: 10103,
        image_background:
          "https://media.rawg.io/media/games/cee/cee577e2097a59b77193fe2bce94667d.jpg",
      },
      {
        id: 121,
        name: "Character Customization",
        slug: "character-customization",
        language: "eng",
        games_count: 4284,
        image_background:
          "https://media.rawg.io/media/games/33b/33b825c76382931df0fd8ecddf5caebe.jpg",
      },
      {
        id: 144,
        name: "Crime",
        slug: "crime",
        language: "eng",
        games_count: 2718,
        image_background:
          "https://media.rawg.io/media/games/5fa/5fae5fec3c943179e09da67a4427d68f.jpg",
      },
      {
        id: 153,
        name: "Satire",
        slug: "satire",
        language: "eng",
        games_count: 1264,
        image_background:
          "https://media.rawg.io/media/screenshots/17a/17a94bcc1b994fa4e24729371d6988b6.jpg",
      },
    ],
    esrb_rating: { id: 4, name: "Mature", slug: "mature" },
    short_screenshots: [
      {
        id: -1,
        image:
          "https://media.rawg.io/media/games/d69/d69810315bd7e226ea2d21f9156af629.jpg",
      },
      {
        id: 79408,
        image:
          "https://media.rawg.io/media/screenshots/ac0/ac00b015e9c51f52a24631c44676f81b.jpg",
      },
      {
        id: 79409,
        image:
          "https://media.rawg.io/media/screenshots/4e8/4e8cb89dc1ac9b3a9b68ad34f1dbc744.jpg",
      },
      {
        id: 79410,
        image:
          "https://media.rawg.io/media/screenshots/cc2/cc205478d728ad862c0e9da48df5db05.jpg",
      },
      {
        id: 79411,
        image:
          "https://media.rawg.io/media/screenshots/6a4/6a4976a45c96960e72d05362375908e4.jpg",
      },
      {
        id: 79412,
        image:
          "https://media.rawg.io/media/screenshots/e26/e26e0f29195d3682e6e9e9f5a74f6250.jpg",
      },
      {
        id: 79413,
        image:
          "https://media.rawg.io/media/screenshots/215/215fbd506ac8c1a24cb2bf9b9a4d5b92.jpg",
      },
    ],
  },
];

const extractedData = gameData.results.map(
  ({ id, name, background_image, rating }) => ({
    id,
    name,
    background_image,
    rating,
  }),
);

console.log(extractedData);

const gameDataPageOne = [
  {
    id: 3498,
    name: "Grand Theft Auto V",
    background_image:
      "https://media.rawg.io/media/games/20a/20aa03a10cda45239fe22d035c0ebe64.jpg",
    rating: 4.47,
  },
  {
    id: 3328,
    name: "The Witcher 3: Wild Hunt",
    background_image:
      "https://media.rawg.io/media/games/618/618c2031a07bbff6b4f611f10b6bcdbc.jpg",
    rating: 4.65,
  },
  {
    id: 4200,
    name: "Portal 2",
    background_image:
      "https://media.rawg.io/media/games/2ba/2bac0e87cf45e5b508f227d281c9252a.jpg",
    rating: 4.61,
  },
  {
    id: 4291,
    name: "Counter-Strike: Global Offensive",
    background_image:
      "https://media.rawg.io/media/games/736/73619bd336c894d6941d926bfd563946.jpg",
    rating: 3.57,
  },
  {
    id: 5286,
    name: "Tomb Raider (2013)",
    background_image:
      "https://media.rawg.io/media/games/021/021c4e21a1824d2526f925eff6324653.jpg",
    rating: 4.05,
  },
  {
    id: 13536,
    name: "Portal",
    background_image:
      "https://media.rawg.io/media/games/7fa/7fa0b586293c5861ee32490e953a4996.jpg",
    rating: 4.51,
  },
  {
    id: 12020,
    name: "Left 4 Dead 2",
    background_image:
      "https://media.rawg.io/media/games/d58/d588947d4286e7b5e0e12e1bea7d9844.jpg",
    rating: 4.09,
  },
  {
    id: 5679,
    name: "The Elder Scrolls V: Skyrim",
    background_image:
      "https://media.rawg.io/media/games/7cf/7cfc9220b401b7a300e409e539c9afd5.jpg",
    rating: 4.42,
  },
  {
    id: 28,
    name: "Red Dead Redemption 2",
    background_image:
      "https://media.rawg.io/media/games/511/5118aff5091cb3efec399c808f8c598f.jpg",
    rating: 4.59,
  },
  {
    id: 4062,
    name: "BioShock Infinite",
    background_image:
      "https://media.rawg.io/media/games/fc1/fc1307a2774506b5bd65d7e8424664a7.jpg",
    rating: 4.38,
  },
  {
    id: 3439,
    name: "Life is Strange",
    background_image:
      "https://media.rawg.io/media/games/562/562553814dd54e001a541e4ee83a591c.jpg",
    rating: 4.12,
  },
  {
    id: 802,
    name: "Borderlands 2",
    background_image:
      "https://media.rawg.io/media/games/49c/49c3dfa4ce2f6f140cc4825868e858cb.jpg",
    rating: 4.02,
  },
  {
    id: 13537,
    name: "Half-Life 2",
    background_image:
      "https://media.rawg.io/media/games/b8c/b8c243eaa0fbac8115e0cdccac3f91dc.jpg",
    rating: 4.49,
  },
  {
    id: 4286,
    name: "BioShock",
    background_image:
      "https://media.rawg.io/media/games/bc0/bc06a29ceac58652b684deefe7d56099.jpg",
    rating: 4.36,
  },
  {
    id: 32,
    name: "Destiny 2",
    background_image:
      "https://media.rawg.io/media/games/34b/34b1f1850a1c06fd971bc6ab3ac0ce0e.jpg",
    rating: 3.53,
  },
  {
    id: 58175,
    name: "God of War (2018)",
    background_image:
      "https://media.rawg.io/media/games/4be/4be6a6ad0364751a96229c56bf69be59.jpg",
    rating: 4.57,
  },
  {
    id: 1030,
    name: "Limbo",
    background_image:
      "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
    rating: 4.15,
  },
  {
    id: 3070,
    name: "Fallout 4",
    background_image:
      "https://media.rawg.io/media/games/d82/d82990b9c67ba0d2d09d4e6fa88885a7.jpg",
    rating: 3.8,
  },
  {
    id: 2454,
    name: "DOOM (2016)",
    background_image:
      "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
    rating: 4.38,
  },
  {
    id: 3939,
    name: "PAYDAY 2",
    background_image:
      "https://media.rawg.io/media/games/73e/73eecb8909e0c39fb246f457b5d6cbbe.jpg",
    rating: 3.51,
  },
];

const gameDataPageTwo = [
  {
    id: 11859,
    name: "Team Fortress 2",
    background_image:
      "https://media.rawg.io/media/games/46d/46d98e6910fbc0706e2948a7cc9b10c5.jpg",
    rating: 3.68,
  },
  {
    id: 41494,
    name: "Cyberpunk 2077",
    background_image:
      "https://media.rawg.io/media/games/26d/26d4437715bee60138dab4a7c8c59c92.jpg",
    rating: 4.16,
  },
  {
    id: 278,
    name: "Horizon Zero Dawn",
    background_image:
      "https://media.rawg.io/media/games/b7d/b7d3f1715fa8381a4e780173a197a615.jpg",
    rating: 4.29,
  },
  {
    id: 4459,
    name: "Grand Theft Auto IV",
    background_image:
      "https://media.rawg.io/media/games/4a0/4a0a1316102366260e6f38fd2a9cfdce.jpg",
    rating: 4.26,
  },
  {
    id: 422,
    name: "Terraria",
    background_image:
      "https://media.rawg.io/media/games/f46/f466571d536f2e3ea9e815ad17177501.jpg",
    rating: 4.06,
  },
  {
    id: 10213,
    name: "Dota 2",
    background_image:
      "https://media.rawg.io/media/games/6fc/6fcf4cd3b17c288821388e6085bb0fc9.jpg",
    rating: 3.07,
  },
  {
    id: 3272,
    name: "Rocket League",
    background_image:
      "https://media.rawg.io/media/games/8cc/8cce7c0e99dcc43d66c8efd42f9d03e3.jpg",
    rating: 3.94,
  },
  {
    id: 766,
    name: "Warframe",
    background_image:
      "https://media.rawg.io/media/games/f87/f87457e8347484033cb34cde6101d08d.jpg",
    rating: 3.42,
  },
  {
    id: 29028,
    name: "Metro 2033",
    background_image:
      "https://media.rawg.io/media/games/120/1201a40e4364557b124392ee50317b99.jpg",
    rating: 3.94,
  },
  {
    id: 7689,
    name: "Rise of the Tomb Raider",
    background_image:
      "https://media.rawg.io/media/games/b45/b45575f34285f2c4479c9a5f719d972e.jpg",
    rating: 4.04,
  },
  {
    id: 3192,
    name: "Metal Gear Solid V: The Phantom Pain",
    background_image:
      "https://media.rawg.io/media/games/490/49016e06ae2103881ff6373248843069.jpg",
    rating: 4.15,
  },
  {
    id: 3287,
    name: "Batman: Arkham Knight",
    background_image:
      "https://media.rawg.io/media/games/310/3106b0e012271c5ffb16497b070be739.jpg",
    rating: 4.24,
  },
  {
    id: 19103,
    name: "Half-Life 2: Lost Coast",
    background_image:
      "https://media.rawg.io/media/games/b7b/b7b8381707152afc7d91f5d95de70e39.jpg",
    rating: 3.45,
  },
  {
    id: 16944,
    name: "The Witcher 2: Assassins of Kings Enhanced Edition",
    background_image:
      "https://media.rawg.io/media/games/6cd/6cd653e0aaef5ff8bbd295bf4bcb12eb.jpg",
    rating: 4.16,
  },
  {
    id: 416,
    name: "Grand Theft Auto: San Andreas",
    background_image:
      "https://media.rawg.io/media/games/960/960b601d9541cec776c5fa42a00bf6c4.jpg",
    rating: 4.51,
  },
  {
    id: 23027,
    name: "The Walking Dead: Season 1",
    background_image:
      "https://media.rawg.io/media/games/8d6/8d69eb6c32ed6acfd75f82d532144993.jpg",
    rating: 4.35,
  },
  {
    id: 17822,
    name: "The Witcher: Enhanced Edition Director's Cut",
    background_image:
      "https://media.rawg.io/media/games/ee3/ee3e10193aafc3230ba1cae426967d10.jpg",
    rating: 4.06,
  },
  {
    id: 11973,
    name: "Middle-earth: Shadow of Mordor",
    background_image:
      "https://media.rawg.io/media/games/d1a/d1a2e99ade53494c6330a0ed945fe823.jpg",
    rating: 3.9,
  },
  {
    id: 290856,
    name: "Apex Legends",
    background_image:
      "https://media.rawg.io/media/games/737/737ea5662211d2e0bbd6f5989189e4f1.jpg",
    rating: 3.65,
  },
  {
    id: 19710,
    name: "Half-Life 2: Episode One",
    background_image:
      "https://media.rawg.io/media/games/7a2/7a2500ee8b2c0e1ff268bb4479463dea.jpg",
    rating: 4.37,
  },
];

const gameDataPageThree = [
  {
    id: 19709,
    name: "Half-Life 2: Episode Two",
    background_image:
      "https://media.rawg.io/media/games/198/1988a337305e008b41d7f536ce9b73f6.jpg",
    rating: 4.45,
  },
  {
    id: 4427,
    name: "BioShock 2",
    background_image:
      "https://media.rawg.io/media/games/157/15742f2f67eacff546738e1ab5c19d20.jpg",
    rating: 4.07,
  },
  {
    id: 9767,
    name: "Hollow Knight",
    background_image:
      "https://media.rawg.io/media/games/4cf/4cfc6b7f1850590a4634b08bfab308ab.jpg",
    rating: 4.4,
  },
  {
    id: 41,
    name: "Little Nightmares",
    background_image:
      "https://media.rawg.io/media/games/8a0/8a02f84a5916ede2f923b88d5f8217ba.jpg",
    rating: 4.04,
  },
  {
    id: 10035,
    name: "Hitman",
    background_image:
      "https://media.rawg.io/media/games/16b/16b1b7b36e2042d1128d5a3e852b3b2f.jpg",
    rating: 3.92,
  },
  {
    id: 2551,
    name: "Dark Souls III",
    background_image:
      "https://media.rawg.io/media/games/da1/da1b267764d77221f07a4386b6548e5a.jpg",
    rating: 4.4,
  },
  {
    id: 4252,
    name: "Mirror's Edge",
    background_image:
      "https://media.rawg.io/media/games/8e4/8e4de3f54ac659e08a7ba6a2b731682a.jpg",
    rating: 4.07,
  },
  {
    id: 3612,
    name: "Hotline Miami",
    background_image:
      "https://media.rawg.io/media/games/9fa/9fa63622543e5d4f6d99aa9d73b043de.jpg",
    rating: 4.38,
  },
  {
    id: 18080,
    name: "Half-Life",
    background_image:
      "https://media.rawg.io/media/games/6c5/6c55e22185876626881b76c11922b073.jpg",
    rating: 4.38,
  },
  {
    id: 3790,
    name: "Outlast",
    background_image:
      "https://media.rawg.io/media/games/9dd/9ddabb34840ea9227556670606cf8ea3.jpg",
    rating: 3.73,
  },
  {
    id: 1447,
    name: "Deus Ex: Mankind Divided",
    background_image:
      "https://media.rawg.io/media/games/00d/00d374f12a3ab5f96c500a2cfa901e15.jpg",
    rating: 3.97,
  },
  {
    id: 4161,
    name: "Far Cry 3",
    background_image:
      "https://media.rawg.io/media/games/15c/15c95a4915f88a3e89c821526afe05fc.jpg",
    rating: 4.24,
  },
  {
    id: 10754,
    name: "BioShock Remastered",
    background_image:
      "https://media.rawg.io/media/games/be0/be01c3d7d8795a45615da139322ca080.jpg",
    rating: 4.24,
  },
  {
    id: 11936,
    name: "Half-Life 2: Deathmatch",
    background_image:
      "https://media.rawg.io/media/games/ec3/ec3a7db7b8ab5a71aad622fe7c62632f.jpg",
    rating: 3.29,
  },
  {
    id: 58134,
    name: "Marvel's Spider-Man",
    background_image:
      "https://media.rawg.io/media/games/9aa/9aa42d16d425fa6f179fc9dc2f763647.jpg",
    rating: 4.45,
  },
  {
    id: 4332,
    name: "Spec Ops: The Line",
    background_image:
      "https://media.rawg.io/media/games/b49/b4912b5dbfc7ed8927b65f05b8507f6c.jpg",
    rating: 4.09,
  },
  {
    id: 19487,
    name: "Alan Wake",
    background_image:
      "https://media.rawg.io/media/games/5c0/5c0dd63002cb23f804aab327d40ef119.jpg",
    rating: 4.1,
  },
  {
    id: 13668,
    name: "Amnesia: The Dark Descent",
    background_image:
      "https://media.rawg.io/media/games/b54/b54598d1d5cc31899f4f0a7e3122a7b0.jpg",
    rating: 3.65,
  },
  {
    id: 3696,
    name: "Wolfenstein: The New Order",
    background_image:
      "https://media.rawg.io/media/games/c80/c80bcf321da44d69b18a06c04d942662.jpg",
    rating: 4.18,
  },
  {
    id: 4386,
    name: "Saints Row: The Third",
    background_image:
      "https://media.rawg.io/media/games/d69/d69810315bd7e226ea2d21f9156af629.jpg",
    rating: 3.95,
  },
];

export default extractedData;
